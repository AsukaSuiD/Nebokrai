//! Базовый 16-байтовый wire-буфер сообщений (`nets/basemessage.{h,cpp}`,
//! пары EXE/PDB шести служб в `server/rust/src/manifest/`). Каноническое
//! описание формата и статусов полей: docs/protocol/message-header.md.
//! Статус layout, курсора, `Update`, числовых/сырых `Add/Get`, `AddEx`,
//! строкового `Add`, обоих `GetStr`, `CGUID Add/Get` и корректных RLE-потоков:
//! `IMPLEMENTED`. `GetEx` и повреждённый RLE-marker имеют локальные статусы
//! `UNKNOWN` либо `BLOCKED_MISSING_FACT` ниже.
//!
//! Конструктор добавляет шестнадцать нулевых байт, записывает little-endian
//! Windows `long(16)` в первые четыре байта и ставит курсор на offset `16`.
//! `+4` доказан как `MsgType`; слова `+8` и `+0xC` буфер хранит opaque/reserved
//! и копирует буквально. Для исходящих game-сообщений клиента их смысл
//! установлен (`+8` — drift-оценка server uptime, `+0xC` — элемент validate-
//! последовательности; docs/reconstruction/client-wire-runtime.md), но сам
//! буфер её не интерпретирует. Числа хранятся побайтово little-endian с точными
//! размерами и signedness; обычная нехватка данных возвращала ноль и не
//! двигала курсор — Rust выражает эту потерю через `Option`/`bool`. После
//! каждого добавления первые четыре байта обновляются фактической длиной, как
//! исходный `Add` и inline `Update`. `CStr` сохраняет контракт `lstrlenA`
//! (`None` ничего не добавляет, завершающий нуль без требования UTF-8), а
//! маркер GUID — буквальную форму (`0` = invalid, иначе читаются 16 байт,
//! курсор после маркера не откатывается).
//!
//! Корректный RLE wire совпадает во всех вариантах: `0x00..=0xF7` — literal;
//! байт `0xF8..=0xFF` экранируется парой `0xF8, byte`; пара `marker, byte` —
//! повтор от одного до восьми раз; длинная серия дробится на `0xFF, byte` и
//! последнюю пару. Владеющий `Vec` заменяет запись в общий scratch-buffer.
//!
//! Аудит send-path доказал: все достигнутые send-владельцы копируют вход до
//! возврата, поэтому owned `Vec` безопасно заменяет lifetime общей памяти, а
//! `Initial`/`Release`/`DoTemptBuffer` аналогов не получают. Это не снимает
//! вопрос `m_CSTemptBuffer`: межпоточную сериализацию send каждый конкретный
//! `CMessage` решает после проверки своей потоковой достижимости.
//!
//! Экземпляр wire-буфера и момент отправки выбирает направление; Shared несёт
//! только байтовый формат и RLE-поток.
//! Доказательства: docs/reconstruction/shared-technical.md#базовый-wire-буфер-и-rle-cbasemessage
...
pub fn encode_rle(input: &[u8]) -> Result<Vec<u8>, RleEncodeError> {
...
pub fn decode_rle(input: &[u8], output_capacity: usize) -> Result<Vec<u8>, RleDecodeError> {
    if input.is_empty() {
...
impl CBaseMessage {
    ...
    pub fn from_header_and_payload(mut header: [u8; HEADER_LEN], payload: &[u8]) -> Self {
    ...
    pub fn wire_bytes_and_cursor_mut(&mut self) -> (&[u8], &mut usize) {
    ...
    pub fn get_char(&mut self) -> Option<i8> {
    ...
    pub fn get(&mut self, destination: &mut [u8]) -> bool {
    ...
    pub fn get_c_string_bytes(&mut self) -> Vec<u8> {
    ...
    pub fn get_str_bytes(&mut self, maximum: usize) -> Option<Vec<u8>> {
        if maximum == 0 {
            let _ = self.get_byte();
            return Some(Vec::new());
        }
    ...
    pub fn get_guid(&mut self) -> Option<CGuid> {
        if self.get_byte()? == 0 {
            return None;
        }
    ...
    pub fn add_char(&mut self, value: i8) {
    ...
    pub fn add_word(&mut self, value: u16) {
        self.append(&value.to_le_bytes());
    }
    ...
    pub fn add_long64(&mut self, value: i64) {
        self.append(&value.to_le_bytes());
    }
    ...
    pub fn add_str(&mut self, value: Option<&CStr>) {
    ...
    pub fn add_ex(&mut self, value: &[u8]) {
    ...
    pub fn add_guid(&mut self, value: CGuid) {
        if value.is_invalid() {
            self.add_byte(0);
        } else {
            self.add_byte(16);
