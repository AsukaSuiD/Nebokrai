...
pub struct RegionRoutePoint {
    pub x: i32,
    pub y: i32,
}

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct RegionNextNode {
...
pub struct RegionRouterNode {
    pub region_id: i32,
    pub entry: RegionRoutePoint,
    pub exit: RegionRoutePoint,
    pub exit_range: i32,
    pub next: BTreeMap<i32, RegionNextNode>,
}
...
pub struct RegionRouteStep {
    pub region_id: i32,
    pub point: RegionRoutePoint,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum RegionRouterChangeOutcome {
    RegionNotFound {
...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct RegionRouterLoadReport {
    pub declared_regions: u32,
    pub inserted_regions: u32,
    pub duplicate_regions: u32,
    pub duplicate_transitions: u32,
    pub trailing_tokens: usize,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RegionRouterDecodeField {
    RegionCount,
    RegionId,
    EntryX,
    EntryY,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct RegionRouterDecodeError {
    pub field: RegionRouterDecodeField,
...
#[derive(Debug)]
pub enum RegionRouterLoadError {
    Io(io::Error),
...
impl RegionRouter {
    ...
    pub fn region_info(&self, region_id: i32) -> Option<(RegionRoutePoint, i32)> {
    ...
    /// Декодирует GameServer snapshot. Оба исходных `std::map::insert`
    /// сохраняют первое значение duplicate key; текущий router очищается до
    /// чтения count, а незавершённый node не публикуется. Возвращает
    /// `(duplicate_regions, duplicate_transitions)` для журнала владельца.
    pub fn decord_from_byte_array(
        &mut self,
        source: &[u8],
        cursor: &mut usize,
    ) -> Result<(usize, usize), RegionRouterDecodeError> {
        self.clear();
    ...
    pub fn load_router_setup_bytes(
        &mut self,
        bytes: &[u8],
    ) -> Result<RegionRouterLoadReport, RegionRouterLoadError> {
        self.clear();
    ...
    pub fn connected_region_destination(
        &self,
        from_region: i32,
        to_region: i32,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct RegionRouterSerializeError {
