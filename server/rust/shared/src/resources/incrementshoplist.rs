...
pub struct IncrementShopItem {
    pub category: u16,
    pub overlapped_amount: u32,
    pub goods_id: u32,
    pub yuan_bao_price: u32,
    pub deduction_goods_id: u32,
    pub icon_id: u8,
...
pub struct CIncrementShopList {
    items: BTreeMap<u8, Vec<IncrementShopItem>>,
    affiche: Vec<u8>,
}

pub enum IncrementShopGoodsQuery<'name> {
    OriginalName(&'name [u8]),
    DisplayName(u32),
}

pub enum IncrementShopGoodsResult {
    Id(u32),
    Name(Option<Vec<u8>>),
}

pub struct IncrementShopLoadReport {
    pub warnings: Vec<Vec<u8>>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum IncrementShopLoadError {
    UnexpectedEnd,
...
impl CIncrementShopList {
    pub fn affiche(&self) -> &[u8] {
        &self.affiche
    }

    pub fn items_on_page(&self, page: u8) -> &[IncrementShopItem] {
        self.items.get(&page).map(Vec::as_slice).unwrap_or_default()
    }

    pub fn get_item(&self, goods_id: u32, page: u8) -> Option<&IncrementShopItem> {
        self.items_on_page(page)
            .iter()
            .find(|item| item.goods_id == goods_id)
    }

    pub fn search_items(&self, needle: &[u8]) -> Vec<&IncrementShopItem> {
        self.items
            .iter()
    ...
    pub fn insert(&mut self, page: u8, item: IncrementShopItem) {
        self.items.entry(page).or_default().push(item);
    }

    pub fn set_affiche(&mut self, affiche: &[u8]) {
        self.affiche.clear();
    ...
    pub fn load_from_bytes<ResolveGoods>(
        &mut self,
        source: &[u8],
...
pub enum IncrementShopStringField {
    Description,
    Key,
    Affiche,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum IncrementShopSerializeError {
    ItemCountOverflow,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum IncrementShopDecodeError {
    UnexpectedEnd {
        offset: usize,
        needed: usize,
        available: usize,
