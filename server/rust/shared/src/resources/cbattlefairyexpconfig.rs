...
//! Отрицательный signed count оригинальный decoder (decoder VA 0x5c7800,
//! выход по JE) принял бы за огромный unsigned loop с чтением за пределами
//! буфера; Rust трактует его как пустой вход (`max(0)`).
...
impl CBattleFairyExpConfig {
    ...
    pub fn contains_exp_list(&self, owner_level: u32) -> bool {
    ...
    pub fn exp_lists(&self) -> &BTreeMap<u32, Vec<u32>> {
        &self.exp_lists
    }
...
pub struct BattleFairyExpLoadReport {
    pub groups: usize,
    pub experience_values: usize,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum BattleFairyExpLoadError {
    MissingRoot,
    MissingOwnerLevel,
...
#[derive(Debug)]
pub enum BattleFairyExpFileLoadError {
    Io(io::Error),
...
pub struct BattleFairyExpSerializeError {
    pub owner_level: Option<u32>,
    pub count: usize,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct BattleFairyExpDecodeError {
    pub offset: usize,
    pub needed: usize,
    pub available: usize,
}
