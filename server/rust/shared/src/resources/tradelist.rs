...
#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct CTradeList {
    trades: BTreeMap<Vec<u8>, Trade>,
}

impl CTradeList {
    pub fn get_trade(&self, npc_name: &[u8]) -> Option<&Trade> {
    ...
    pub fn load_from_file<ResolveNpcName, ResolveGoodsId>(
        &mut self,
    ...
    pub fn load_from_bytes<ResolveNpcName, ResolveGoodsId>(
        &mut self,
        source: &[u8],
        resolve_npc_name: &mut ResolveNpcName,
        resolve_goods_id: &mut ResolveGoodsId,
...
impl Trade {
    pub fn goods(&self) -> &[TradeGoods] {
        &self.goods
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum TradeListFormatError {
...
#[derive(Debug)]
pub enum TradeListFileLoadError {
    Io(std::io::Error),
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum TradeListSerializeError {
