...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct DaKongExternalAttribute {
...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct DaKongLoadReport {
    pub delux_modify_missing: bool,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum DaKongLoadError {
    MissingMainFile,
}
...
impl CDaKongXiangQian {
    ...
    /// использует.
    pub fn get_add_type(destination: &mut BTreeSet<i32>) -> bool {
    ...
    pub fn clear_primary_state(&mut self) {
        self.info.clear();
    ...
    pub fn add_to_byte_array(&self, destination: &mut Vec<u8>) -> Result<(), DaKongSerializeError> {
    ...
    pub fn set_key(&mut self, enabled: bool) -> bool {
        self.key = enabled;
        enabled
    }
    ...
    pub fn set_log_key(&mut self, enabled: bool) -> bool {
        self.log_key = enabled;
        enabled
    }

    pub const fn log_key(&self) -> bool {
        self.log_key
    }

    pub fn delux_modify(&self) -> &[DaKongDeluxModify] {
        &self.delux_modify
    }

    /// Exact `GetSuccessProbability`: roll `0..9999` сравнивается через
    /// `<=`, поэтому нулевая вероятность сохраняет один успешный исход.
    pub fn get_success_probability(&self, index: i32, mut random: impl FnMut(i32) -> i32) -> bool {
    ...
    pub fn get_color(&self, index: i32, mut random: impl FnMut(i32) -> i32) -> i32 {
    ...
    pub fn make_sure_external_attribute(
        &self,
        group: i32,
        goods_id: u32,
    ...
    pub fn check_external_property(&self, goods_id: u32, group: i32) -> bool {
...
fn map_read_block(block: crate::protocol::LegacyReadBlock) -> DaKongDecodeError {
    DaKongDecodeError {
        offset: block.offset,
        needed: block.needed,
