...
//! чисел. `Vec` заменяет статический для процесса `std::vector`. Ответ клиента на
//! выданную строку установлен (send-обёртка `Nebokrai.exe` VA `0x43AC80` штампует
//! очередной элемент с текущей позиции в исходящий заголовок `+0x0C`, позиция
//! движется с зацикливанием), но входящий штамп исходный GameServer не читает
//! (docs/reconstruction/client-wire-runtime.md), поэтому Rust ответ не проверяет.
