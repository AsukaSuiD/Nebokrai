...
use super::regionparam::RegionParamState;
use nebokrai_shared::protocol::LegacyReader;
use thiserror::Error;

#[derive(Clone, Debug, Eq, Error, PartialEq)]
pub enum ProxyRegionDecodeError {
...
impl CProxyServerRegion {
    ...
    pub const fn set_country(&mut self, country: u8) {
        self.country = country;
    }

    /// Proxy наследует virtual `CServerRegion::ReSetWarState`; короткий wire
    /// эти runtime-поля не заменяет.
    pub const fn reset_war_state(&mut self, war_number: i32, state: i32) {
        self.war_number = war_number;
        self.city_state = state;
    }

    pub const fn war_state(&self) -> (i32, i32) {
        (self.war_number, self.city_state)
    }

    pub const fn on_war_declare(&mut self, war_number: i32) {
        self.war_number = war_number;
        self.city_state = 1;
    }

    pub const fn on_war_start(&mut self) {
        self.city_state = 3;
    }

    pub const fn on_war_end(&mut self) {
        self.war_number = 0;
        self.city_state = 0;
    }

    pub const fn on_war_mass(&mut self) {
        self.city_state = 2;
    }

    pub const fn set_owned_city_org(&mut self, faction_id: i32, union_id: i32) {
        self.param.owned_faction_id = faction_id;
        self.param.owned_union_id = union_id;
    }

    pub const fn owned_city_org(&self) -> (i32, i32) {
        (self.param.owned_faction_id, self.param.owned_union_id)
    }

    pub const fn war_region_type(&self) -> i32 {
        self.war_region_type
    }

    pub const fn param(&self) -> &RegionParamState {
        &self.param
    }

    /// Применяет авторитетный налоговый снимок World без локальной публикации.
    pub const fn set_tax_snapshot(
        &mut self,
        today_total_tax: u32,
        total_tax: u32,
...
fn read_u8(
    ...
    let value = reader
        .read_u8()
        .map_err(|block| proxy_error(field, block))?;
...
fn proxy_reader<'source>(
    source: &'source [u8],
    cursor: usize,
    field: &'static str,
    needed: usize,
) -> Result<LegacyReader<'source>, ProxyRegionDecodeError> {
    LegacyReader::at(source, cursor).map_err(|block| ProxyRegionDecodeError::UnexpectedEnd {
        field,
        offset: block.offset,
        needed,
...
) -> ProxyRegionDecodeError {
    ProxyRegionDecodeError::UnexpectedEnd {
        field,
        offset: block.offset,
        needed: block.needed,
        available: block.available,
