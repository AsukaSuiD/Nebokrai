...
//! `CServerRegion`. Исходник `appserver/area.cpp`; сверка по точной паре
//! `gameserver.exe` + `GameServer.pdb`.
//!
//! Блокировка исходного critical section не переносится: `CArea` физически
//! принадлежит закрытому массиву региона, а все изменяющие вызовы уже держат
//! исключительный `&mut CServerRegion`. Самоссылочный father-slot выражен
//! структурным `AreaParentLink::OwningServerRegion` без self-reference и
//! `unsafe` — это смена формы API, не контракта identity/topology. Подтверждённые
//! quirks сохранены: `RemoveObject` буквально не удаляет unknown-type hash из
//! `m_vOtherShapes`; `DelWarSoul` лишь помечает point `(-1,-1)`; goods
//! timestamp/protection удаляются только при успешном `CGoods` RTTI, а type 700
//! membership — по base GUID. Девять `Unwind`-funclet освобождения временного
//! STL allocation покрыты RAII и не имеют domain-семантики.
...
use super::shape::{MonsterAreaClass, ShapeResolver, ShapeRuntimeFacts, ShapeView};
...
pub enum AreaParentLink {
    Unassigned,
    OwningServerRegion,
}

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct WarSoulPoint {
    pub x: i32,
    pub y: i32,
}

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct AreaMonsterAiFacts {
    pub hibernated: bool,
    pub tamed: bool,
    pub carriage: bool,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum AreaWokenMonsterClass {
    Active,
    Pet,
    Carriage,
}

pub trait AreaAiContext {
...
impl CArea {
    ...
    pub const fn with_storage_defaults() -> Self {
    ...
    pub const fn assign_to_server_region(&mut self, x: i32, y: i32) {
    ...
    pub fn append_active_monster_ids(&self, destination: &mut Vec<i32>) {
        destination.extend_from_slice(&self.active_monsters);
        destination.extend_from_slice(&self.pets);
        destination.extend_from_slice(&self.carriages);
    }
    ...
    pub fn append_sleeping_monster_ids(&self, destination: &mut Vec<i32>) {
        destination.extend_from_slice(&self.sleeping_monsters);
    }

    pub fn take_sleeping_monster_ids(&mut self) -> Vec<i32> {
    ...
    pub fn restore_sleeping_monster(&mut self, id: i32) {
        self.sleeping_monsters.push(id);
    }

    pub fn push_woken_monster(&mut self, id: i32, class: AreaWokenMonsterClass) {
        match class {
    ...
    pub fn can_pick_up_goods(&self, ex_id: CGuid, player_id: i32, player_team_id: i32) -> bool {
    ...
    pub fn active_shape_candidates(&self) -> Vec<ShapeIdentity> {
    ...
    pub fn forget_unresolved_active_shape(&mut self, identity: ShapeIdentity) {
    ...
    pub fn begin_ai<Context: AreaAiContext>(
        &mut self,
    ...
    pub fn finish_ai<Context: AreaAiContext>(
        &mut self,
    ...
    pub fn del_war_soul(&mut self, player_id: u32, point: WarSoulPoint) -> bool {
        if player_id == 0 {
            return false;
        }
    ...
    pub fn find_war_souls(&self, destination: &mut BTreeMap<u32, WarSoulPoint>) {
    ...
    pub fn get_num_shapes(&self) -> u32 {
        [
            self.players.len(),
            self.active_monsters.len(),
    ...
    pub fn add_object(&mut self, identity: ShapeIdentity, facts: ShapeRuntimeFacts, now_ms: u32) {
    ...
    pub fn remove_object(&mut self, identity: ShapeIdentity, facts: ShapeRuntimeFacts) {
    ...
    pub fn find_shapes<Resolver: ShapeResolver>(
        &self,
        object_type: i32,
        resolver: &Resolver,
    ...
    pub fn append_player_ids(&self, destination: &mut Vec<i32>) -> bool {
        if self.parent != AreaParentLink::OwningServerRegion {
            return false;
        }
    ...
    pub fn append_pet_ids(&self, destination: &mut Vec<i32>) -> bool {
        if self.parent != AreaParentLink::OwningServerRegion {
            return false;
        }
        destination.extend_from_slice(&self.pets);
    ...
    pub fn append_carriage_ids(&self, destination: &mut Vec<i32>) -> bool {
        if self.parent != AreaParentLink::OwningServerRegion {
            return false;
        }
        destination.extend_from_slice(&self.carriages);
    ...
    pub fn get_all_shapes<Resolver: ShapeResolver>(
        &self,
        resolver: &Resolver,
        destination: &mut Vec<ShapeView>,
    ) -> bool {
    ...
    pub fn player_enter_neighbors(&self) -> [(i32, i32); 9] {
