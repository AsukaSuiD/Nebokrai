//! Агрегат `CMoveShape` (тип `MoveShapeState`): пространственная база
//! `CShape`, реестр навыков, каноническая арена состояний и скалярные
//! колонки, плюс общая основа wire-команд движения. Исходный владелец —
//! `appserver/moveshape.h/.cpp`; сверка по точной паре `gameserver.exe` +
//! `GameServer.pdb` (идентификаторы сборки —
//! `server/rust/src/manifest/_gameserver_export_manifest.toml`). Агрегат
//! материализован здесь вместе с `Default`, Deref-швом в арену состояний и
//! accessor-ами чтения/записи колонок; старый пакет хранит тонкую оболочку
//! `CMoveShape` над этим типом: делегаты навыков и арены состояний,
//! клиентские snapshot-фасады и dispatcher-оркестрация wire-команд. Основа
//! wire-команд `0xBF603/604/605` возвращает решение и готовый пакет
//! типизированным результатом; around-доставку и запись пространственного
//! членства выполняет dispatcher-обвязка старого пакета, а чтение span и
//! запрета клетки региона основа получает узким швом `RegionSpanView`,
//! реализованным переходным владельцем хранилищ у себя.
//!
//! Публичные символы семейства: `SetPosXY` (RVA `0x000CD050`),
...
//! свойств монстра.
//! Общая операция `CMoveShape::Stiffen` (RVA `0x000CD2F0`) ведёт окно/limit
//! по скалярам owner-а и получает setup value-формой
