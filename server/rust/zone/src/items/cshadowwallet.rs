...
pub trait ShadowCurrencyKind: CurrencyKind {
    const SOURCE_CONTAINER_EXTEND_ID: i32;
}

impl ShadowCurrencyKind for GoldCoinCurrency {
    const SOURCE_CONTAINER_EXTEND_ID: i32 = PlayerContainerKind::Wallet.extend_id();
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ShadowCurrencyAddBlock {
    MissingGoods,
    InvalidCurrency {
        expected: u32,
        actual: u32,
...
pub enum ShadowCurrencyAddOutcome {
    Added {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ShadowCurrencyRemovedCallback {
    pub accepted: bool,
    pub removed: Option<ShadowRemovedReport>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CShadowCurrencyContainer<K> {
...
impl<K: ShadowCurrencyKind> CShadowCurrencyContainer<K> {
    ...
    pub const fn on_source_added(&mut self) -> bool {
        false
    }

    /// Exact override удаляет shadow целиком при любом remove
    /// source-object и возвращает true даже для null/missing.
    pub fn on_source_removed(&mut self, goods_id: Option<CGuid>) -> ShadowCurrencyRemovedCallback {
...
pub type CShadowWallet = CShadowCurrencyContainer<GoldCoinCurrency>;
