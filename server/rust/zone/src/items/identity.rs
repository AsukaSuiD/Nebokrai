...
//!
//! Тело перенесено буквально из прежнего `src/gameserver/appserver/goods/identity.rs`;
//! единственное отличие — нормализация `pub(crate)`→`pub` на границе crate.
...
pub fn create_goods_guid() -> Option<CGuid> {
    match CGuid::create() {
        Ok(guid) => Some(guid),
        Err(error) => {
...
pub fn clone_goods_with_new_guid(source: &CGoods) -> Option<CGoods> {
