...
//! каждому live-контейнеру `CPlayer` соответствует wire-номер, которым
//! message boundary адресует его в `*_container_extend_id` полях.
//!
//! Значения собраны из уже установленных в коде констант и маршрутов, а не из
//! отдельной машинной сверки всей таблицы: equipment `2`
...
//! таблицы Zone `trade/currency.rs`.
//!
//! Каталог охватывает только плеер-контейнеры. Trade-рамка
//! (`plug << 8 | kind`) и session-упаковки extend-id им сознательно не
//! унифицируются; их таблицы остаются в Zone `trade/`. Depot-нумерация
//! размечается владельцем склада: wire extend-id `9` покрыт вариантом ниже, а
//! константа `161` — внутренний конец расширения позиций (`96 + 13·5`), не
//! wire-номер контейнера; вариант для неё здесь не вводится.
