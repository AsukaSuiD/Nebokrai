...
pub struct GoodsShadow {
    pub original_container_type: i32,
    pub original_container_id: i32,
    pub original_container_extend_id: i32,
    pub original_goods_position: u32,
    pub goods_id: CGuid,
    pub goods_base_properties_index: u32,
    pub goods_amount: u32,
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CGoodsShadowContainer {
...
impl CGoodsShadowContainer {
    ...
    pub const fn set_container_extend_id(&mut self, extend_id: i32) {
        self.container_extend_id = extend_id;
    }

    pub const fn container_extend_id(&self) -> i32 {
        self.container_extend_id
    }

    pub const fn shadows(&self) -> &BTreeMap<CGuid, GoodsShadow> {
        &self.shadows
    }
    ...
    pub const fn is_supported_player_container(extend_id: i32) -> bool {
        matches!(
            PlayerContainerKind::from_extend_id(extend_id),
            Some(
                PlayerContainerKind::Packet
                    | PlayerContainerKind::Equipment
                    | PlayerContainerKind::Wallet
                    | PlayerContainerKind::YuanBao,
            )
        )
    }
    ...
    pub fn insert_shadow_record(&mut self, record: GoodsShadow) -> ShadowRecorded {
    ...
    pub fn goods_id_at(&self, position: u32) -> Option<CGuid> {
    ...
    pub fn goods_source_container_extend_id(&self, goods_id: CGuid) -> i32 {
        self.shadows
            .get(&goods_id)
            .map_or(0, |shadow| shadow.original_container_extend_id)
    }

    pub fn shadow_position(&self, previous: &PreviousContainer) -> Option<u32> {
        self.shadows
            .iter()
            .find(|(_, shadow)| {
    ...
    pub fn find<'a, Resolve>(&self, goods_id: CGuid, mut resolve: Resolve) -> Option<&'a CGoods>
    where
    ...
    pub fn get_goods<'a, Resolve>(&self, position: u32, mut resolve: Resolve) -> Option<&'a CGoods>
    where
        Resolve: FnMut(&GoodsShadow) -> Option<&'a CGoods>,
    {
    ...
    pub fn get_first_goods<'a, Resolve>(
        &self,
    ...
    pub fn get_goods_by_base_properties<'a, Resolve>(
        &self,
        base_properties_index: u32,
        mut resolve: Resolve,
    ) -> Vec<&'a CGoods>
    ...
    pub fn traversing_goods<'a, Resolve>(&self, mut resolve: Resolve) -> Vec<&'a CGoods>
    where
        Resolve: FnMut(&GoodsShadow) -> Option<&'a CGoods>,
    {
        self.shadows
    ...
    pub fn contents_weight<Resolve>(&self, factory: &CGoodsFactory, mut resolve: Resolve) -> u32
    where
    ...
    pub fn add_shadow_report(&self, goods_id: CGuid) -> Option<ShadowPresenceReport> {
    ...
    pub fn on_source_removed(&mut self, goods_id: CGuid, amount: u32) -> ShadowSourceChangeOutcome {
        let Some(shadow) = self.shadows.get_mut(&goods_id) else {
            return ShadowSourceChangeOutcome::Missing;
        };
        if shadow.goods_amount < amount {
