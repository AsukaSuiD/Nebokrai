...
use super::cgoods::{CGoods, GoodsBasePropertyBlock, GoodsDecodeError};
...
pub const EQUIPMENT_COLUMN_LIMIT: u32 = 17;
pub const EQUIPMENT_AROUND_UPDATE_MESSAGE_TYPE: u32 = 0x0b_f720;
pub const EQUIPMENT_FAIRY_UPDATE_MESSAGE_TYPE: u32 = 0x0b_f918;
...
pub struct EquipmentClearedGoods {
    pub column: EquipmentColumn,
    pub goods: CGoods,
    pub listeners: Vec<ContainerListenerHandle>,
}

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct EquipmentAddPartialEffects {
    pub start_point_initialized: bool,
    pub registered_with_goods_ai: bool,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum EquipmentAddBlock {
    MissingGoods,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct EquipmentOwnerPlayerFacts {
    /// Exact `CanMountEquip` должен вернуть magic success-code 9.
    pub can_mount_result: i32,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct EquipmentAddRuntimeFacts {
    /// `None` означает, что lookup игрока по owner id не дал объект.
    pub owner_player: Option<EquipmentOwnerPlayerFacts>,
    pub pack_add_enabled: bool,
    pub now: u64,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct EquipmentAroundUpdate {
    pub owner_id: i32,
    pub column: EquipmentColumn,
    pub added: bool,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct EquipmentPlayerAddedEffects {
    pub add_war_soul_skill: bool,
    pub recompute_properties: bool,
    pub around_update: EquipmentAroundUpdate,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct EquipmentPlayerRemovedEffects {
    pub clear_war_soul_status: bool,
    pub delete_war_soul_skill: bool,
    pub recompute_without_removed_slot: bool,
    pub clamp_hp_and_mp: bool,
    pub around_update: EquipmentAroundUpdate,
}

#[must_use = "report содержит обязательные internal/player/listener эффекты"]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EquipmentAddedReport {
    pub owner_type: i32,
    pub owner_id: i32,
    pub column: EquipmentColumn,
    ...
    pub base_properties_index: u32,
    pub amount: u32,
    pub partial_effects: EquipmentAddPartialEffects,
    pub player_effects: Option<EquipmentPlayerAddedEffects>,
    pub package_extension_applied: bool,
    pub package_extension_delta: u32,
    pub listeners: Vec<ContainerListenerHandle>,
}

#[must_use = "blocked add может уже содержать timed/AI partial effects"]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EquipmentAddBlockedReport {
    pub reason: EquipmentAddBlock,
    pub partial_effects: EquipmentAddPartialEffects,
}

#[must_use = "blocked add может уже содержать timed/AI partial effects"]
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EquipmentAddOutcome {
    Added(EquipmentAddedReport),
    Blocked(EquipmentAddBlockedReport),
}

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct EquipmentRemovePartialEffects {
    /// Значение уже вычтено из `expanded_package_num` с wrapping semantics.
    pub package_extension_subtracted: u32,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct EquipmentRemoveRuntimeFacts {
    /// Lookup игрока по owner id для derived remove-callback-а.
    pub owner_player_present: bool,
    pub pack_add_enabled: bool,
    /// Результат player-wide `GetGoodsById` после QueryAttribute/value1==2.
    pub player_goods_package_extension: Option<u32>,
    /// Уже вычисленный exact skill gate `battle fairy && Can...()==0`.
    pub active_war_soul_blocks_headgear: bool,
}

#[must_use = "report сохраняет removed ownership и callback ordering"]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EquipmentRemovedEvent {
    pub owner_type: i32,
    pub owner_id: i32,
    pub column: EquipmentColumn,
    pub partial_effects: EquipmentRemovePartialEffects,
    pub player_effects: Option<EquipmentPlayerRemovedEffects>,
    pub listeners: Vec<ContainerListenerHandle>,
}

#[must_use = "report сохраняет removed ownership и callback ordering"]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EquipmentRemovedReport {
    pub event: EquipmentRemovedEvent,
    pub goods: CGoods,
}

#[must_use = "missing/blocked remove может уже изменить package-extension счётчик"]
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EquipmentRemoveOutcome {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct EquipmentSwapRuntimeFacts {
    pub pack_add_enabled: bool,
    /// `None` означает отсутствие player-а для pre-remove capacity guard.
    pub active_unused_package_slots: Option<u32>,
    pub remove: EquipmentRemoveRuntimeFacts,
    pub incoming_add: EquipmentAddRuntimeFacts,
    /// Новый clock/player snapshot для legacy rollback `Add(old)`.
    pub rollback_add: EquipmentAddRuntimeFacts,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum EquipmentSwapBlock {
    MissingIncoming,
    MissingBaseProperties {
        index: u32,
    },
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EquipmentUnserializedEntry {
    DecoderReturnedNull {
        position: u32,
    },
    Added {
...
pub struct EquipmentUnserializeReport {
    pub cleared: Vec<EquipmentClearedGoods>,
    pub entries: Vec<EquipmentUnserializedEntry>,
}

#[must_use = "failure сохраняет уже очищенный и частично заполненный state"]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EquipmentUnserializeFailure {
    pub error: EquipmentContainerCodecError,
    pub report: EquipmentUnserializeReport,
}

#[derive(Clone, Copy, Debug)]
pub struct EquipmentFairyExpRuntimeFacts {
    pub grow_log_enabled: bool,
    pub egg_max_level: u32,
    pub upgrade_rate: f32,
    pub remove: EquipmentRemoveRuntimeFacts,
    pub replacement_add: EquipmentAddRuntimeFacts,
}

#[derive(Clone, Copy, Debug)]
pub struct EquipmentBattleFairyExpRuntimeFacts {
    pub player: Option<BattleFairyPlayerFacts>,
    pub remove: EquipmentRemoveRuntimeFacts,
    pub replacement_add: EquipmentAddRuntimeFacts,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum EquipmentFairyMoveOperation {
    DeleteObject,
    NewObject,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EquipmentFairyObjectMove {
    pub operation: EquipmentFairyMoveOperation,
    pub owner_type: i32,
    pub owner_id: i32,
    pub column: EquipmentColumn,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EquipmentFairyGoodsUpdate {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EquipmentFairyTransitionEffect {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EquipmentFairyTransition {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EquipmentFairyExpOutcome<Effects> {
    NoChange(FairyExpReport<Effects>),
    Updated {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum EquipmentFairyExpBlock {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum EquipmentBattleFairyExpOutcome {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum EquipmentBattleFairyExpBlock {
...
impl CEquipmentContainer {
    ...
    pub const fn expanded_package_num(&self) -> u32 {
    ...
    pub const fn set_expanded_package_num_snapshot(&mut self, value: u32) {
    ...
    pub fn is_slot_empty(&self, column: EquipmentColumn) -> bool {
    ...
    pub fn find(&self, goods_id: CGuid) -> Option<&CGoods> {
        self.equipment
            .values()
            .find(|goods| goods.identity().ex_id == goods_id)
    }

    pub fn find_mut(&mut self, goods_id: CGuid) -> Option<&mut CGoods> {
        self.equipment
            .values_mut()
            .find(|goods| goods.identity().ex_id == goods_id)
    }

    pub fn query_goods_position_by_id(&self, goods_id: CGuid) -> Option<EquipmentColumn> {
        self.equipment
            .iter()
    ...
    pub fn query_goods_position(&self, goods: &CGoods) -> Option<EquipmentColumn> {
        self.equipment
            .iter()
    ...
    pub fn traversing_goods(&self) -> Vec<(EquipmentColumn, &CGoods)> {
        self.equipment
            .iter()
    ...
    ) -> Result<EquipmentFairyExpOutcome<Effects>, EquipmentFairyExpBlock>
    where
    ...
    fn replace_headgear_fairy(
        ...
        let mut effects = vec![
            EquipmentFairyTransitionEffect::ObjectMove(EquipmentFairyObjectMove {
                ...
                container_extend_id: PlayerContainerKind::Equipment.extend_id() as u32,
                goods: old_identity,
    ...
    pub fn does_equip_place_fit(column: EquipmentColumn, equip_place: i32) -> bool {
        match equip_place {
    ...
    pub fn unserialize_with<Decode, Runtime>(
        &mut self,
        source: &[u8],
        cursor: &mut usize,
        factory: &CGoodsFactory,
    ...
    pub fn clear(&mut self) -> Vec<EquipmentClearedGoods> {
