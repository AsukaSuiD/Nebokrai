...
use crate::content::goods::{GAP_BF_GEM, GAP_GEM_TYPE};
...
pub enum UpgradeShadowAddBlock {
    UnsupportedGoods,
...
pub struct UpgradeShadowAdded {
    pub cell: UpgradeEquipmentCell,
    pub shadow: AmountShadowAdded,
}

#[must_use = "callback outcome определяет amount/remove и slot-map эффекты"]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct UpgradeShadowSourceRemoved {
    pub shadow: ShadowSourceChangeOutcome,
    pub erased_cell: Option<UpgradeEquipmentCell>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct CEquipmentUpgradeShadowContainer {
...
impl CEquipmentUpgradeShadowContainer {
    ...
    pub fn positions(&self) -> &BTreeMap<UpgradeEquipmentCell, CGuid> {
        &self.positions
    }
