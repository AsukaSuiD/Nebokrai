//! Денежное ядро обмена и маршруты bank/ground валюты GameServer,
...
//! currency разведки «Zone player: машинная разведка trade/auction/bank/
//! ground currency» от 26 сентября 2026:
//!
...
//!
//! Риск-нота merge (разведка порции T): машинный пересчёт commit выполняет
...
//! не «улучшается».
//!
//! Швы переноса: таблицы и предикаты перенесены буквально (pub/пути
//! нормализованы); живые контейнеры wire достаёт прежний owner, который
//! вызывает правила ниже в исходном порядке. `CBank` lock-gate отдельно от
//! `CDepot` и позиции `<0x60/0x30` — свидетельства контейнерной порции, здесь
//! не переоткрываются.
