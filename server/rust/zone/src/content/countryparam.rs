...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct CountryExilePoint {
    pub region_id: i32,
    pub x: i32,
    pub y: i32,
    pub legacy_fourth: i32,
}

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct CountryTechLevel {
    pub country_tech_exp: i32,
    pub country_power: i32,
}

#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct CountryMainReturnPoint {
    pub region_id: i32,
    pub rect: CountryMainRect,
    pub direction: i32,
}
...
pub struct CountryParamInputBlock {
...
impl CCountryParam {
    ...
    pub const fn exile_time(&self) -> Option<i32> {
        self.parameters[EXILE_TIME]
    }

    pub const fn max_exploit(&self) -> Option<i32> {
        self.parameters[MAX_EXPLOIT]
    }

    pub const fn exploit_increment(&self) -> Option<i32> {
        self.parameters[INC_EXPLOIT]
    }

    pub fn country_tech_level(&self, level: i32) -> Option<&CountryTechLevel> {
        self.country_tech_levels.get(&level)
    }

    pub fn country_tech_level_count(&self) -> i32 {
        self.country_tech_levels.len() as i32
    }

    pub fn exile_rect(&self, country: u8) -> Option<CountryMainRect> {
        self.exile_rects.get(&country).copied()
    }

    pub fn exile_point(&self, country: u8) -> Option<CountryExilePoint> {
    ...
    pub fn exile_rects(&self) -> &BTreeMap<u8, CountryMainRect> {
        &self.exile_rects
    }

    /// Сохраняет три mutating `operator[]` lookup-а оригинала.
    pub fn main_return_point(&mut self, country: u8) -> CountryMainReturnPoint {
...
fn read_country_u8(
    ...
    let value = reader
        .read_u8()
        .map_err(|block| country_error(field, block))?;
