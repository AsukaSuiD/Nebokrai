//! Реестр запуска и поисковая часть `CGoodsFactory` GameServer, перенесённые
//! в Zone `content/` вместе с DaKong-алгоритмом камней исходного владельца.
//!
...
//!
//! Волна Z-G0b: тела перенесены буквально из прежнего
//! `src/gameserver/appserver/goods/cgoodsfactory.rs`; отличия — нормализация
//! `pub(crate)`→`pub` на границе crate и замкнутость зависимостей на zone/shared:
//! `CGoods` и типы addon-ов — Zone `items/cgoods.rs`, GAP-константы и
//! `CGoodsBaseProperties` — Zone `content/goods.rs`, таблицы
//! `CDaKongXiangQian` — Shared resources. Точное DaKong-семейство того же
//! исходного владельца `cgoodsfactory.cpp` (`DealEnchaseGem`, ветви
//! `DealWithExternAttr`, условие седьмого слота, снимки камня/предмета и типы
//! его событий) было реконструировано ранее в сессионном `cequipmentdakong.rs`
//! и теперь возвращено к фабрике; прежний сессионный файл держит сессионные
//! типы и реэкспорт для старого пакета. RNG поступает generic-замыканиями
//! вызывающей стороны — адаптер `with_legacy_random_stream` остаётся в `CGame`
//! без переноса. Шов `GoodsBasePropertiesLookup` из `items/cgoods.rs`
//! реализован здесь же делегацией inherent-поиску: impl-точка переехала вместе
//! с владельцем реестра, старый shim-impl удалён.
//!
