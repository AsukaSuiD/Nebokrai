...
//! стопки и поштучное создание остальных типов; обычное улучшение ещё требует
//! реконструкции. Магазин NPC замыкает формулы ремонта и продажи с
...
//!
//! DaKong-семейство того же исходного владельца `cgoodsfactory.cpp`
//! (`DealEnchaseGem`, ветви `DealWithExternAttr`, условие седьмого слота,
//! снимки камня/предмета и типы его событий) живёт здесь у владельца
//! реестра; сессионный `cequipmentdakong` старого пакета держит сессионные
//! типы и re-export. RNG поступает generic-замыканиями вызывающей стороны —
//! адаптер `with_legacy_random_stream` остаётся в `CGame`. Шов
//! `GoodsBasePropertiesLookup` из `items/cgoods.rs` реализован здесь
//! делегацией inherent-поиску. Таблицы `CDaKongXiangQian` — Shared resources.
