...
//! (dash/flash/littleflash/rush).
//!
//! Путь и список поражённых принадлежат concrete-владельцам и не копируются
//! через callback атаки; контакт — общий оружейный расчёт с TARGET_DAMAGE_FACTOR.
//! Региональные швы `dash_*` перечитывают owner-а на каждый вызов (native
//! держит один borrow региона на весь цикл AI); недостижимое исчезновение
//! посередине — закрытая клетка (`unwrap_or(1)`).
//!
//! Сознательное отклонение от native-UB: `check_dash_path` обращается к
//! `back()` пути даже после подрезки `truncate(maximum)` до нуля — пустой
//! путь остаётся безопасным отказом без выдуманной клетки назначения.
//!
//! UNKNOWN: место push в Attack-списке CFlash, потребители raw CAN
//! `available`.
//!
//! Исходные владельцы PDB: `appserver/skills/{flash,littleflash,littleflash2}.cpp`.
