...
//! impactattack (объявленные швы ниже), общий lifecycle и поколенческий ключ —
//! playercast прежнего пакета.
//!
...
//!
//! Pillar-гейт отбрасывания: цель не отбрасывается при живом состоянии навыка
//! `0x74`. По факту `0x74` — это `PILLAR_SKILL_ID` защитной стойки CPillar
//! (`skills/pillar.rs`; Rage — 0x6D, RageBreak — 0x6e, другая полоса):
//! прежняя локальная константа при переносе заменена семейной.
//!
//! Объявленные швы переноса (не расхождения): hub-трейты `ThunderBlow2Game`/
//! `ThunderBlow2Player`/`ThunderBlow2MoveShape`/`ThunderBlow2Contact` —
//! переходные фасады прежнего владельца `CGame`/`CPlayer`/`CMoveShape`
//! (реализация у делегата старого пакета `appserver/skills/thunderblow2.rs`);
//! имена членов сохраняют исходную операцию. `knock_back_impact_target`
//! (общий шаг отбрасывания) и `apply_thunder_blow_2_attack` (формула,
//! raw-контакт без RP) остаются у владельца `appserver/skills/impactattack.rs`
//! и открыты только как швы. Региональные фасады перечитывают owner-а на
//! каждый вызов (первичный гейт существования региона сохранён, прецедент
