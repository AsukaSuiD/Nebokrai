...
//!
//! Quirks: отказ Check дополнительно отправляет visual2 перед End(0); AI не
//! повторяет оружейную проверку; обход ограничивает окно 5×5 размерами региона
//! включительно, каждая клетка заново вызывает одиночный GetShape после
//! предыдущего наложения — нет снимка целей, дедупликации и начисления RP;
//! End(1) вызывается после обхода даже без целей.
//!
//! Швы: hub `selfcast::*`; общий зарегистрированный вход и visual2 по отказу
//! Check — у `playercast` делегата; одиночный GetShape, SAFE-гейт и размеры
//! региона — одноимённые швы hub. Создание состояния — `skills/roarstate.rs`.
//!
//! Исходный владелец PDB: `appserver/skills/roar.cpp/.h`.
