...
mod elementphalanx; // снимок и числовой расчёт элементального удара призванных областей; живое применение попадания и проход боевых духов.
pub mod energyholding; // Check/AI накопления энергии CEnergyHolding (hub-швы `selfcast`).
pub mod energyholdingstate; // живые add/consume/restart/End CEnergyHoldingState над швами `selfcast`.
// Исполнение зарегистрированного навыка: typed payload player/BF и полная
// запись реестра; hub-monster payload подключается generic-сваркой.
pub mod execution;

mod firewall; // правила призыва и маска области CFireWall.
mod fatalblow; // Check/AI и Summon CFatalBlow (0x21C) над hub-швами `battlefairyskill`.
mod fatalblowphalanx; // снаряд CFatalBlowPhalanx: форма, тики, клиентский снимок и формула.
pub mod flash; // CFlash (0x69): Check/AI рывка, visual, master_info/target_level семейства.
pub mod fury; // общая RP-подготовка CFury/CRageBreak (check/расход RP, свип девяти id) и RP-шов RageCastPlayer; сам CFury остаётся у прежнего владельца (хелперы размещены здесь).
...
mod leiming2; // CLeiming2 (0x21B): семейные с CThunder Check/AI и Summon с AddElementAtk-слагаемым.
mod lifecycle; // база и стадии живого навыка.
mod lifeshield; // Check/AI CLifeShield (0x220) над hub-швами `battlefairyskill`.
...
pub mod poisonarrow; // CPoisonArrow (0x21E): Check/AI и позднее наложение яда буквально; общая для стрел 0x21D/0x21E обёртка `execute_periodic_battle_fairy_arrow` (hub `battlefairyskill` + швы арены ядов/PK).
mod poisonfog; // данные и живая форма области CPoisonFogPhalanx; тело Summon.
pub mod poisonfogstate; // живые Begin/update/restart/AI/End CPoisonFogState.
pub mod poisonmoth; // CPoisonMoth (0xCF): Check/AI поклеточного выстрела буквально (квазнота MAX+1 против MAX в Check, двойной visual(3), одна клетка за тик, visual-target перед ударом, (0,0)-гейт); швы семей `rangedweaponcast`/`crossbowattack`.
pub mod promotion; // Check/AI CPromotion (hub-швы `statecast`).
pub mod promotionstate; // живой Begin/restart CPromotionState и wire-тип его пакета.
...
pub mod ragebreak; // CRageBreak (0x6E): Check/AI и порядок состояний (End+dtor прежнего 0x6E → новый state → свип девяти id → только-End первого 0x131 → CCureState → UpdateProperty → End(1)); visual с DWORD-формой mode 8 (hub-швы `statecast`+`fury`).
pub mod ragebreakstate; // живые Begin×3/End/Restart/AI и пересчёт CRageBreakState, общие AttackGain-callbacks семьи и consume для ThunderSlash (hub-шов `statecast` + AttackGain-фасады CGame).
pub mod roar; // Check/AI и границы обхода клеток CRoar (hub-швы `selfcast`).
pub mod roarstate; // живые replace/restart/AI/End и OnUpdateProperties CRoarState.
...
pub mod selfcast; // hub-швы self/zone-кастов семьи blind/energyholding/roar/pillar/callosity/soulmirror (реализация фасадов у прежнего владельца).
...
mod snowstorm; // данные и живая форма области CSnowStormPhalanx; тело Summon и применение окна.
pub mod soulmirror; // маска, параметры клетки и живой обход области CSoulMirror (hub-швы `selfcast`).
pub mod spiderpoison; // CSpiderPoison (0x191): Check/AI/Attack/Calc и позднее наложение яда буквально поверх hub `baseattackruntime`; общая арена ядовой линии `SpiderPoisonStateArena` (Cure-факт, замена первого 0x191); обвязка stateskill остаётся hub прежнего пакета.
pub mod spidermist; // CSpiderMist (0x198): Check/AI/Summon буквально, область CSpiderMistPhalanx (маска, AI обхода, entry 0xBF502), hub-швы семьи summoncreatureskill.
pub mod statecast; // общие hub-швы и wire-кадр visual state-кастов пятёрки и heal-квартета (реализация фасадов у прежнего владельца).
...
mod weak; // правила и живая форма CWeakPhalanx; тело Summon CWeak.
pub mod weakstate; // живые Begin/update/restart/AI/End и смена региона CWeakState.
mod wuxing; // ID и подготовка 24 параметров пяти состояний У-син.
mod yinyang; // параметры и маски областей CYinYang и CYinYang2; тела их Summon.
pub mod yunshenglightning; // CYunShengLightning (0x19E): кадры visual, формулы и dispatch-предикаты; hub-оркестрация у делегата.
pub mod zonalcast; // hub-швы и скелет Begin/Check/AI/visual/End областных призывов Weak/PoisonFog/SnowStorm/YinYang[2]/GodThunder[2]/FireWall/ChaosSphere/SoulMirror.
...
// Zonalcast-хаб: общий скелет и швы областных призывов, тела Summon
// владельцев, композиты областей и применение элементных ударов.
...
// Семьи боевых навыков монстров: hub-трейты доставки и тела навыков
// 0x2bd/0x2d1/0x2ef/0x197/0xd4.
...
// Трупная/ядовая state-линия: hub-фасады и тела навыков 0x191 (stateskill-
