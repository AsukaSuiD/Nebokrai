...
//! `CThunderBlowPhalanx`. Пара навыка и области слита в один файл:
//! конструктор области вызывается только здесь.
//!
//! Машинный порядок AI (важно): списание MP, обновление состояния (+0x164) и
//! поворот выполняются ДО повторной проверки дальности — её отказ теряет уже
//! списанные MP и поворот, общий хвост AfterUse выполняется (достижимо только
//! при смене цели/пути между Check и AI). Собственный `End` не меняет
//! движение, но вызывает оружейный `CAttackSkill::End`; все исходы —
//! один хвост с reuse. Формула формы — 2 RNG (диапазон, крит) с x87-критом.
//!
//! Швы: hub-трейты `ThunderBlow*` — фасады прежнего владельца (делегат
//! `appserver/skills/thunderblow.rs` старого пакета); свёртка и регистрация
//! области с безусловным `0xBF502` — фасад `add_thunder_blow_phalanx` владельца
//! региона; `time_reached` — локальная копия адаптера baseattack.
//!
//! UNKNOWN: запись CAN=10006 → `[+0x3C]` в player-kernel не материализуется
//! (конвенция summon-полосы).
//!
//! Исходные владельцы PDB: `appserver/skills/thunderblow.cpp`,
//! `thunderblowphalanx.cpp`.
//! Доказательства: docs/reconstruction/gameserver-skills.md#thunderblow--cthunderblow-0x13f-и-cthunderblowphalanx
...
/// Общий адаптер прежнего `baseattack`, сохранённый буквально.
