...
//! `src/gameserver/appserver/skills/thunderphalanx.rs`; живое тело области
//! (форма, окна, тики, wire, calc) перенесено буквально порцией T2
//! «BF-облака области»; run-делегации (Attack-обход, регистрация) остаются
//! у прежнего владельца. Размещение — отдельный flat-модуль: Initialize и
//! AddToByteArray у фаланги ICF-фолднуты с `CGodThunderPhalanx2`, но
//! ctor/Calc/Attack/AI собственные, поэтому класс живёт рядом с
//! `godthunder`, а не внутри него; общая маска 7×7 берётся из
//! `godthunder` повторно, без дублирования.
//!
...
//!   остаются у прежнего владельца (региональные run-делегации этой
//!   порцией не переносятся).
...
//!   `n = ([B0]/[C4])*[DC]` + raw 8n относится к отдельной поздней
//!   phalanx-хвостовой порции и здесь не переоткрывается: wire сохраняет
...
/// (порция T2); открывают только прежние обращения, имена сохраняют исходную
/// операцию. Реализация — у делегата старого пакета
/// `appserver/skills/thunderphalanx.rs`.
