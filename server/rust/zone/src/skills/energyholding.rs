...
//!
//! Quirks: Check проверяет RTTI первого state с ID 0x89 — отсутствие
//! состояния не подменяется нулём зарядов при проверке предела; типизированный
//! первый state 0x89 увеличивается без чтения параметров нового; успех
//! накопления не меняет завершающий End(1).
//!
//! Швы: hub `selfcast::SelfCastGame`; машина накопления `accumulatedstate`
//! объявлена швом `add_energy_holding_state` (владелец поделён с SoulCollect
//! и пока остаётся у старого пакета).
//!
//! Исходный владелец PDB: `appserver/skills/energyholding.cpp`.
