...
//!
//! Quirks: AI без задержки — первый проход расходует MP, задаёт
...
//! усилить только игрока; прежний первый state144 — End + dtor той же
//! позиции, новый — Begin(U,S) → append → UpdateProperty цели → End(1).
//!
//! Швы: hub `statecast::*` — у владельца старого пакета; состояние и его
//! hub-lifecycle — `heartenstate` семья.
//!
//! Исходные владельцы PDB: `appserver/skills/hearten.cpp/.h`,
//! `heartenstate.cpp/.h`.
//! Доказательства: docs/reconstruction/gameserver-skills.md#hearten--chearten-0x144
