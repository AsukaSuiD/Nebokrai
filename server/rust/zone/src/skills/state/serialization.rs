//! DB Save/Load-кодек применённых состояний `CMoveShape`. Contract:
//! docs/gameplay/attributes-and-states.md.
//! Перенесён из hub `appserver/moveshape.rs`: Save/Load-семья
//! (`serialize_ex_states_for_save`, `serialized_ex_states`, общий обход
//! записей, `replace_ex_states`, `clear_persisted_runtime_state`) вместе с
//! примитивами кодека. Арена записей — соседний `storage`; decode одиночной
//! записи — `skills::statefactory`.
//!
//! Нормализация при переносе (тела буквальные): методы переходного CMoveShape
//! → свободные функции над заимствованными `LegacyStateCodec` и
//! `AppliedStateEntries`; identity — значением `ShapeIdentity`, часы —
//! clock-замыканием caller-а. Уплотнение Save вызывает `compact` арены
//! напрямую (hub `compact_state_slots` остаётся для живого UpdateAbnormality).
//! Бинарные примитивы read/write `pub` — ими пользуются RAW-операции hub
//! moveshape.
//!
