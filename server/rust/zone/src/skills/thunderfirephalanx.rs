...
//! `appserver/skills/thunderfirephalanx.cpp/.h`. Прежний переходный владелец —
//! `src/gameserver/appserver/skills/thunderfirephalanx.rs`; тела перенесены
//! порцией T4.
//!
...
//!
//! Машинный факт MATCH (FIX #2 порции T4, якорь AI `0x5E1970`):
//!
//! ```text
...
//! порцией T4, исчерпание проверяется в точке due (см. `tick`). После End
...
//! `0x5E1310`; по решению волны блок не добавляется и не удаляется,
//! вопрос остаётся открытым (см. тело `calculate_owned_thunder_fire_attack`).
