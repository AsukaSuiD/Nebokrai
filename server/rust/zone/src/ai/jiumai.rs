...
//! Тела дочитаны машинно волной Z-AI (6 из 6 функций класса, кроме ctor
//! `0x0060A5D0`, не читанного построчно — его известные эффекты зафиксированы
...
//! | hurt-отход: найденный ориентир задаёт направление формы `CShape::SetDir` (owner vtable `+0x60`, `0x0044A1C0`) перед `GetDirPos` и общим координатным `CBaseAI::MoveTo(run=0)` (AI vtable `+0x58`, `0x004C9020`); отсутствие ориентира сохраняет текущее `GetDir` (vtable `+0x5C`) | RVA-место `0x0060A89F`/`0x0060A8E7`; vtable `CShape` `0x0064EA04` | [`retarget_jiumai_after_hurt`] (запись направления) | `MATCH`; прежний hub эту запись не выполнял — исправлено волной Z-AI |
//!
...
//!
//! Граница порции Z-AI (не расхождения): общий monster tick hub — `Run`
//! (`0x0060E250` → `CBaseAI::Run` `0x004C7D10`), материализация active/passive
//! FIFO, реальный путь `monsterbaseattack` и runtime-входы `CGame` остаются
//! hub-владением своих порций. `CGame` по-прежнему владеет журналированием,
//! пакетами, привязкой игрока и фактическим спавном/переносом: они приходят
//! через фасады ниже. Отложенная синхронизация хранит метку связанной цели
//! (местная метка [`JiuMaiAiState::linked_target`], машиной не требуется —
//! см. прежнего владельца).
//!
