...
//! `server/gameserver/appserver/ai/playerai.cpp`.
//!
...
//! - Трёхаргументный virtual `MoveTo` (RVA `0x10A480`, пролог сверен) для
//!   живого игрока очищает эмоцию, удаляет старейшие назначения
...
//!   void-контракт); задержка шага — общая формула (те же f32
//!   `1e6`/`1.414e6`, refs raw `0x109046`/`0x1090A7`, и фактор `0.001`,
//!   refs raw `0x109031`/`0x109092`).
...
//!   `0x2540E8`, VA `0x6540E8`; единственная ссылка raw `0x1094D4`), и
//!   `vigour = trunc(x*log10(exp+600) - y)` с cap;
...
//!   (`0x00485540`, байты `c3`+padding), поэтому idle игрока не ставит базовый
//!   `Stand` на 1000 мс. Канонический `CPlayer`
...
//!
//! UNKNOWN (не достраиваются догадкой): источник события
