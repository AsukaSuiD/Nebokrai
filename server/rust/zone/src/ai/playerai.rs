//! Состояние и тела расписаний игрового AI игрока `CPlayerAI` исторического
//! GameServer без hub-типов: назначения клиента (destination FIFO),
//! независимые очереди навыков игрока и боевого духа, выбранные/текущие
//! команды, object-цель проверки и хвост `Run` (авто-прирост опыта/бодрости
//! и регенерация энергии). Базовые FIFO, цель и back-stage список — в
//! `super::baseai::CBaseAI`; steaming passive-реакции — `super::reactions`.
//!
//! Точная пара `GameServer/gameserver.exe + GameServer/GameServer.pdb`
//! (EXE SHA-256 `4F5C98E0FDF6147D8AECF55F7937AAF6E2CF5E4F5A2C44491A6359228762C80E`,
//! PDB RSDS `5BEE6DD1-BF90-49B8-8BE9-EB25C4038D53` age 2, match; RVA истинные
//! `off pub + 0x1000`). Исходный владелец PDB:
//! `server/gameserver/appserver/ai/playerai.cpp`. Тела перенесены буквально из
//! старого hub-файла `appserver/ai/playerai.rs` волной Z-AI-player; до
//! переноса свидетельства были зафиксированы в его шапке, дополнительные
//! точечные сверки этой волны отмечены ниже отдельно.
//!
//! Машинная база (VERIFIED по этой паре):
//!
//! - `OnSchedule` (RVA `0x1098D0`; извлечение и допуск `0x50993E..0x50999D`)
//!   извлекает обычный запрос до Begin; End/OnLoseTarget не подменяют
//!   текущую команду головой pending FIFO. Повторное завершение
//!   отсутствующего исполнения не удаляет будущий запрос с тем же dispatch.
//!   При свободных очередях действий новый запрос заменяет текущую цель,
//!   даже если прежняя команда ещё сохранена. Riding-отказ вызывает
//!   OnLoseTarget до pop и не устанавливает цель отклоняемого запроса
//!   (`0x50998E..0x50999D`). Pop не меняет текущую цель сам по себе.
//!   `HasTarget` (RVA `0x0C7DD0`) не допускает Begin при нулевой координате
//!   или неположительном type/id. При null owner после разрешения выбирается
//!   virtual default игрока с прежней целью (`0x509A34..0x509A60`); WarSoul
//!   не выбирает default при null GetSkill (`0x509804`), очищает цель до
//!   проверки пустоты pending FIFO (`0x509780..0x509789`).
//! - `Attack` обеих перегрузок (RVA `0x109FF0`/`0x10A230`) до изменения FIFO
//!   требует существующий GetSkill и заменяет только ожидающую команду
//!   (`m_qTarget`), не текущее исполнение и не выбранный ID. Для WarSoul
//!   point-ветвь `0x10A334..0x10A3BF` сравнивает голову, удаляет старые
//!   запросы и добавляет новый без Reject; обычная очередь отдельно
//!   отправляет Reject для каждой замены (`0x10A43C`). У подключённых
//!   WarSoul `0x212..0x224` нет записи prepared (см. battlefairyskill),
//!   поэтому проверка повторного prepared Attack не даёт им дополнительный
//!   End.
//! - Снятия active-фазы: Attack снимается лишь после подтверждённого
//!   завершения concrete навыка и IsEnded/IsPrepared-проверки `OnFighting`
//!   (RVA `0x1092B0`); prepared-экземпляр переносится в back-stage список до
//!   ChangeSkill после пройденного фонового обхода. `OnFightingWithWarSoul`
//!   (RVA `0x109230`) проверяет IsEnded до вызова AI и не ставит ChangeSkill;
//!   очередь WarSoul Attack ставится после Begin и переживает собственный
//!   End навыка. `OnChangeSkill` игрока (RVA `0x108E40`) возвращает
//!   вычисленный после End default и снимает цель до завершения такта.
//!   `OnChangeSkillWithWarSoul` (RVA `0x109310`, исходный
//!   `playerai.cpp:629`) выполняет End(1) выбранного навыка и выбирает
//!   `0x224`; `OnLoseTargetWarSoul` повторяет тот же последний side effect
//!   после полного concrete End(1), включая оружейный эффект и cooldown.
//! - Трёхаргументный virtual `MoveTo` (RVA `0x10A480`, пролог сверен этой
//!   волной) для живого игрока очищает эмоцию, удаляет старейшие назначения
//!   до длины не более трёх и затем добавляет `(direction, is_run)`: очередь
//!   после вызова содержит не более четырёх элементов. Изъятие front после
//!   попытки MoveTo выполняет `OnSchedule` независимо от результата region
//!   cast и самого движения. Четырёхаргументный `MoveTo` игрока
//!   (`0x508F10`/`0x5090FB`) повторяет общий `CBaseAI::Slip` один/два/три
//!   раза по `is_run`/riding с исходным направлением команды и всегда ставит
//!   `ASA_MOVE` после начатой spatial-мутации (`CMoveShape::OnMove` —
//!   void-контракт); задержка шага — общая формула (сверено этой волной:
//!   те же f32 `1e6`/`1.414e6`, refs raw `0x109046`/`0x1090A7`, и фактор
//!   `0.001`, refs raw `0x109031`/`0x109092`).
//! - Хвост `CPlayerAI::Run`: авто-прирост опыта и бодрости живого игрока с
//!   ненулевой фракцией — `level³ * auto_exp_2 * (1 + faction_level*0.05) +
//!   auto_exp_1`, масштабированный константой f32 `0.00011574074` (raw
//!   `0x2540E8`, VA `0x6540E8`; единственная ссылка raw `0x1094D4` —
//!   сверено этой волной), и `vigour = trunc(x*log10(exp+600) - y)` с cap;
//!   беззнаковая проверка срока `last >= now - interval` намеренно не
//!   wrap-safe: первый живой tick энергии только заводит clock, полная
//!   энергия не двигает его дальше. Формула регенерации
//!   `trunc((level*0.1 - 1)*5 + faction_bonus + 10)`; faction bonus —
//!   `max(1, min(level*0.01, 1) * (faction_level*0.5))` для живого члена
//!   фракции.
//! - Базовый idle: слот +0x48 vtable `CPlayerAI` указывает на пустой RET
//!   (`0x00485540`, байты `c3`+padding, сверено этой волной), поэтому idle
//!   игрока не ставит базовый `Stand` на 1000 мс. Канонический `CPlayer`
//!   владеет очередями навыков; `CMoveShape::AI` передаёт первый элемент
//!   конкретному исполнителю и удаляет его только после завершения либо
//!   отказа. Координатор публикует настоящий `CPlayerAI` на время обоих
//!   callback смерти; отдельной очереди смерти или копии снимка удара нет.
//!
//! Швы переноса (не расхождения):
//!
//! - Игрок приходит узким фасадом `AutoIncPlayer` — двенадцать аксессорных
//!   операций хвоста `Run` прежнего `CPlayer` (`appserver/player.rs`);
//!   hub-реализация и переэкспорт — в делегате старого пакета, имена членов
//!   сохраняют исходные операции. Ничего иного от `CPlayer` перенесённый код
//!   не требует.
//! - Часы каждого события читаются отдельным вызовом делегата старого main
//!   loop (`&mut dyn FnMut() -> u32`, как в `skills/baseattackruntime.rs`);
//!   шов издателей/подписчиков исполнения и реестр игроков — у hub-владельца
//!   (`game.rs` take/restore), как у monster-dispatcher.
//! - `PlayerSkillDispatch`/`BattleFairySkillDispatch` и `SkillExecutionKernel`
//!   — уже Zone (`skills/dispatch.rs`, `skills/lifecycle.rs`);
//!   `BATTLE_FAIRY_BASE_MAGIC_SKILL_ID` — `skills/battlefairybasemagicphalanx`.
//! - `VecDeque` заменяет внутренности `std::queue<std::deque<...>>`, сохраняя
//!   FIFO и `push_back`; `Option` кодирует извлечённую текущую команду,
//!   нулевое начальное поле `selected_battle_fairy_skill_id` — установленную
//!   конструктором базовую атаку `0x224`.
//!
//! Честные UNKNOWN этой порции (не достраиваются догадкой): источник события
//! `ChangeSkillWithWarSoul` в данной паре не найден (из 82 прямых вызовов
//! `AddAIEvent` только `0x509877` передаёт WarSoul=1, с действием Attack;
//! все 13 постановок ChangeSkill используют WarSoul=0 — синтетический
//! ChangeSkill после End не добавляется); полный registered `CSkill::End`
//! (отказ Begin без concrete-данных, визуальные ресурсы); exact поведение
//! слота `+0x8C CPlayer` на пути MoveTo (`0x004856A0`, `xor al,al; ret 4`).
