...
//! неделимый ID-поток session/plug, typed-карты и порядок GC. По карте
//! realm-and-zone «общий ID-поток неделим» единый реестр держит
//! `next_session_id/next_plug_id`, typed-карты и порядок GC для команд, сделок,
//! лавок и операций экипировки вместе. MainLoop-стадии GC и отключение
//! container-listener-ов по owner остаются у прежнего владельца `CGame`;
//! coherent-impl шва around-runtime живёт у трейта в `replication/around`,
//! обёртка владения в старом пакете ликвидирована.
//!
//! Один принадлежащий `CGame` экземпляр заменяет две статические карты процесса;
//! `BTreeMap` — детерминированная замена: достигнутые функции не наблюдают
//! порядок обхода хеш-таблицы и выполняют только поиск по точному ключу.
//! `register_*` материализует хранилище реестра, а `goodsmessage 0x8FC25`
//! выполняет упорядоченный поиск разъёма по типу и идентификатору владельца.
//! Завершения `End/Exit` хранятся здесь; сборка завершённой сессии сохраняет
//! порядок сессии и разъёмов и идентификатор владельца для отключения
//! слушателей на MainLoop-стадии. Session start timestamps берутся из текущего
//! MainLoop sample; ненулевой lifetime команды завершается через terminal
//! report, чтобы `CGame` сохранил derived side effects.
...
use crate::items::camountlimitgoodsshadowcontainer::AmountShadowAdded;
use crate::items::ccontainer::PreviousContainer;
use crate::items::cequipmentcomposeshadowcontainer::{ComposeEquipmentCell, ComposeShadowAddBlock};
use crate::items::cequipmentdakongcontainer::{DaKongAddBlock, DaKongAddOutcome, DaKongCell};
use crate::items::cequipmentupgradeshadowcontainer::{UpgradeEquipmentCell, UpgradeShadowAddBlock};
...
use crate::sessions::cequipmentcompose::CEquipmentCompose;
use crate::sessions::cequipmentdakong::CEquipmentDaKong;
use crate::sessions::cequipmentupgrade::CEquipmentUpgrade;
use crate::sessions::cpersonalshopbuyer::CPersonalShopBuyer;
use crate::sessions::cpersonalshopseller::CPersonalShopSeller;
use crate::sessions::cplug::CPlug;
use crate::sessions::csession::CSession;
use crate::sessions::cteam::CTeam;
use crate::sessions::cteamate::{CTeamate, TeamMateAvailability};
use crate::trade::ctrader::{
    CTrader, TRADE_PLUG_TYPE, TRADE_SESSION_LIFETIME, TRADE_SESSION_MAXIMUM_PLUGS,
    TRADE_SESSION_MINIMUM_PLUGS,
};
use nebokrai_shared::values::CGuid;

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum EquipmentSessionPlugKind {
    Upgrade,
    DaKong,
    Compose,
}
...
pub enum EquipmentSessionShadowAddBlock {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EquipmentSessionShadowAdded {
    pub kind: EquipmentSessionPlugKind,
    pub plug_id: i32,
    pub position: u32,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EquipmentSessionShadowRemoved {
    pub kind: EquipmentSessionPlugKind,
    pub plug_id: i32,
    pub original: PreviousContainer,
    pub removed: ShadowRemovedReport,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PersonalShopShadowAddBlock {
    MissingSessionOrPlug,
    OwnerMismatch,
    InvalidContainerIndex,
    PositionUnavailable,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct PersonalShopShadowAdded {
    pub plug_id: i32,
    pub position: u32,
    pub shadow: AmountShadowAdded,
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct PersonalShopShadowRemoved {
    pub plug_id: i32,
    pub original: PreviousContainer,
    pub removed: ShadowRemovedReport,
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TerminalEquipmentSessionCollected {
    pub session_id: i32,
    pub plugs: Vec<TerminalEquipmentPlugCollected>,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct TerminalEquipmentPlugCollected {
    pub plug_id: i32,
    pub owner_id: i32,
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TeamSessionCreated {
    pub session_id: i32,
    pub team_id: u32,
    pub empty_snapshot: Vec<u8>,
    pub leader_snapshot: Vec<u8>,
    pub candidate_snapshot: Vec<u8>,
    pub teammate_ids: Vec<i32>,
}

#[must_use = "team insertion сохраняет new owner snapshot и resulting count"]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TeamMemberInserted {
    pub session_id: i32,
    pub team_id: u32,
    pub snapshot: Vec<u8>,
...
pub struct TeamMemberSnapshot {
    ...
    pub owner_id: i32,
    pub plug_ended: i32,
    pub owner_region_id: i32,
    pub owner_name: Vec<u8>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TeamSessionSnapshot {
    pub minimum_plugs: u32,
    pub maximum_plugs: u32,
    pub lifetime: u32,
    pub team_id: u32,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TeamSessionRestored {
    pub session_id: i32,
    pub team_id: u32,
    pub insertion_snapshots: Vec<Vec<u8>>,
...
pub struct TeamPlugAiReport {
    pub recovered: Vec<TeamMemberRecovered>,
    pub ended: Vec<TeamPlugEnded>,
    pub expired: Vec<TeamMemberRemoved>,
}

#[must_use = "ended team plug сохраняет owner и remaining members после terminal callback"]
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TeamPlugEnded {
    pub session_id: i32,
    pub team_id: u32,
    pub owner_type: i32,
    pub owner_id: i32,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TeamSessionDisbanded {
    pub session_id: i32,
    pub team_id: u32,
    pub leader_id: i32,
    pub player_ids: Vec<i32>,
...
impl CSessionFactory {
    ...
    pub fn insert_team_member_owned(
        &mut self,
        now_ms: u32,
        session_id: i32,
        owner_type: i32,
    ...
    pub fn update_team_member_region(
        &mut self,
        session_id: i32,
        owner_type: i32,
        owner_id: i32,
    ...
    pub fn query_team(&self, session_id: i32) -> Option<&CTeam> {
        self.teams.get(&session_id)
    }

    pub fn team_member_count(&self, session_id: i32) -> Option<usize> {
    ...
    pub fn set_team_leader(&mut self, session_id: i32, player_id: i32) -> Option<i32> {
    ...
    pub fn set_team_allocation_scheme(
        &mut self,
        session_id: i32,
        actor_id: i32,
    ...
    pub fn team_player_ids(&self, session_id: i32) -> Option<Vec<i32>> {
        let session = self.sessions.get(&session_id)?;
        self.teams.get(&session_id)?;
        session
            .plug_ids_storage()
    ...
    pub fn expire_team_sessions(&mut self, now_ms: u32) -> Vec<TeamSessionDisbanded> {
    ...
    pub fn team_member_descriptors(&self, session_id: i32) -> Option<Vec<(i32, i32, i32)>> {
        let session = self.sessions.get(&session_id)?;
        self.teams.get(&session_id)?;
        session
            .plug_ids_storage()
    ...
    pub fn disband_team(&mut self, session_id: i32) -> Option<TeamSessionDisbanded> {
        let team = self.teams.get(&session_id)?;
        let team_id = team.team_id();
        let leader_id = team.leader_id();
        let player_ids = self
    ...
    pub fn create_player_trade_session(
        &mut self,
        now_ms: u32,
        inviter_id: i32,
        answerer_id: i32,
    ...
    pub fn query_trader(&self, plug_id: i32) -> Option<&CTrader> {
        self.trader_plugs.get(&plug_id)
    }

    pub fn query_trader_mut(&mut self, plug_id: i32) -> Option<&mut CTrader> {
        self.trader_plugs.get_mut(&plug_id)
    }

    pub fn trader_plug_by_owner(&self, session_id: i32, owner_id: i32) -> Option<i32> {
    ...
    pub fn contrary_trader_id(&self, session_id: i32, plug_id: i32) -> Option<i32> {
        self.sessions
            .get(&session_id)?
            .plug_ids_storage()
            .iter()
    ...
    pub fn trade_session_plug_ids(&self, session_id: i32) -> Option<Vec<i32>> {
        let session = self.sessions.get(&session_id)?;
    ...
    pub fn trade_session_available(
        &self,
        session_id: i32,
    ...
    pub fn abort_session(&mut self, session_id: i32) {
    ...
    pub fn create_personal_shop_seller_session(
        &mut self,
        now_ms: u32,
        player_id: i32,
    ) -> Option<(i32, i32)> {
    ...
    pub fn personal_shop_seller(&self, plug_id: i32) -> Option<&CPersonalShopSeller> {
        self.personal_shop_seller_plugs.get(&plug_id)
    }

    pub fn personal_shop_seller_mut(&mut self, plug_id: i32) -> Option<&mut CPersonalShopSeller> {
        self.personal_shop_seller_plugs.get_mut(&plug_id)
    }

    pub fn personal_shop_buyer(&self, plug_id: i32) -> Option<&CPersonalShopBuyer> {
        self.personal_shop_buyer_plugs.get(&plug_id)
    }

    pub fn personal_shop_seller_plug_id(&self, session_id: i32) -> Option<i32> {
        self.sessions
            .get(&session_id)?
            .plug_ids_storage()
            .iter()
    ...
    pub fn personal_shop_session_available(&self, session_id: i32) -> bool {
        self.sessions
            .get(&session_id)
            .is_some_and(CSession::is_available_prefix)
    }

    pub fn insert_personal_shop_buyer(&mut self, session_id: i32, owner_id: i32) -> Option<i32> {
    ...
    pub fn remove_personal_shop_buyer(
        &mut self,
        session_id: i32,
        plug_id: i32,
    ...
    pub fn personal_shop_participants(
        &self,
        session_id: i32,
    ...
    pub fn is_personal_shop_seller_container(
        &self,
        session_id: i32,
        extend_id: i32,
        player_id: i32,
    ...
    pub fn record_personal_shop_shadow(
        &mut self,
        session_id: i32,
        extend_id: i32,
        player_id: i32,
    ...
    pub fn personal_shop_shadow_original(
        &self,
        session_id: i32,
        extend_id: i32,
        player_id: i32,
    ...
    pub fn personal_shop_shadow_position(
        &self,
        session_id: i32,
        extend_id: i32,
        player_id: i32,
    ...
    pub fn remove_personal_shop_shadow(
        &mut self,
        session_id: i32,
        extend_id: i32,
        player_id: i32,
    ...
    pub fn record_equipment_session_shadow(
        &mut self,
        session_id: i32,
        extend_id: i32,
        player_id: i32,
        ...
        let (position, shadow) = match kind {
            EquipmentSessionPlugKind::Upgrade => {
                ...
                let cell = if requested_position == u32::MAX {
                    ...
                    UpgradeEquipmentCell::from_position(requested_position).ok_or(
                        EquipmentSessionShadowAddBlock::Upgrade(
                            UpgradeShadowAddBlock::InvalidPosition {
            ...
            EquipmentSessionPlugKind::DaKong => {
                ...
                let cell = if requested_position == u32::MAX {
                    ...
                    DaKongCell::from_position(requested_position).ok_or(
                        EquipmentSessionShadowAddBlock::DaKong(DaKongAddBlock::InvalidPosition {
                            position: requested_position,
                        }),
                    )?
                };
            ...
            EquipmentSessionPlugKind::Compose => {
                ...
                let cell = if requested_position == u32::MAX {
                    ...
                    ComposeEquipmentCell::from_position(requested_position).ok_or(
                        EquipmentSessionShadowAddBlock::Compose(
                            ComposeShadowAddBlock::InvalidPosition {
                                position: requested_position,
                            },
                        ),
                    )?
    ...
    pub fn equipment_session_shadow_original(
        &self,
        session_id: i32,
        extend_id: i32,
        player_id: i32,
    ...
    pub fn equipment_session_shadow_position(
        &self,
        session_id: i32,
        extend_id: i32,
        player_id: i32,
    ...
    pub fn remove_equipment_session_shadow(
        &mut self,
        session_id: i32,
        extend_id: i32,
        player_id: i32,
    ...
    pub fn create_equipment_session(
        &mut self,
        now_ms: u32,
    ...
    pub fn register_session(&mut self, session_id: i32, session: CSession) -> Option<CSession> {
        self.sessions.insert(session_id, session)
    }

    pub fn register_plug(&mut self, plug_id: i32, mut plug: CPlug) -> Option<CPlug> {
        plug.set_id(plug_id);
    ...
    pub fn query_session_plug_by_owner(
        &self,
        session_id: i32,
        owner_type: i32,
        owner_id: i32,
    ...
    pub fn session_player_ids(&self, session_id: i32) -> Option<Vec<i32>> {
        Some(
            self.sessions
                .get(&session_id)?
                .plug_ids_storage()
    ...
    pub fn query_session_id_by_owner(&self, owner_type: i32, owner_id: i32) -> Option<i32> {
        self.plugs
            .values()
    ...
    pub fn end_session(&mut self, session_id: i32) {
        let Some(session) = self.sessions.get_mut(&session_id) else {
    ...
    pub fn exit_plug(&mut self, session_id: i32, plug_id: i32) {
    ...
    pub fn register_equipment_compose_plug(
        &mut self,
        plug_id: i32,
    ...
    pub fn query_equipment_compose_plug(&self, plug_id: i32) -> Option<&CEquipmentCompose> {
        self.equipment_compose_plugs.get(&plug_id)
    }

    pub fn query_equipment_compose_plug_mut(
        &mut self,
        plug_id: i32,
    ...
    pub fn take_equipment_compose_plug(&mut self, plug_id: i32) -> Option<CEquipmentCompose> {
        self.equipment_compose_plugs.remove(&plug_id)
    }

    pub fn register_equipment_da_kong_plug(
        &mut self,
        plug_id: i32,
    ...
    pub fn query_equipment_da_kong_plug(&self, plug_id: i32) -> Option<&CEquipmentDaKong> {
        self.equipment_da_kong_plugs.get(&plug_id)
    }

    pub fn query_equipment_da_kong_plug_mut(
        &mut self,
        plug_id: i32,
    ...
    pub fn take_equipment_da_kong_plug(&mut self, plug_id: i32) -> Option<CEquipmentDaKong> {
        self.equipment_da_kong_plugs.remove(&plug_id)
    }

    pub fn register_equipment_upgrade_plug(
        &mut self,
        plug_id: i32,
    ...
    pub fn query_equipment_upgrade_plug(&self, plug_id: i32) -> Option<&CEquipmentUpgrade> {
        self.equipment_upgrade_plugs.get(&plug_id)
    }

    pub fn take_equipment_upgrade_plug(&mut self, plug_id: i32) -> Option<CEquipmentUpgrade> {
    ...
    pub fn garbage_collect_session(&mut self, session_id: i32) -> Vec<i32> {
    ...
    pub fn garbage_collect_terminal_equipment_sessions(
        &mut self,
