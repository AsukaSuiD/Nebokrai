//! Жизненный цикл ресурсов World: установка, чтение загрузчиками и освобождение.
//! Привязки: World CGame::LoadServerResource/Release, public/clientresource.cpp/.h;
//! поиск skill-cache — CSkillFactory. Каталог и выбор источника находятся в Shared.
//! Правила: docs/architecture/resources-and-configuration.md.
...
use nebokrai_shared::resources::{
    CRFile, ResourceCatalog, ResourceLoadError, ResourceOpenError, ResourcePackageLoad, ResourceSource,
    open_resource, resolve_resource_path,
};
...
impl DefaultClientResourceOwner {
    ...
    pub(crate) fn root_directory(&self) -> Option<&Path> {
        self.installed
            .as_ref()
    ...
    pub(crate) fn open(&self, path: &[u8]) -> Option<CRFile> {
        let context = self.installed.as_ref().map(|installed| {
            ResourceSource::new(&installed.root, installed.resource.as_ref())
        });
