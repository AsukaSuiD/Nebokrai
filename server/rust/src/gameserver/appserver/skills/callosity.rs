...
//! и callosity2.cpp. Тела Check/AI перенесены буквально в Zone
//! `skills/callosity.rs` (порция №6c «self/zone-касты»; основание и машинные
//! статусы см. там). Здесь — тонкие делегации с прежними сигнатурами:
//! зарегистрированный вход идёт общим playercast с захватом исходного U,
//! швы Zone реализованы над `CGame` в `skills/selfcast.rs`; потребители
//! не меняются.
...
fn check_cast<Runtime: GameMainLoopRuntime>(
    ...
    let _ = _runtime;
    callosity::check_cast(game, instance, original_user, game_tick_milliseconds)
}
...
fn run_ai<Runtime: GameMainLoopRuntime>(
    ...
    let _ = _runtime;
    selfcast_outcome(callosity::run_ai(game, instance, game_tick_milliseconds))
}
...
pub(crate) fn execute_player_callosity<Runtime: GameMainLoopRuntime>(
    ...
    if !callosity::is_callosity_dispatch(dispatch) {
