...
//! ожидания ещё требуют реконструкции.
...
use crate::gameserver::appserver::script::buffskillfunc::{
    BuffSkillScriptFunctionOutcome, run_buff_skill_script_function,
};
...
pub(crate) use nebokrai_zone::scripts::functions::*;
...
pub(crate) fn run_gods_battle_script_function<Runtime: ScriptFunctionRuntime>(
    ...
    match kind {
        ...
        GodsBattleScriptKind::ChangePlayerSzl => {
            let Some(requested) =
                evaluated_arguments[0].filter(|value| *value != SCRIPT_INT_PARAMETER_ERROR)
            else {
        ...
        GodsBattleScriptKind::SetFactionXyd => {
            let faction = evaluated_arguments[0];
            let xyd = evaluated_arguments[1]
                .filter(|value| *value != SCRIPT_INT_PARAMETER_ERROR && *value >= 0);
            let sent = faction.zip(xyd).is_some_and(|(faction, xyd)| {
                game.request_gods_battle_faction_xyd(faction, xyd as u32)
            });
            handled(
                0,
                GodsBattleScriptDisposition::FactionXydRequested {
                    faction,
                    xyd,
                    sent,
                },
        ...
        GodsBattleScriptKind::EnterContend => {
            let Some(seconds) = evaluated_arguments[0]
                .filter(|value| *value != SCRIPT_INT_PARAMETER_ERROR && *value >= 0)
            else {
                return handled(
                    0,
                    GodsBattleScriptDisposition::ContendArgumentRejected {
                        seconds: evaluated_arguments[0],
                    },
                );
            };
            let (Some(player_id), Some(npc_id)) = (script_player_id, script_npc_id) else {
    ...
    match kind {
        NationWarScriptKind::SendPlayerId => {
            let Some(target_player_id) = argument(0) else {
                tracing::debug!(function_id, ?kind, argument = 0, "сценарной функции войны наций не передан аргумент");
                return handled(0);
            };
            let Some(script_player_id) = script_player_id else {
                tracing::debug!(function_id, ?kind, "для сценарной функции войны наций не найден игрок");
                return handled(0);
            };
            let started =
            ...
            tracing::debug!(function_id, ?kind, script_player_id, target_player_id, started, "обработан запуск отсчёта войны наций");
            handled(0)
        }
        NationWarScriptKind::CarriageBackTown => {
            ...
            let Some((region_id, country)) = script_player_id.and_then(|player_id| {
                ...
                tracing::debug!(function_id, ?kind, ?script_player_id, "для возврата обоза войны наций не найден регион игрока");
                return handled(0);
            };
            let completed = game.script_nation_carriage_back_town(region_id, country).is_some();
            if completed { nation_war_script_debug(game, b"GS1054"); }
            tracing::debug!(function_id, ?kind, region_id, country, completed, "обработан возврат обоза войны наций");
            handled(0)
        }
        NationWarScriptKind::GetFlagStatus => {
            ...
            let value = region_id
                ...
                .unwrap_or(0);
            tracing::trace!(function_id, ?kind, ?region_id, ?script_player_id, value, "прочитано сценарное значение войны наций");
            handled(value)
        }
        NationWarScriptKind::GetTime => {
            nation_war_script_debug(game, b"GS1055");
            let Some(player_id) = script_player_id else {
                tracing::debug!(function_id, ?kind, "для чтения времени войны наций не найден игрок");
                return handled(0);
            };
            let debug_time = game
                .four_nation_war_sys()
            ...
            let Some(player_name) = game
                ...
                tracing::debug!(function_id, ?kind, player_id, "для чтения времени войны наций не найден игрок");
                return handled(0);
            };
            ...
            let value = game
                .four_nation_war_sys()
                .player_war_time_seconds(player_id) as i32;
            tracing::trace!(function_id, ?kind, player_id, value, "прочитано время игрока на войне наций");
            handled(value)
        }
        NationWarScriptKind::EnterContend => {
            ...
            let Some(player_id) = script_player_id else {
                tracing::debug!(function_id, ?kind, "для входа в захват войны наций не найден игрок");
                return handled(0);
            };
            if game.find_player(player_id).is_none() {
                tracing::debug!(function_id, ?kind, player_id, "для входа в захват войны наций не найден игрок");
                return handled(0);
            }
            ...
            tracing::debug!(function_id, ?kind, player_id, duration_ms, entered, "обработан вход в захват войны наций");
            handled(0)
        }
        NationWarScriptKind::CountrySignUp => {
            ...
            if operation == 0 {
                tracing::trace!(function_id, ?kind, "регистрация страны на войну наций пропущена");
                return handled(0);
            }
            let Some(country) = script_player_id
                ...
                tracing::debug!(function_id, ?kind, ?script_player_id, "для регистрации страны не найден игрок");
                return handled(0);
            };
            ...
            tracing::debug!(function_id, ?kind, country, ?delivery, legacy_return, "отправлена регистрация страны на войну наций");
        ...
        NationWarScriptKind::ClearPlayerTime => {
            ...
            tracing::debug!(function_id, ?kind, player_id, ?previous_ms, "очищено время игрока на войне наций");
            handled(0)
        }
        NationWarScriptKind::GetNationStatus => {
            ...
            tracing::trace!(function_id, ?kind, ?region_id, ?script_player_id, value, "прочитано состояние страны на войне наций");
            handled(value)
        }
        NationWarScriptKind::SetPlayerTime => {
            ...
            if time_ms == SCRIPT_INT_PARAMETER_ERROR {
                tracing::debug!(function_id, ?kind, argument = 1, "сценарной функции войны наций не передан аргумент");
                return handled(0);
            }
            ...
            tracing::debug!(function_id, ?kind, player_id, time_ms, ?previous_ms, "установлено время игрока на войне наций");
            handled(0)
        }
        NationWarScriptKind::IsPlayerWeak => {
            let value = script_player_id
                ...
                .unwrap_or(0);
            tracing::trace!(function_id, ?kind, ?script_player_id, value, "прочитана слабость игрока на войне наций");
            handled(value)
        }
        ...
        NationWarScriptKind::ClearMorale => {
            ...
            tracing::debug!(function_id, ?kind, previous, "очищен боевой дух войны наций");
            handled(0)
        }
    }
}
...
fn run_core_player_script_function<Runtime: ScriptFunctionRuntime>(
    ...
    match function_id {
        ...
        | SCRIPT_FUNCTION_GET_EX_STATE_NEW => {
            ...
            let legacy_return = match function_id {
                SCRIPT_FUNCTION_ADD_EX_STATE | SCRIPT_FUNCTION_ADD_EX_STATE_NEW => {
                    game.add_script_extended_state(
                        player_id, state_id, kind, &mut || runtime.now_milliseconds(),
                    )
                }
                SCRIPT_FUNCTION_DELETE_EX_STATE | SCRIPT_FUNCTION_DELETE_EX_STATE_NEW => {
                    game.delete_script_extended_state(player_id, state_id, kind)
                }
        ...
        SCRIPT_FUNCTION_MODIFY_AUCTION_SPACE => {
            if let Some(player_id) = script_player_id {
                game.modify_script_player_auction_space(
        ...
        SCRIPT_FUNCTION_DELETE_EX_STATE_BY_TYPE => {
            ...
            Some(ScriptFunctionDispatchOutcome::Handled {
                legacy_return: game
                    .delete_script_extended_state_by_type(player_id, state_type)
                    as i32,
            })
        }
