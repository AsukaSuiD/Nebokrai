...
//! и коэффициент, а эта формула увеличивает максимум MP с FISTP-усечением,
//! wrapping-сложением и верхней границей `i32::MAX`. Недостигнутые перегрузки
//! сохранены ниже.
//! Default ctor 0x005D54F0 задаёт keep=0/coefficient=1; промежуточный объект
...
pub(crate) fn apply(coefficient: u32, properties: &mut PlayerCombatProperties) {
    let delta = truncate_original(f64::from(coefficient)
        * f64::from(0.01_f32)
        * f64::from(properties.maximum_mp)) as u32;
