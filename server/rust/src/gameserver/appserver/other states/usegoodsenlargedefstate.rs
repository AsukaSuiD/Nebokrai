...
//! хранит время и коэффициент в ScriptMoveState; формула защиты выполняет исходное
//! FISTP-усечение, wrapping-сложение и сужение к младшим 16 битам.
//! Default ctor 0x005D5030 задаёт keep=0/coefficient=1; промежуточный объект
...
pub(crate) fn apply(coefficient: u32, properties: &mut PlayerCombatProperties) {
    let delta = truncate_original(f64::from(coefficient)
        * f64::from(0.01_f32)
        * f64::from(properties.defense)) as u32;
