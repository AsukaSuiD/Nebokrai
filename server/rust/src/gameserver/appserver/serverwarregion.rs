...
//!
//! Данные contender-ов и payload-логов, typed-границы арифметики, wire-stream
//! readers с `RegionDecodeInputBlock`, error-семейство `WarRegionDecodeError`
//! и чистые context-контракты перенесены в Zone `regions/serverwarregion`
//! (волна Z-M-Xd, семья war-регионов war+godsbattle+village); здесь
//! hub-обёртка `CServerWarRegion` поверх живого `CServerRegion` с прежними
//! сигнатурами, hub-поля contender/symbol колонок (decoder-ы city/godsbattle
//! шимов, timeout aggregate city и `CGame` обращаются к ним напрямую),
//! decode/contend context-контракты над owner-ом хранилищ, re-export
//! семейства для старого пакета и evidence-блок.
//!
...
pub(crate) use nebokrai_zone::regions::serverwarregion::*;
