...
//! `0x54A9E0`. `INFERRED`: это account — тот же `std::string` глобального
//! game-owner-а по `+0x80` клиент пишет первым полем `0x2FD09`, а текущий
//! Login owner читает первое поле этого сообщения как account. Прямое чтение
//! matching Login EXE для этой семантической связи ещё не восстановлено, поэтому
//! статус не повышается до `VERIFIED`. Game не использует строку для выбора
//! игрока: принимается строка с NUL и ровно четырьмя байтами аргумента после неё.
