...
//! - ATTR-семья `UpdateProperty` (`0x004593E0`): `MountAllEquip`/
//!   `MountCiQingEquip` пересчёт со сбором двух снимков и `0xC010E`-разницы,
//!   battle-fairy equipment prelude/pair, `apply_*_state` формулы состояний
//!   (undead/extended/change-body/ride), commit `tagProperty` wire и
//!   equipment flash, client allocation `0x8FA01`, серверный домен
//!   `GetCurrentTypeValue` — `zone::players::properties`; hub бережёт
//!   делегаты прежних сигнатур.
//! - Codec- и ATTR-типы, wire-константы — re-export совместимости ниже для
//!   hub-а и его callers; делегаты передают заёмные проекции полей.
...
//! `UpdateProperty` +0x24 состояний — общий проход CMoveShape (формулы —
//! `zone::players::properties`); SelfTarget: virtual +0x78 в `0x00488E20`,
...
use super::goods::cgoodsbaseproperties::{
    EQUIP_PLACE_HEADGEAR, GAP_BF_ABRAVE_ADDON, GAP_BF_AGILITY_ADDON, GAP_BF_ATTACK,
    GAP_BF_ATTACK_ADDON, GAP_BF_BATTLE_FAIRY, GAP_BF_BLAST, GAP_BF_BLAST_ADDON,
    GAP_BF_CUT_HURT_ADDON, GAP_BF_CUT_HURT_SCALE, GAP_BF_HP, GAP_BF_LEVEL, GAP_BF_LIFE_ADDON,
    GAP_BF_MP, GAP_BF_MP_ADDON, GAP_BF_SPRITE_ADDON, GAP_BF_SPRITUALISE_ADDON, GAP_BF_STRENGH_ADDON,
    GAP_CIQING_PROPERTY1, GAP_CIQING_PROPERTY2, GAP_EXCEPTION_STATE, GAP_GOODS_BIND,
    GAP_GOODS_LIFE_TYPE, GAP_GOODS_MAXIMUM_DURABILITY, GAP_GOODS_PACKAGE_EXTENTION, GAP_MOUNT_LEVEL,
    GAP_MOUNT_TYPE, GAP_PARTICULAR_ATTRIBUTE, GAP_REQUIRE_GENDER, GAP_REQUIRE_OCCUPATION,
...
use nebokrai_zone::players::{clientsnapshot, gamesave, properties};
...
use nebokrai_zone::players::properties::{PlayerCombatWireParts, PlayerEquipmentPropertyParts};
...
// Типы ATTR-семьи UpdateProperty — Zone `players::properties` (P-ATTR
// перенос); re-export совместимости для делегатов hub-а и его callers.
pub(crate) use nebokrai_zone::players::properties::{
    PlayerAllPropertiesDiagnosticSnapshot, PlayerPropertyRecompute, PlayerStatAllocationMutation,
...
impl CPlayer {
    ...
    /// Сбор заёмной ATTR-проекции hub-полей для Zone `players::properties`;
    /// живёт только на время одного property-вызова.
    fn equipment_property_parts(&mut self) -> PlayerEquipmentPropertyParts<'_> {
        PlayerEquipmentPropertyParts {
            equipment: &mut self.equipment,
            ci_qing: &mut self.ci_qing,
            base_properties: &mut self.base_properties,
            base_property_wire: &self.base_property_wire,
            battle_fairy_summoned: self.battle_fairy_summoned,
            ci_qing_add_values: &mut self.ci_qing_add_values,
            ci_qing_tao_zhuang_add_values: &self.ci_qing_tao_zhuang_add_values,
            equipment_changed: &mut self.equipment_changed,
        }
    }

    /// Делегат в Zone `players::properties`: полный исходный
    /// `CPlayer::UpdateProperty` до `OnChangeProperties` — BF prelude,
    /// CiQing base, два `MountAllEquip` снимка, BF pair и разница.
    pub(crate) fn recompute_update_property(
        ...
        properties::recompute_update_property(
            &mut self.equipment_property_parts(),
            coefficients,
            base_combat_scales,
            critical_rate,
            factory,
    ...
    /// Делегат в Zone `players::properties`: формула
    /// `CNotDisappearAfterDead::OnUpdateProperties` (машинные детали там же).
    pub(crate) fn apply_undead_state_properties(
        ...
        properties::apply_undead_state_properties(
            properties,
            coefficients,
    ...
    pub(crate) fn apply_extended_state_properties(
        ...
        properties::apply_extended_state_properties(properties, state)
    }
    ...
    pub(crate) fn apply_active_change_body_state_properties(
        ...
        properties::apply_active_change_body_state_properties(properties, state)
    }

    /// Делегат в Zone `players::properties`: `CRideState::OnUpdateProperties`
    /// выбирает первый packet goods по имени именно этого экземпляра.
    pub(crate) fn apply_ride_state_properties(
        ...
        properties::apply_ride_state_properties(
            properties,
            state,
            coefficients,
            goods_factory,
    ...
    /// Делегат в Zone `players::properties`: exact `GetPlayerAllProperties`
    /// diagnostic projection concrete `tagBaseProperty`/`tagProperty` slots.
    pub(crate) fn all_properties_diagnostic_snapshot(
        ...
        properties::player_all_properties_diagnostic_snapshot(
            self.shape().base_object().get_name(),
            self.occupation(),
            self.level(),
            self.experience(),
    ...
    /// Делегат в Zone `players::properties`: результат виртуального
    /// `UpdateProperty` плюс синхронизация `tagProperty` и equipment flash.
    pub(crate) fn apply_recomputed_combat_properties(
        &mut self,
        properties: PlayerCombatProperties,
        goods_factory: &CGoodsFactory,
    ) {
        properties::apply_recomputed_combat_properties(
            &mut PlayerCombatWireParts {
                combat_properties: &mut self.combat_properties,
                combat_property_wire: &mut self.combat_property_wire,
                equipment: &self.equipment,
                flash_previous: &mut self.flash_previous,
                flash_current: &mut self.flash_current,
                flash_changed: &mut self.flash_changed,
            },
            properties,
            goods_factory,
        )
    ...
    fn sync_combat_property_wire(&mut self) {
        properties::sync_combat_property_wire(
            &self.combat_properties,
            &mut self.combat_property_wire,
        )
    }

    /// Делегат в Zone `players::properties`: `DoneFlash` забирает один
    /// pending snapshot и одновременно продвигает previous-table.
    pub(crate) fn take_flash_update(&mut self) -> Option<[(u32, u32); 17]> {
        properties::take_flash_update(
            &mut self.flash_previous,
            &mut self.flash_current,
            &mut self.flash_changed,
        )
    }

    pub(crate) const fn stat_allocation_state(&self) -> PlayerStatAllocationState {
        properties::player_stat_allocation_state(&self.base_properties)
    }

    /// Делегат в Zone `players::properties`: восстанавливает owned поля
    /// `m_BaseProperty`, участвующие в client allocation `0x8FA01`.
    pub(crate) const fn restore_stat_allocation_state(&mut self, state: PlayerStatAllocationState) {
        properties::restore_player_stat_allocation_state(&mut self.base_properties, state)
    }

    /// Делегат в Zone `players::properties`: exact mutation-tail `0x8FA01`.
    pub(crate) fn allocate_stat_point(
        ...
        let player_id = self.player_id();
        properties::allocate_stat_point(
            &mut self.base_properties,
            player_id,
            selector,
            constitution_hp,
    ...
    /// Делегат в Zone `players::properties`: объединение CiQing add-values с
    /// TaoZhuang add-values (контракт кадра `0xC010E`).
    pub(crate) fn ci_qing_property_result(&self) -> BTreeMap<u32, u32> {
        properties::ci_qing_property_result(
            &self.ci_qing_add_values,
            &self.ci_qing_tao_zhuang_add_values,
        )
    }

    /// Делегат в Zone `players::properties`: насыщенная разница двух снимков
    /// `GetCurrentTypeValue` (`UpdateCiQingProperty`).
    pub(crate) fn update_ci_qing_property_difference(
        previous: &BTreeMap<u32, u32>,
        current: &BTreeMap<u32, u32>,
    ) -> Option<BTreeMap<u32, u32>> {
        properties::update_ci_qing_property_difference(previous, current)
    }
    ...
    pub(crate) fn combat_type_values(&self) -> BTreeMap<u32, u32> {
        properties::combat_type_values_from(self.combat_properties)
    }

    /// Делегат в Zone `players::properties`: серверный домен
    /// `GetCurrentTypeValue` (ровно 15 ключей `2..0x10`, кадр `0xC010E`).
    pub(crate) fn combat_type_values_from(
        properties: PlayerCombatProperties,
    ) -> BTreeMap<u32, u32> {
        properties::combat_type_values_from(properties)
    }
...
impl BattleFairyGearHost for BattleFairyGearPlayerAdapter<'_> {
    ...
    fn clamp_headgear_current(&mut self, current_property: i32, maximum_property: i32) {
        if let Some(goods) = self.player.equipment.get_goods_mut(10) {
            properties::clamp_battle_fairy_current(
                goods,
                self.factory,
                current_property,
                maximum_property,
