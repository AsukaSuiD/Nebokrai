...
use crate::nets::mysocket::legacy_bind_endpoint;

/// Размер одного старого IOCP receive-buffer для принятого соединения.
pub(crate) const SERVER_RECEIVE_CHUNK: usize = 0x2000;
/// Размер одного старого receive-вызова исходящего `CClient`.
pub(crate) const CLIENT_RECEIVE_CHUNK: usize = 0x2800;

/// Создаёт IPv4 TCP socket и выполняет совместимый bind без начала listen.
pub(crate) fn bind_tcp_ipv4(address: Option<Ipv4Addr>, port: u32) -> io::Result<TcpSocket> {
...
pub(crate) fn listen_tcp_ipv4(socket: TcpSocket, backlog: i32) -> io::Result<TcpListener> {
    socket.listen(backlog as u32)
}

/// Подключает ранее созданный TCP socket к уже разрешённому IPv4 endpoint.
pub(crate) async fn connect_tcp_ipv4(
    socket: TcpSocket,
    remote: SocketAddrV4,
) -> io::Result<TcpStream> {
...
pub(crate) fn try_write_tcp(stream: &TcpStream, buffer: &[u8]) -> io::Result<usize> {
...
pub(crate) async fn write_once_tcp(stream: &TcpStream, buffer: &[u8]) -> io::Result<usize> {
    loop {
...
pub(crate) async fn read_server_tcp_chunk(stream: &TcpStream) -> io::Result<Vec<u8>> {
...
pub(crate) async fn read_client_tcp_chunk(stream: &TcpStream) -> io::Result<Vec<u8>> {
...
pub(crate) fn shutdown_tcp(stream: &TcpStream) -> io::Result<()> {
