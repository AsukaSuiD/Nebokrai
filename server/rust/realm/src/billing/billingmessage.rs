...
pub enum BillingMessageOutcome {
...
pub struct BillingMessageHandler<'a> {
    queues: &'a CBillingPlayerManager,
}

impl<'a> BillingMessageHandler<'a> {
    pub const fn new(queues: &'a CBillingPlayerManager) -> Self {
        Self { queues }
    }

    pub fn on_billing_message(&self, message: &mut CMessage) -> BillingMessageOutcome {
