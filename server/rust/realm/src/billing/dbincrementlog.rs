...
#[derive(Debug)]
pub enum DbIncrementLogLoadFailure {
    MissingConnection,
...
#[derive(Debug)]
pub struct DbIncrementLogLoad {
    pub rows: Vec<DbIncrementLogRow>,
    pub completion: Result<(), DbIncrementLogLoadFailure>,
}
...
fn parse_row(row: &Row, row_index: usize) -> Result<DbIncrementLogRow, DbIncrementLogLoadFailure> {
...
fn required_i32(
    ...
    if let Ok(value) = row.try_get::<i32, _>(column) {
        return value.ok_or(DbIncrementLogLoadFailure::MissingRequiredValue { row_index, column });
    }
    ...
    match row.try_get::<i64, _>(column) {
        Ok(Some(value)) => i32::try_from(value)
