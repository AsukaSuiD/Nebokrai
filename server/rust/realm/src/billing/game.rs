...
use super::billingmessage::{BillingMessageHandler, BillingMessageOutcome};
use super::billingplayermanager::{
    BillingPlayerManagerNotice, BillingPlayerManagerRuntime, CBillingPlayerManager,
    CreateBillingPlayerWorkerError,
};
use super::playerfillmgr::{
    CPlayerFillMgr, PlayerFillNotice, PlayerFillRuntime, StartPlayerFillError,
};
use super::rsplayeraccount::{BillingDatabaseSettings, BillingDatabaseSettingsParts};
use super::rsplayerfillmgr::{PlayerFillDatabaseSettings, PlayerFillDatabaseSettingsParts};
use super::servermessage::{ServerMessageHandler, ServerMessageOutcome};
use crate::app::billing_message::{BillingMessageHandlers, CMessage};
use crate::app::billing_server::CServerForGS;
use crate::app::billing_server_client::BillingReceiveError;
...
pub struct BillingSetupLoadReport {
    pub parsed_pairs: usize,
    pub stopped_at_pair: Option<usize>,
}

#[derive(Debug)]
pub struct BillingSetupOpenError {
    pub path: PathBuf,
    pub source: io::Error,
}

#[derive(Debug)]
pub enum BillingServerInitializationError {
    MissingSetupField(&'static str),
    Host(ServerHostError),
}

#[derive(Debug)]
pub struct BillingServerInitialization {
    pub allowed_clients_error: Option<io::Error>,
}

#[derive(Debug)]
pub enum BillingPlayerManagerInitializationError {
    MissingSetupField(&'static str),
    MissingGameServer,
...
#[derive(Debug)]
pub enum BillingPlayerFillInitializationError {
    MissingSetupField(&'static str),
    MissingGameServer,
...
#[derive(Debug)]
pub enum BillingGameMessageOutcome {
    Billing(BillingMessageOutcome),
    Server(ServerMessageOutcome),
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum BillingGameEvent {
    RefreshInfo,
    PingStarted,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum BillingMainLoopError {
    MissingRefreshInfoTime,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct BillingRuntimePaths {
    pub setup: PathBuf,
    pub allowed_game_servers: PathBuf,
}
...
#[derive(Debug)]
pub enum BillingInitializationNotice {
    AllowedGameServersUnavailable(io::Error),
}

#[derive(Debug)]
pub enum BillingInitializationError {
...
#[derive(Debug)]
pub struct BillingInitializationReport {
    pub setup: BillingSetupLoadReport,
    pub notices: Vec<BillingInitializationNotice>,
}

#[derive(Debug)]
pub enum BillingNetworkRuntimeError {
    MissingGameServer,
    Task(JoinError),
}

#[derive(Debug, Default)]
pub struct BillingNetworkTurn {
    ...
    pub snapshot_errors: Vec<ServerSnapshotError<BillingReceiveError>>,
}

#[derive(Debug, Default)]
pub struct BillingRuntimeStep {
    pub network: BillingNetworkTurn,
    pub keep_running: bool,
    pub message_outcomes: Vec<BillingGameMessageOutcome>,
    pub events: Vec<BillingGameEvent>,
    pub billing_player_notices: Vec<BillingPlayerManagerNotice>,
    pub player_fill_notices: Vec<PlayerFillNotice>,
}
...
#[derive(Debug)]
pub struct BillingReleaseReport {
    pub legacy_result: bool,
    pub network_cleanup_performed: bool,
    pub billing_player_manager_error: Option<BillingPlayerManagerInitializationError>,
    pub player_fill_error: Option<BillingPlayerFillInitializationError>,
    pub network_task_errors: Vec<JoinError>,
}

#[derive(Debug)]
pub struct BillingGameThreadReport {
    pub initialization: Result<BillingInitializationReport, BillingInitializationError>,
    pub completed_turns: u64,
    pub runtime_error: Option<BillingRuntimeError>,
    pub release: BillingReleaseReport,
}
...
impl CGame {
    ...
    pub fn set_gs_server(&mut self, server: CServerForGS) {
        self.gs_server = Some(server);
    }

    pub fn set_refresh_info_time(&mut self, interval_ms: u32) {
        self.setup.refresh_info_time_ms = Some(interval_ms);
    }

    pub fn billing_players(&self) -> &Arc<CBillingPlayerManager> {
    ...
    pub fn init_billing_player_manager(
        &mut self,
    ...
    pub fn release_billing_player_manager(
        &mut self,
    ...
    pub fn init_player_fill_manager(
        &mut self,
    ...
    pub fn release_player_fill_manager(
        &mut self,
    ) -> Result<bool, BillingPlayerFillInitializationError> {
        let enabled = self.setup.player_fill_check_enabled.ok_or(
            BillingPlayerFillInitializationError::MissingSetupField("bPlayerFillCheckSrvSwitch"),
    ...
    pub async fn release(&mut self) -> BillingReleaseReport {
    ...
    pub fn pop_player_fill_notice(&self) -> Option<PlayerFillNotice> {
    ...
    pub fn main_loop(&mut self) -> Result<bool, BillingMainLoopError> {
    ...
    pub fn pop_message_outcome(&mut self) -> Option<BillingGameMessageOutcome> {
        self.message_outcomes.pop_front()
    }

    pub fn pop_event(&mut self) -> Option<BillingGameEvent> {
