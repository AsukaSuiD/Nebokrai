...
use nebokrai_shared::runtime::{
    CNetSessionManager, CTimer, CreatedNetSession, NetSessionAsyncResult,
    NetSessionAsyncResultKind, NetSessionCreateBlock, NetSessionEndpoint,
    NetSessionManagerBeginBlock, NetSessionSetCallbackBlock, TimerId, put_string_to_file,
};
...
use crate::app::world_game_view::{
    WorldCreateRoleOrganizingLookupBlock, WorldCreateRoleOrganizingView, WorldGameView,
    WorldRegionNameLookup,
};
use crate::app::world_message::{CMessage, SendMessageError};
use crate::app::world_organizing_view::{WorldOrganizingDispatchView, WorldOrganizingView};
use crate::app::worldothermessage::WorldPlayerNameLookupError;
...
use crate::organizations::faction::{
    CFaction, CityWarEnemyRefreshOutcome, FactionApplyForJoinBlock, FactionApplyForJoinContext,
    FactionApplyForJoinEffects, FactionApplyForJoinOutcome, FactionBillboardKind,
    FactionBillboardStatBlock, FactionCloneSaveBlock, FactionContributorBlock,
    FactionContributorContext, FactionContributorOutcome, FactionDelMemberBlock,
    FactionDelMemberReport, FactionDeleteOrganizingBuildError, FactionDeleteOrganizingOutcome,
    FactionDisbandBlock, FactionDisbandContext, FactionDisbandOutcome, FactionDisbandProgress,
    FactionDisbandRejection, FactionDoJoinBlock, FactionDoJoinContext, FactionDoJoinEffects,
    FactionDoJoinOutcome, FactionEditLeaveWordOutcome, FactionEnemyDelivery,
    FactionEnemyMutationBlock, FactionEnemyMutationContext, FactionEnemyWarLogArgument,
    FactionExperienceBlock, FactionExperienceUpdate, FactionFeatureFunctionUpdate,
    ...
    FactionMemberInfoRequest, FactionOperationAuthorityContext, FactionOperationBlock,
    FactionOperationOutcome, FactionOrganizingInfoContext, FactionOtherInfoBuildError,
    FactionOtherInfoDelivery, FactionOwnedCityDelivery, FactionOwnedCityRefreshBlock,
    FactionOwnedCityRefreshReport, FactionOwnedCityUpdateBuildError, FactionPermitBlock,
    FactionPermitUpdate, FactionPlayerHeaderBlock, FactionPlayerHeaderContext,
    FactionPronounceBlock, FactionPronounceOutcome, FactionPropertyDelivery,
    FactionReinitializationBlock, FactionReinitializationEntry, FactionRemoveApplyMemberOutcome,
    FactionSetParameterBlock, FactionSetParameterContext, FactionSetParameterOutcome,
    FactionSuperiorOrganizingBlock, FactionTalkDelivery, FactionUpgradeBlock,
    FactionUpgradeContext, FactionUpgradeOutcome, FactionUploadIconBlock,
    FactionUploadIconContext, FactionUploadIconOutcome, MemberEnterOutcome, MemberExitOutcome,
...
use crate::organizations::rsunion::{RsUnionOwner, UnionDatabaseLoadRecord, UnionLoadOutcome};
use crate::organizations::union::{
    CUnion, CityTransferEndpointBlock, CityTransferTerminal, ConfederationCreationEndpointBlock,
    ConfederationCreationTerminal, UnionAddFactionBlock, UnionAddFactionEffects,
    UnionAddFactionOutcome, UnionApplicationFactionBlock, UnionApplicationFactionSnapshot,
    UnionApplicationTerminal, UnionApplyForJoinBlock, UnionApplyForJoinContext,
    UnionApplyForJoinEffects, UnionApplyForJoinOutcome, UnionClientSnapshotContext,
    UnionDemiseBlock, UnionDemiseOutcome, UnionDisbandBlock, UnionDisbandOutcome,
    UnionDoJoinBlock, UnionDoJoinContext, UnionDoJoinOutcome, UnionExitBlock, UnionExitContext,
    UnionExitOutcome, UnionFactionFanoutReport, UnionFactionJoinContext, UnionFactionLevelBlock,
    UnionFactionMemberContext, UnionFactionStateMutationContext, UnionFireOutBlock,
    UnionFireOutContext, UnionFireOutEffects, UnionFireOutOutcome, UnionFormatArgument,
    UnionInitialBlock, UnionInitialMutationContext, UnionInitialReport, UnionInviteBlock,
    UnionInviteEffects, UnionInviteOutcome, UnionMasterFactionQueryContext,
    UnionMasterProjectionMutationContext, UnionMemberSnapshotBlock,
    UnionOperatorValidationContext, UnionOwnedCityBooleanMutationReport,
    UnionOwnedCityFanoutReport, UnionOwnedCityMutationBlock, UnionOwnedCityMutationContext,
    UnionPlayerRefreshContext, UnionPlayerRefreshReport, UnionSendInfoContext,
...
pub struct WorldFactionWarDeclarationEffects<'a> {
    game: &'a dyn WorldGameView,
...
pub struct CreateConfederationEndpoint {
    request: ConfederationCreationSessionRequest,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ConfederationCreationSessionReport {
    pub session: CreatedNetSession,
}

pub enum ConfederationCreationSessionBlock {
...
pub fn begin_confederation_creation_session(
...
pub struct PlayerTransferOwnerCity {
    request: CityTransferSessionRequest,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CityTransferSessionReport {
    pub session: CreatedNetSession,
}

pub enum CityTransferSessionBlock {
    Create(NetSessionCreateBlock),
    SetCallback {
        session: CreatedNetSession,
        source: NetSessionSetCallbackBlock,
...
pub fn begin_city_transfer_session(
    manager: &CNetSessionManager,
    request: CityTransferSessionRequest,
    runtime: Arc<dyn CityTransferSessionRuntime>,
    random: impl FnMut(i32) -> i32,
...
/// Синк organizing save/delete очередей `m_stDBData` во владельца сохранения;
/// реализация принадлежит внешнему host-у (старый пакет делегирует её
/// inherent-методам `CGame`) по прецеденту `CountrySaveSink`.
pub trait OrganizingSaveSink {
...
pub struct COrganizingPlayerUpdater<'a> {
...
impl COrganizingCtrl {
 ...
 /// `Initialize`; MSVC tree/list и vtable не переносятся.
    pub const fn with_reached_callback_state() -> Self {
        Self {
 ...
 /// затирает hour/minute/second, а `milliseconds` сохраняет из template.
    pub fn schedule_initial_new_day<Callback: Copy>(
        &mut self,
    ...
    pub fn on_new_day<Callback: Copy, SetCountryDay>(
        &mut self,
        current_time: TagTime,
        timer: &mut CTimer<Callback>,
        callback: Callback,
 ...
 /// `SetTimeEvent(OnNewDay, 0)`, затем member/offense/defence snapshot-ы.
    pub fn finish_database_initialize<Callback: Copy>(
        &mut self,
        current_time: TagTime,
        timer: &mut CTimer<Callback>,
        on_new_day: Callback,
 ...
 /// при уже описанном suffix-block.
    pub async fn initialize_from_database_owners<UnionDb, FactionDb, Callback>(
        &mut self,
 ...
 /// прежний owner, но не сохраняет старую pointer leak.
    pub fn publish_database_organizing(
        &mut self,
        union_master_title: &[u8],
    ...
    pub fn publish_database_load_outcomes(
        &mut self,
        union_master_title: &[u8],
 ...
 /// устраняя старый null-dereference вместо публикации нового отказа.
    pub fn reset_permit_demise(&mut self) -> Vec<i32> {
    ...
    pub fn stat_billboard(&mut self) -> Result<(), FactionBillboardStatBlock> {
 ...
 /// только `0/1/2`.
    pub fn add_faction_billboard_to_byte_array(
        &self,
        output: &mut Vec<u8>,
        billboard_type: i32,
    ) {
    ...
    pub fn upgrade_faction<Context>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        faction_id: i32,
    ...
    pub fn set_faction_parameter<Context>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        faction_id: i32,
    ...
    pub fn upload_faction_icon<Context>(
        &mut self,
        parameters: &COrganizingParam,
        faction_id: i32,
        player_id: i32,
    ...
    pub fn set_contributor_for_player<Context>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        requester_id: i32,
    ...
    pub fn add_contributor_experience(
        &mut self,
        game: &dyn WorldGameView,
        faction_id: i32,
        player_id: i32,
 ...
 /// соответствующий соседний virtual owner `OnMember*Change`.
    pub fn change_faction_member_state<ReadValue>(
        &mut self,
        game: &dyn WorldGameView,
        faction_id: i32,
        player_id: i32,
    ...
    pub fn operate_faction_city_gate(
        &self,
        faction_id: i32,
        player_id: i32,
        region_id: i32,
    ...
    pub fn set_faction_admission_permit(
        &mut self,
        game: &dyn WorldGameView,
        faction_id: i32,
        player_id: i32,
 ...
 /// полного переноса.
    pub fn generate_save_data(
        &mut self,
        sink: &impl OrganizingSaveSink,
        force_all: bool,
        ...
        for (&map_key, faction) in &mut self.factions {
            ...
            if let Some(save_copy) = save_copy {
                sink.append_save_faction(Box::new(save_copy), faction.goods_war_count());
                faction.set_change_data(0);
                saved_factions += 1;
            }
        }
        ...
        for union in self.confederations.values_mut() {
            ...
            if let Some(save_copy) = union.clone_save_data() {
                sink.append_save_union(Box::new(save_copy));
 ...
 /// `nullptr`; caller сам сохраняет последующую pointer-семантику.
    pub fn faction_by_id(&self, faction_id: i32) -> Option<&CFaction> {
        self.factions.get(&faction_id).and_then(Option::as_deref)
    }

    pub fn faction_by_id_mut(&mut self, faction_id: i32) -> Option<&mut CFaction> {
        self.factions
            .get_mut(&faction_id)
            .and_then(Option::as_deref_mut)
    }

    pub fn country_by_faction(
        &self,
        faction_id: i32,
    ...
    pub fn add_owned_city_to_faction(
        &mut self,
        game: &dyn WorldGameView,
        faction_id: i32,
        region_id: i32,
    ...
    pub fn set_faction_goods_war_count(
        &mut self,
        faction_id: i32,
        count: i32,
    ) -> Option<i32> {
    ...
    pub fn confederation_by_id(&self, union_id: i32) -> Option<&CUnion> {
        if union_id < 1 {
            return None;
        }
        self.confederations
    ...
    pub fn faction_id_by_master_player(
        &self,
        player_id: i32,
    ...
    pub fn union_id_by_master_faction(
        &self,
        faction_id: i32,
    ...
    pub fn enable_leave_word_for_master<Context>(
        &mut self,
        player_id: i32,
        context: &mut Context,
    ...
    pub fn edit_leave_word_for_player(
        &mut self,
        game: &dyn WorldGameView,
        player_id: i32,
        leave_word_id: i32,
    ...
    pub fn declare_war_faction_count(
        &self,
    ...
    pub fn faction_count_by_country(
        &self,
        country: u8,
 ...
 /// master-player lookup ветки `0x60118`.
    pub fn apply_for_named_union_join<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        map_key: i32,
        applicant_faction_id: i32,
 ...
 /// signed map-order его reentrant membership/remove проходов.
    pub fn apply_for_faction_join_by_map_key<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        map_key: i32,
 ...
 /// один local-time snapshot, повторный lookup и virtual `DoJoin`.
    pub fn do_faction_join_by_manager<Effects, GetLocalTime>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        manager_id: i32,
    ...
    pub fn add_faction_to_client_by_player_id(
        &self,
        game: &dyn WorldGameView,
        player_id: i32,
    ...
    pub fn add_union_to_client_by_player_id(
        &mut self,
        game: &dyn WorldGameView,
        player_id: i32,
    ...
    pub fn add_all_faction_info_to_client_by_player_id(
        &self,
        game: &dyn WorldGameView,
        player_id: i32,
 ...
 /// сериализацией: для допустимых имён wire тот же, memory overflow удалён.
    pub fn declare_war_faction_page(
        &self,
        source_faction_id: i32,
        requested_page: i32,
    ...
    pub fn union_id_by_master_player(
        &self,
        player_id: i32,
    ...
    pub fn union_id_by_player_membership(
        &self,
        player_id: i32,
    ...
    pub fn apply_for_union_join<Effects>(
        &mut self,
        game: &dyn WorldGameView,
    ...
    pub fn invite_faction_to_union<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        union_id: i32,
        inviter_faction_id: i32,
 ...
 /// virtual `CUnion::FireOut(manager, target faction)`.
    pub fn fire_out_union_by_master<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        manager_id: i32,
 ...
 /// disband через заново вычисленный `CUnion::GetPlayerHeader()`.
    pub fn exit_union_by_player<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        player_id: i32,
 ...
 /// virtual `CUnion::Demise(old master, new master faction)`.
    pub fn demise_union_by_master<Effects>(
        &mut self,
        game: &dyn WorldGameView,
 ...
 /// disband, delete-очередь, owned-city refresh и удаление owner-а.
    pub fn disband_confederation<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        manager_id: i32,
    ...
    pub fn send_organizing_info_to_client(
        game: &dyn WorldGameView,
    ...
    pub fn send_organizing_info_to_all(
        game: &dyn WorldGameView,
        info: &[u8],
        kind: u32,
        color: u32,
    ...
    pub fn disband_faction<Context>(
        &mut self,
        game: &dyn WorldGameView,
        player_id: i32,
        faction_id: i32,
    ...
    pub fn reinitialize_factions_by_level(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
 ...
 /// `CUnion::GetAllFacs`; отсутствующие member-owner-ы пропускаются.
    pub fn expand_city_war_faction_organizings(
        &self,
        faction_id: i32,
 ...
 /// string-table/war-log owner-ом, а не упрощается до прямой вставки set.
    pub fn set_enemy_faction_relation<Context>(
        &mut self,
        first_faction_id: i32,
        second_faction_id: i32,
        context: &mut Context,
    ...
    pub fn generate_db_organizing_id(&self) -> i32 {
    ...
    pub fn set_all_city_faction_enemy_changed(&mut self, changed: bool) {
        for faction in self.factions.values_mut().flatten() {
            faction.set_city_war_enemy_factions_changed(changed);
        }
    }

    pub fn clear_all_city_faction_relations(&mut self) {
        for faction in self.factions.values_mut().flatten() {
            faction.clear_city_war_enemy_factions();
        }
    }

    pub fn update_all_city_enemy_faction_relations(
        &self,
        game: &dyn WorldGameView,
        update_player: &mut dyn FnMut(i32),
    ...
    pub fn is_union_application_reserved(&self, faction_id: i32) -> bool {
        self.request_establishment_union_players
            .iter()
            .any(|reserved_id| *reserved_id == faction_id)
    }

    pub fn push_to_establishment_list(&mut self, faction_id: i32) {
        self.request_establishment_union_players
            .push_back(faction_id);
    }

    pub fn remove_from_establishment_list(&mut self, faction_id: i32) -> bool {
    ...
    pub fn on_attack_city_end<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        result: i32,
        region_id: i32,
    ...
    pub fn on_player_invite_faction<Effects>(
        &mut self,
        game: &dyn WorldGameView,
    ...
    pub fn prepare_faction_creation<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        player_id: i32,
    ...
    pub fn finish_faction_creation<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        player_id: i32,
 ...
 /// endpoint имя первой faction через virtual slot `+0x60`.
    pub fn create_confederation<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        first_faction_id: i32,
        second_faction_id: i32,
    ...
    pub fn finish_confederation_creation<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        first_player_id: i32,
 ...
 /// безусловно продолжает ко второму проходу.
    ...
    pub fn transfer_city_owner<Effects>(
        &mut self,
        game: &dyn WorldGameView,
    ...
    pub fn finish_city_transfer<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        source_faction_id: i32,
        target_faction_id: i32,
    ...
    pub fn finish_union_application<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        union_id: i32,
    ...
    pub fn finish_union_invitation<Effects>(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
        union_id: i32,
 ...
 /// `CUnion::AddToByteArray` в исходном цикле.
    pub fn add_union_to_client_by_faction_id(
        &mut self,
        game: &dyn WorldGameView,
    ...
    pub fn remove_person_from_apply_faction_list(
        &mut self,
        game: &dyn WorldGameView,
        player_id: i32,
    ...
    pub fn faction_by_player_in_apply_list(&self, player_id: i32) -> ApplyFactionLookup {
        for (&map_key, faction) in &self.factions {
            let Some(faction) = faction.as_deref() else {
    ...
    pub fn add_one_top_info(&mut self, timer_flag: i32, param: i32, info: &[u8]) -> i32 {
    ...
    pub fn run_top_info_expiry(&mut self) -> usize {
 ...
 /// а не превращается в новый observable отказ всего main loop.
    ...
    pub fn send_top_info_to_client(
        &self,
        game: &dyn WorldGameView,
        top_info_id: i32,
        timer_flag: i32,
    ...
    pub fn send_all_top_info_to_one_client(
        &self,
        game: &dyn WorldGameView,
        player_id: i32,
...
impl PlayerRankOrganizingLookup for COrganizingCtrl {
    fn faction_lookup_of_player(
        &self,
        player_id: i32,
    ) -> PlayerRankFactionLookup {
        match self.is_free_player(player_id) {
            FreePlayerLookup::NoFaction => {
                PlayerRankFactionLookup::NoFaction
            }
            FreePlayerLookup::Faction(faction_id) => {
                PlayerRankFactionLookup::Faction(
                    faction_id,
                )
            }
            FreePlayerLookup::BlockedNullFaction { map_key } => {
                PlayerRankFactionLookup::NullFaction {
                    map_key,
                }
            }
        }
...
impl WorldDeleteRoleOrganizingGate for COrganizingCtrl {
    fn apply_delete_role(
        &mut self,
        game: &dyn WorldGameView,
        parameters: &COrganizingParam,
...
/// Делегирует inherent `organizing_by_name` той же свёрткой, которую давал
/// прежний адаптер старого пакета: любой lookup-block отражается как
/// `NullOwner` (шов намеренно не переносит полную таблицу причин).
...
/// владельцу: Realm контекст-трейты не принимают владельца игры (прецедент
...
/// `&mut COrganizingCtrl` и `&dyn WorldGameView` локальным bridge на месте
/// вызова; переходным адаптерам старого пакета поля остаются видимыми.
pub struct UnionOrganizingBridge<'a> {
    pub organizing: &'a mut COrganizingCtrl,
    pub game: &'a dyn WorldGameView,
}
...
impl WorldOrganizingView for COrganizingCtrl {
...
impl WorldOrganizingDispatchView for COrganizingCtrl
{
    fn is_free_player(&self, player_id: i32) -> FreePlayerLookup {
        COrganizingCtrl::is_free_player(self, player_id)
    }
