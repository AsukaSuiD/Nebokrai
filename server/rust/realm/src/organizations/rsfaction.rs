...
use crate::organizations::checkpoint::check_point;
use crate::organizations::faction::{
    CFaction, FactionBaseProperty, FactionDatabaseBaseState, FactionInitialBlock,
    TagApplyPerson, TagLeaveWord, TagPronounceWord, UnterminatedLeaveWordField,
};
...
/// Linux/TDS-реализация `CRsFaction` с caller-транзакцией и собственной
/// notice-очередью, перенесённая из старого пакета без изменения SQL-порядка,
/// partial effects и наблюдаемых результатов.
#[derive(Default)]
pub struct TiberiusRsFaction {
...
fn build_faction_leave_word_insert(
    ...
    let escaped_content = check_point(&content[..content.len() - 1]);
