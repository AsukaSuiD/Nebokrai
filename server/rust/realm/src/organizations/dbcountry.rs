//! DB-владелец стран WorldServer из `dbcountry.cpp`, перенесённый в Realm
//! `organizations/`: трейт `DbCountryOwner`, его data-семья (save-проекции)
//! и Tiberius-реализация `TiberiusDbCountry`. Источник контракта — точная пара
...
use crate::organizations::country::{CCountry, CountryMinisterState};
use crate::organizations::countryhandler::{CCountryHandler, CountryAppendDisposition};
...
/// Linux/TDS-реализация `CDbCountry` с caller-соединением и собственной
/// notice-очередью, перенесённая из старого пакета без изменения SQL-порядка,
/// исходных значений bool и уже применённых записей при поздней ошибке.
#[derive(Default)]
pub struct TiberiusDbCountry {
