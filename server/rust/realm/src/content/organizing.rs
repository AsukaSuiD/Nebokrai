...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum MemberPurviewMutation {
    InvalidPurview,
    MemberNotFound,
    Unchanged,
    Changed,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct UnterminatedMemberField {
...
#[derive(Clone, Copy)]
#[repr(C)]
pub struct TagMemInfo {
    pub id: i32,
    pub name: [u8; MEMBER_NAME_CAPACITY],
    pub level: i32,
    pub occupation: i32,
    pub job_level: i32,
    pub title: [u8; MEMBER_TEXT_CAPACITY],
    pub purview: [EPurviewOwnState; PURVIEW_COUNT],
    pub region: [u8; MEMBER_TEXT_CAPACITY],
    pub last_online_time: TagTimeValue,
    pub contribute: bool,
}
...
impl TagMemInfo {
    ...
    pub fn name_wire_bytes(&self) -> Result<&[u8], UnterminatedMemberField> {
        terminated_field(&self.name, "strName")
    }

    pub fn title_wire_bytes(&self) -> Result<&[u8], UnterminatedMemberField> {
        terminated_field(&self.title, "strTitle")
    }

    pub fn region_wire_bytes(&self) -> Result<&[u8], UnterminatedMemberField> {
        terminated_field(&self.region, "strRegion")
    }

    pub fn purview_wire_bytes(&self) -> [u8; 44] {
        let mut bytes = [0; 44];
    ...
    pub fn last_online_wire_bytes(&self) -> [u8; 16] {
