...
//! владельца DB-слоя делегируется по `CPlayer` (`characters/player`). Block-типы
//! ошибок вставки и addon-применения остаются у владельца игрока и codec
//! товара, поэтому load-семья параметризована `AddonBlockT`/`InsertBlockT` и
//! связана с трейтом через associated types. Будущее `load_goods` захватывает
...
use crate::characters::player::{CPlayer, PlayerLoadedGoodsInsertBlock};
use crate::content::cgoods::GoodsLoadedAddonBlock;
use crate::content::cgoodsfactory::create_goods_no_probability;
use crate::content::goods::GoodsBasePropertiesRegistry;
use crate::content::goodsdb::{
    GoodsAddonPropertySnapshot, GoodsObjectSnapshot, GoodsPropertiesSnapshot,
};
use crate::content::goodslistener::{
    GoodsContainerTraversalSnapshot, GoodsListener, GoodsTraversalBlock,
};
use crate::persistence::row::{get_integer, get_value};
...
impl DbGoodsOwner<CPlayer> for TiberiusDbGoods {
    ...
    async fn load_goods(
        ...
        changed_goods_indices: &BTreeMap<u32, u32>,
        dakong_addon_types: &BTreeSet<i32>,
    ) -> GoodsLoadOutcome<GoodsLoadedAddonBlock, PlayerLoadedGoodsInsertBlock> {
