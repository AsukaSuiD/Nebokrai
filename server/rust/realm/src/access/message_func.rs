...
use super::dbqueue::{AuthQuestData, DbQuest, LockQuestData, LockUntil, ServerInfo};
use super::kl_ipfilter::IpFilter;
use crate::app::auth_message::{AuthMessageHandler, AuthMessageKind, CMessage, DispatchError};

#[derive(Debug, Eq, PartialEq)]
pub enum LoginServerNotice {
    Connected {
        address: Ipv4Addr,
        socket_id: i32,
        allowed: bool,
...
pub struct AuthMessageHandlers {
...
impl AuthMessageHandlers {
    ...
    pub fn replace_ip_filter(&mut self, enabled: bool, allowed_patterns: Vec<[u8; 4]>) {
        self.ip_filter_enabled = enabled;
        self.ip_allower.replace_patterns(allowed_patterns);
    }

    pub fn login_server_socket_id(&self, area_id: i32) -> i32 {
        self.login_servers.get(&area_id).copied().unwrap_or(0)
    }

    pub fn login_server_area_id(&self, socket_id: i32) -> i32 {
        self.area_for_socket(socket_id).unwrap_or(0)
    }

    pub fn login_server_socket_ids(&self) -> impl Iterator<Item = i32> + '_ {
        self.login_servers.values().copied()
    }

    pub fn pop_notice(&mut self) -> Option<LoginServerNotice> {
