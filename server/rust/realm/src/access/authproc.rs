...
pub enum AuthDatabaseOperation {
...
pub struct AuthDatabaseNotice {
    /// Операция, завершившаяся ошибкой.
    pub operation: AuthDatabaseOperation,
    /// Ошибка без connection string и credential values.
    pub error: AuthDatabaseError,
}
...
pub struct TiberiusAuthDatabase {
    auth_config: Config,
    log_config: Config,
    runtime: Runtime,
}
...
impl TiberiusAuthDatabase {
    ...
    pub fn new(config_reader: &ConfigReader) -> Result<Self, AuthDatabaseError> {
    ...
    Database: AuthDatabase,
{
    ...
    pub fn pop_notice(&mut self) -> Option<AuthDatabaseNotice> {
        self.notices.pop_front()
    }

    /// Обрабатывает одну доказанную DB-команду и публикует её результат.
    pub fn process_quest(&mut self, game: &AuthDbContext, quest: DbQuest) {
        match quest {
...
#[derive(Debug)]
pub enum AuthDatabaseWorkerStartError {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct AuthDatabaseWorkerJoinError {
    /// Номер worker’а в исходном порядке создания.
...
pub struct AuthDatabaseWorkers {
...
impl AuthDatabaseWorkers {
    ...
    pub fn stop_and_join(&mut self) -> Vec<AuthDatabaseWorkerJoinError> {
    ...
    pub fn pop_notice(&self) -> Option<AuthDatabaseNotice> {
