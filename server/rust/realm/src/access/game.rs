...
//! `server/rust/src/manifest/_loginserver_export_manifest.toml`). Связывает
//! AuthServer, маршрут World/client, CD-key, фоновые запросы к БД, setup и
//! цикл `Init -> MainLoop -> Release`.
//!
//! Перед чтением `aslist.ini` список очищается; неполная последняя запись не
//! отменяет прочитанные адреса. Отсутствие Auth-соединения не прерывает Init.
//! Reconnect перебирает адреса по порядку и передаёт новый client через FIFO
//! старого; старый уничтожается до включения нового. Карта World сохраняет
//! числовой порядок: повторный CD-key отбрасывается, поиск и удаление
//! выбирают первый World по ID, полный промах очищает login-map.
//! `PrepareEnter`, `KickOut` и маршруты ролей допускают отсутствующий World и
//! не откатывают выполненные эффекты.
//!
//! Записи online-user и server-info не объединены в транзакцию: отдельная
//! SQL-ошибка не останавливает пакет. Setup-файлы читаются позиционно, без
//! проверки имён слева от значений; значение `m_lVerifiSignUpper` не найдено
//! (UNKNOWN) и остаётся явной внешней границей. Reload не перезапускает
//! listeners; abstract Unix socket заменяет GUI-проверку единственного
//! экземпляра.
//!
//! Очереди снимаются в порядке World -> Client -> Auth; `MainLoop` сохраняет
//! wrapping-таймеры, очистку telemetry перед ping и паузу 1 ms. `Release`
//! после любого исхода `Init` останавливает сети, workers и БД в исходном
//! порядке; повторный вызов не повторяет очистку.
...
use super::acclogthread::{AccLogThread, AccLogThreadNotice};
use super::gasthread::{
...
use super::acclogqueue::AccLogQueue;
...
use super::servlogqueue::{ServLog, ServLogQueue};
use nebokrai_shared::network::{ClientConnectError, ClientSendQueue};
use nebokrai_shared::network::{DEFAULT_SOCKET_TYPE, legacy_inet_addr, legacy_ipv4_word};
use crate::app::login_message::{CMessage, SendMessageError};
use crate::app::login_auth_client::{
    AuthClientEvent, AuthClientEventPublisher, AuthClientIoError, AuthClientIoStep,
    CMyNetClientAuth,
};
use crate::app::login_server::CMyNetServerClient;
use crate::app::login_world_server::CMyNetServerWorld;
use crate::app::login_server_client::ClientReceiveError;
use crate::app::login_world_server_client::WorldReceiveError;
...
use nebokrai_shared::runtime::ini_decode;
...
impl AuthServerConfig {
    pub fn new(host: Vec<u8>, port: u16) -> Self {
        Self { host, port }
    }
...
pub struct LoadAsListOutcome {
    pub loaded: usize,
    pub stopped_on_invalid_pair: bool,
}

#[derive(Debug)]
pub enum AuthConnectFailure {
...
#[derive(Debug)]
pub struct AuthConnectAttempt {
    pub endpoint: AuthServerConfig,
    pub failure: Option<AuthConnectFailure>,
}

#[derive(Debug)]
pub struct AuthInitializationReport {
    pub legacy_result: bool,
    pub attempts: Vec<AuthConnectAttempt>,
    pub connected: Option<AuthServerConfig>,
    pub login_server_info: Result<i32, SendMessageError>,
}

#[derive(Debug)]
pub struct AuthReconnectReport {
    pub connected: bool,
    pub attempts: Vec<AuthConnectAttempt>,
}
...
impl WorldRoute {
    pub fn new(world_id: i32, name: Vec<u8>, state_level: i32) -> Self {
        Self {
            world_id,
            name,
            state_level,
...
impl PingGameServerInfo {
    pub fn new(ip: Vec<u8>, port: u32, player_count: u32) -> Self {
        Self {
            ip,
            port,
            player_count,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct PingWorldServerInfo {
    ip: Vec<u8>,
    port: u32,
    player_count: u32,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum WorldActivationFailure {
    UnknownWorld,
    NameMismatch,
}

#[derive(Debug)]
pub struct WorldInfoBroadcastFailure {
    pub account: Vec<u8>,
    pub error: GameRouteError,
}

#[derive(Debug)]
pub enum WorldActivationOutcome {
    Activated {
        world_count: i32,
...
#[derive(Debug)]
pub struct WorldDeactivationOutcome {
    pub world_count: i32,
    pub broadcast_failures: Vec<WorldInfoBroadcastFailure>,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ServerInfoLogDisposition {
    Disabled,
    Queued,
    SetupValueMissing,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ServerInfoDatabaseOperation {
    Connect,
    LoginLookup,
    LoginWrite,
    WorldLookup {
...
#[derive(Debug)]
pub enum ServerInfoDatabaseFailure {
    Runtime(io::Error),
    Database {
...
#[derive(Debug)]
pub struct ServerInfoDatabaseReport {
    pub staged_world_records: usize,
    pub staged_game_records: usize,
    pub queued_log_records: i32,
    pub attempted_log_records: usize,
    pub failure: Option<ServerInfoDatabaseFailure>,
}
...
#[derive(Debug)]
pub enum OnlineUserDatabaseFailure {
    Runtime(io::Error),
...
#[derive(Debug)]
pub struct OnlineUserDatabaseReport {
    pub world_id: i32,
    pub requested_accounts: usize,
    pub delete_attempted: bool,
    pub insert_attempts: usize,
    pub failures: Vec<OnlineUserDatabaseFailure>,
}

#[derive(Debug)]
pub enum OnlineUserUpdateStartError {
    SettingsMissing,
    Spawn(io::Error),
}
...
#[derive(Debug)]
pub enum ProcessMessageError<HandlerError> {
    AuthLifecycle(AuthLifecycleError),
    Handler(HandlerError),
}

#[derive(Debug)]
pub enum LoginMainLoopError {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum LoginNetworkDirection {
    World,
    Client,
}

#[derive(Debug, Default)]
pub struct LoginWorldNetworkTurn {
    pub admissions: Vec<AdmissionOutcome>,
    pub accept_errors: Vec<io::Error>,
    pub io_completions: Vec<ServerIoCompletion>,
    pub processed_commands: i32,
    pub snapshot_errors: Vec<ServerSnapshotError<WorldReceiveError>>,
}

#[derive(Debug, Default)]
pub struct LoginClientNetworkTurn {
    pub admissions: Vec<AdmissionOutcome>,
    pub accept_errors: Vec<io::Error>,
    pub io_completions: Vec<ServerIoCompletion>,
    pub processed_commands: i32,
    pub snapshot_errors: Vec<ServerSnapshotError<ClientReceiveError>>,
}

#[derive(Debug, Default)]
pub struct LoginNetworkTurn {
    pub world: LoginWorldNetworkTurn,
    pub client: LoginClientNetworkTurn,
    pub auth: Option<Result<AuthClientIoStep, AuthClientIoError>>,
}

#[derive(Debug, Default)]
pub struct LoginRuntimeStep {
    pub network: LoginNetworkTurn,
    pub main_loop: Option<LoginMainLoopOutcome>,
    pub auth_handler_notices: Vec<AuthHandlerNotice>,
    pub world_operator_log_records: Vec<WorldOperatorLogRecord>,
    pub online_user_database_reports: Vec<OnlineUserDatabaseReport>,
    pub server_info_database_reports: Vec<ServerInfoDatabaseReport>,
    pub rs_cdkey_notices: Vec<RsCdKeyNotice>,
}

#[derive(Debug)]
pub enum LoginNetworkRuntimeError {
...
#[derive(Debug)]
pub struct LoginGameThreadReport {
    pub initialization: Result<LoginInitializationReport, LoginInitializationError>,
    pub completed_turns: u64,
    pub runtime_error: Option<LoginRuntimeError>,
    pub release: LoginReleaseReport,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PasswordFailureOutcome {
    Disabled,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum LoginSetupSource {
    Plain,
    Encoded,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct LoginSetupLoadReport {
    pub source: LoginSetupSource,
    pub parsed_pairs: usize,
    pub stopped_at_pair: Option<usize>,
    pub state_levels_applied: bool,
    pub login_queue_initialized: bool,
}

#[derive(Debug)]
pub struct LoginSetupOpenError {
    plain: io::Error,
    encoded: io::Error,
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct LoginSetupExLoadReport {
    pub parsed_pairs: usize,
    pub stopped_at_pair: Option<usize>,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct LoginListenPortLoadReport {
    pub parsed_pairs: usize,
    pub stopped_at_pair: Option<usize>,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct LoginWorldSetupLoadReport {
    pub parsed_records: usize,
    pub unique_worlds: usize,
    pub stopped_at_record: Option<usize>,
}

#[derive(Debug)]
pub enum LoginNoQueueAccountsLoadDisposition {
    Loaded(NoQueueAccountsLoadReport),
    Failed(NoQueueAccountsLoadError),
}

#[derive(Debug)]
pub struct LoginNoQueueAccountsReloadOutcome {
    pub legacy_result: bool,
    pub load: LoginNoQueueAccountsLoadDisposition,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct LoginQueueOwnerMissing;
...
#[derive(Debug)]
pub struct LoginSetupReloadOutcome {
    pub legacy_result: bool,
    pub load: Result<LoginSetupLoadReport, LoginSetupOpenError>,
}

#[derive(Debug)]
pub enum LoginSetupReloadError {
...
#[derive(Debug)]
pub struct LoginWorldSetupReloadOutcome {
    pub legacy_result: bool,
    pub load: Result<LoginWorldSetupLoadReport, io::Error>,
    pub broadcast_failures: Vec<WorldInfoBroadcastFailure>,
}

#[derive(Debug)]
pub struct LoginInitializationPrefixReport {
    pub setup: LoginSetupLoadReport,
    pub listen_ports: LoginListenPortLoadReport,
    pub setup_ex: Result<LoginSetupExLoadReport, io::Error>,
    pub world_setup: LoginWorldSetupLoadReport,
    pub auth_servers: LoadAsListOutcome,
    pub world_routes: usize,
    pub login_queue_initialized: bool,
    pub no_queue_accounts: Result<NoQueueAccountsLoadReport, NoQueueAccountsLoadError>,
    pub online_user_clear: Result<(), RsCdKeyDatabaseError>,
    pub auth: AuthInitializationReport,
    pub startup_log: ServerInfoLogDisposition,
}

#[derive(Debug)]
pub struct LoginInitializationThroughGasReport {
    pub prefix: LoginInitializationPrefixReport,
    pub gas_thread_start: Result<(), io::Error>,
}

#[derive(Debug)]
pub struct LoginInitializationReport {
    pub through_gas: LoginInitializationThroughGasReport,
    pub account_log_thread_start: Result<(), io::Error>,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum LoginReleaseOperatorNotice {
    Exiting,
    Exited,
}

#[derive(Debug)]
pub struct LoginReleaseReport {
    pub legacy_result: bool,
    pub performed: bool,
    pub operator_notices: [LoginReleaseOperatorNotice; 2],
    pub world_network_was_present: bool,
    pub client_network_was_present: bool,
    pub world_network_shutdown_snapshots: u32,
    pub client_network_shutdown_snapshots: u32,
    pub world_network_task_errors: Vec<JoinError>,
    pub client_network_task_errors: Vec<JoinError>,
    pub auth_client_was_present: bool,
    pub account_log_thread_was_present: bool,
    pub gas_thread_was_present: bool,
    pub database_owner_was_present: bool,
    pub online_user_clear: Option<Result<(), RsCdKeyDatabaseError>>,
    pub login_queue_was_present: bool,
    pub online_user_workers_joined: usize,
    pub server_info_workers_joined: usize,
}
...
#[derive(Debug)]
pub enum LoginInitializationError {
...
#[derive(Debug)]
pub enum LoginNetworkInitializationError {
    MissingSetupField(&'static str),
...
#[derive(Debug)]
pub enum LoginSetupExReloadError {
    Load(io::Error),
    MissingClientServer,
    MissingWorldServer,
}
...
impl LoginSetup {
    ...
    pub const fn server_version(&self) -> Option<i32> {
        self.server_version
    }

    pub const fn ip_checks(&self) -> Option<(bool, bool, bool)> {
        match (
    ...
    pub const fn inside_use(&self) -> Option<i32> {
        self.inside_use
    }
...
pub fn resolve_legacy_ascii_case(
...
pub fn load_runtime_area_id(runtime_directory: &Path) -> Result<i32, io::Error> {
...
impl CGame {
    ...
    pub fn load_as_list(&mut self, path: &Path) -> Result<LoadAsListOutcome, io::Error> {
    ...
    pub async fn init_auth_client(&mut self) -> AuthInitializationReport {
    ...
    pub fn is_connect_as(&self) -> bool {
        self.auth_client
            .as_ref()
            .is_some_and(CMyNetClientAuth::is_connected)
    }

    pub fn disconnect_as(&mut self) {
    ...
    pub async fn reconnect_as(&mut self) -> Result<AuthReconnectReport, AuthLifecycleError> {
    ...
    pub async fn start_reconnect_thread(&mut self) -> Result<(), AuthLifecycleError> {
        let publisher = self
            .auth_client
            .as_ref()
    ...
    pub fn process_next_auth_event(
        &mut self,
    ...
    pub async fn run_auth_io_once(&mut self) -> Result<AuthClientIoStep, AuthClientIoError> {
        let client = self
            .auth_client
            .as_mut()
    ...
    pub fn auth_send_queue(&self) -> Option<&ClientSendQueue> {
        self.auth_client.as_ref().map(CMyNetClientAuth::send_queue)
    }

    pub fn current_auth_server(&self) -> Option<&AuthServerConfig> {
        self.current_auth_server.as_ref()
    }

    pub fn pending_auth_events(&self) -> i32 {
        self.auth_client
            .as_ref()
    ...
    pub async fn process_message<Runner, HandlerError>(
        &mut self,
    ...
    pub async fn release(&mut self) -> LoginReleaseReport {
        if self.exit_requested {
    ...
    pub fn init_net_server_client(&mut self) -> Result<(), LoginNetworkInitializationError> {
    ...
    pub fn init_net_server_world(&mut self) -> Result<(), LoginNetworkInitializationError> {
    ...
    pub fn attach_client_sender(&mut self, sender: ServerCommandHandle) {
        self.client_sender = Some(sender);
    }

    pub fn attach_world_sender(&mut self, sender: ServerCommandHandle) {
        self.world_sender = Some(sender);
    }

    pub fn attach_rs_cdkey_owner(&mut self, owner: Box<dyn RsCdKeyOwner>) {
        self.rs_cdkey_owner = Some(owner);
    }

    pub const fn login_setup(&self) -> &LoginSetup {
        &self.setup
    }

    pub const fn login_setup_ex(&self) -> &LoginSetupEx {
        &self.setup_ex
    }
    ...
    pub fn set_list_world_info_by_setup(&mut self) -> usize {
    ...
    pub fn world_server_is_open_state(&self, world_id: i32) -> bool {
    ...
    /// сохраняются.
    pub async fn initialize_before_worker_threads(
        &mut self,
        runtime_directory: &Path,
        auth_manager: &mut AuthManager,
    ...
    pub async fn initialize_through_gas_thread(
        &mut self,
        runtime_directory: &Path,
        auth_manager: &mut AuthManager,
    ...
    pub fn get_login_world_cdkey_numbers(&self) -> i32 {
    ...
    pub fn initialize_rs_cdkey_from_setup(
        &mut self,
    ...
    pub fn initialize_server_info_database_from_setup(&mut self) {
    ...
    pub fn attach_tiberius_rs_cdkey(
        &mut self,
    ...
    pub fn pop_rs_cdkey_notice(&mut self) -> Option<RsCdKeyNotice> {
        self.rs_cdkey_owner
            .as_mut()
            .and_then(|owner| owner.pop_notice())
    }

    pub fn configure_password_failure_policy(&mut self, ban_minutes: i32, error_limit: i32) {
        self.password_ban_minutes = ban_minutes;
        self.password_error_limit = error_limit;
    }

    pub fn configure_valid_code_policy(&mut self, enabled: bool, error_limit: i32) {
        self.setup_ex.valid_code = i32::from(enabled);
        self.setup_ex.valid_error_upper_limit = error_limit;
    }

    pub const fn configure_valid_error_stay_time(&mut self, stay_time_ms: u32) {
        self.setup_ex.valid_error_stay_time_ms = stay_time_ms;
    }

    pub const fn configure_login_queue_timeouts(
        &mut self,
        matrix_timeout_ms: i32,
        valid_code_overtime_ms: i32,
    ) {
    ...
    pub const fn configure_quest_player_data_interval(&mut self, interval_ms: i32) {
        self.setup_ex.quest_player_data_interval_ms = interval_ms;
    }

    pub const fn quest_player_data_interval_ms(&self) -> u32 {
        self.setup_ex.quest_player_data_interval_ms as u32
    }

    pub const fn valid_code_enabled(&self) -> bool {
        self.setup_ex.valid_code != 0
    }

    pub const fn valid_error_limit(&self) -> i32 {
        self.setup_ex.valid_error_upper_limit
    }

    pub const fn valid_error_stay_time_ms(&self) -> u32 {
    ...
    pub fn rs_cdkey_owner_mut(&mut self) -> Option<&mut (dyn RsCdKeyOwner + '_)> {
    ...
    pub fn upsert_world_route(&mut self, route: WorldRoute) {
        self.world_routes.insert(route.world_id, route);
    }

    pub fn append_ping_world_server_info(&mut self, info: PingWorldServerInfo) -> usize {
        self.ping_world_server_info.push(info);
        self.ping_world_server_info.len() - 1
    }

    pub fn add_cdkey(&mut self, account: &[u8], world_id: i32) -> bool {
    ...
    pub fn find_cdkey(&self, account: &[u8]) -> i32 {
        let account = legacy_c_string_prefix(account);
        self.world_cdkeys
            .iter()
    ...
    pub fn clear_login_cdkey(&mut self, account: &[u8]) -> bool {
        self.login_cdkey_world
            .remove(legacy_c_string_prefix(account))
            .is_some()
    }

    pub fn clear_cdkey(&mut self, account: &[u8]) {
        let account = legacy_c_string_prefix(account);
    ...
    pub fn start_online_user_database_update(
        &mut self,
        world_id: i32,
        accounts: Vec<Vec<u8>>,
    ...
    pub fn pop_online_user_database_report(&mut self) -> Option<OnlineUserDatabaseReport> {
        self.collect_finished_online_user_updates();
    ...
    pub fn pop_server_info_database_report(&mut self) -> Option<ServerInfoDatabaseReport> {
        self.collect_finished_server_info_logs();
    ...
    pub fn set_world_socket_map_id(
        &self,
        socket_id: i32,
        world_id: i32,
    ...
    pub fn world_name_by_id(&self, world_id: i32) -> Option<&[u8]> {
        self.world_routes
            .get(&world_id)
            .map(|route| route.name.as_slice())
    }

    pub fn clear_cdkeys_by_world_id(&mut self, world_id: i32) {
    ...
    pub fn queue_world_connected_operator_log(&mut self, world_name: &[u8]) {
        self.world_operator_log_records
    ...
    pub fn queue_world_lost_operator_log(&mut self, world_name: &[u8]) {
        self.world_operator_log_records
    ...
    pub fn deactivate_world(&mut self, world_id: i32) -> WorldDeactivationOutcome {
        if let Some(route) = self.world_routes.get_mut(&world_id) {
            route.state_level = 0;
        }
        let broadcast_failures = self.update_world_info_to_all_clients();
    ...
    pub fn queue_world_connected_server_info_log(
        &mut self,
        source_ip: u32,
        world_id: i32,
        world_name: &[u8],
    ...
    pub fn server_info_log_disposition(&self) -> ServerInfoLogDisposition {
        match self.setup.server_info_log_time {
    ...
    pub fn pop_server_info_log_record(&self) -> Option<ServLog> {
        self.server_info_log_shared.logs.pop()
    }

    pub fn pop_world_operator_log_record(&mut self) -> Option<WorldOperatorLogRecord> {
    ...
    pub fn world_id_by_name(&self, name: &[u8]) -> i32 {
        self.world_routes
            .values()
    ...
    pub fn is_exit_world(&self, name: &[u8]) -> bool {
        self.world_id_by_name(name) != -1
    }

    pub fn login_world_player_num_by_name(&self, name: &[u8]) -> i32 {
    ...
    pub fn login_cdkey_world_server(&self, account: &[u8]) -> Option<&[u8]> {
        self.login_cdkey_world.get(account).map(Vec::as_slice)
    }

    pub fn set_login_cdkey_world_server(&mut self, account: &[u8], world_name: &[u8]) {
        self.login_cdkey_world
            .insert(account.to_vec(), world_name.to_vec());
    }

    pub fn send_msg_to_world(
        &self,
        message: &CMessage,
        world_id: i32,
    ) -> Result<i32, GameRouteError> {
    ...
    pub fn kick_out(&self, account: &[u8]) -> Result<bool, GameRouteError> {
        let account = legacy_c_string_prefix(account);
    ...
    pub fn quit_client_by_cdkey(&self, account: &[u8]) -> Result<i32, GameRouteError> {
        let sender = self
            .client_sender
            .as_ref()
            .ok_or(GameRouteError::MissingClientServer)?;
    ...
    pub fn push_back_pwd_checked(&mut self, checked: TagPwdChecked) {
    ...
    pub fn login_queue(&self) -> Arc<CLoginQueue> {
        Arc::clone(self.login_queue_ref())
    }

    pub fn handle_pwd_checked(&mut self) -> HandlePwdCheckedReport {
        let queue = Arc::clone(self.login_queue_ref());
        queue.handle_pwd_checked(self)
    }

    pub fn run_login_queue(
        &mut self,
        auth_manager: &mut AuthManager,
    ) -> LoginQueueRunReport {
        let queue = Arc::clone(self.login_queue_ref());
    ...
    pub fn run_login_queue_timeout_tail(&self) -> LoginQueueTimeoutReport {
    ...
    pub fn account_leave_log(&mut self, account: &[u8]) {
        let end = account
            .iter()
            .position(|byte| *byte == 0)
    ...
    pub fn leave_log(&mut self, account: &[u8]) {
        self.account_log_queue
    ...
    pub fn add_world_info_to_message_for_account(
        &self,
    ...
    pub fn register_password_failure(&mut self, account: &[u8]) -> PasswordFailureOutcome {
    ...
    pub fn push_auth_handler_notice(&mut self, notice: AuthHandlerNotice) {
        self.auth_handler_notices.push_back(notice);
    }

    pub fn pop_auth_handler_notice(&mut self) -> Option<AuthHandlerNotice> {
        self.auth_handler_notices.pop_front()
    }

    pub fn send_all_world(&self, message: &CMessage) -> Result<i32, GameRouteError> {
        let sender = self
            .world_sender
            .as_ref()
            .ok_or(GameRouteError::MissingWorldServer)?;
    ...
    pub fn send_to_auth(&self, message: &CMessage) -> Result<i32, GameRouteError> {
        let sender = self
            .auth_send_queue()
...
pub async fn clear_online_user_database(
    settings: LoginDatabaseSettings,
