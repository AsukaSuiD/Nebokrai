...
#[derive(Clone)]
pub struct LoginDatabaseSettings {
    host: Vec<u8>,
    database: Vec<u8>,
    user: Vec<u8>,
    password: Vec<u8>,
...
pub async fn connect_login_database(
...
impl LoginDatabaseSettings {
    ...
    pub fn new(host: Vec<u8>, database: Vec<u8>, user: Vec<u8>, password: Vec<u8>) -> Self {
        Self {
            host,
            database,
            user,
...
#[derive(Debug)]
pub struct RsCdKeyInitializationError(TryCurrentError);

impl fmt::Display for RsCdKeyInitializationError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(formatter, "не найден runtime Login DB: {}", self.0)
    }
}
...
pub struct ActiveBanRecord {
    pub account: Vec<u8>,
    pub ban_until: Vec<u8>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ActiveBanList {
    pub total_count: i32,
    pub records: Vec<ActiveBanRecord>,
}

/// Узкая объектная граница достигнутых операций исходного `CRsCDKey`.
pub trait RsCdKeyOwner {
...
impl TiberiusRsCdKey {
    ...
    pub fn new(settings: LoginDatabaseSettings) -> Result<Self, RsCdKeyInitializationError> {
...
impl RsCdKeyOwner for TiberiusRsCdKey {
    fn cd_key_ban(&mut self, account: &[u8], minutes: i32) -> bool {
        ...
        match self.block_on_database(Self::apply_ban(self.config.clone(), account_text, minutes)) {
            Ok(inserted) => {
    ...
    fn fix_pt_account(&mut self, account: &[u8]) -> Vec<u8> {
        ...
        let account_text = decode_ansi(account);
        match self.block_on_database(Self::read_fixed_account(self.config.clone(), account_text)) {
    ...
    fn get_ban_time(&mut self, account: &[u8]) -> Option<NaiveDateTime> {
        ...
        match self.block_on_database(Self::read_ban_time(self.config.clone(), account)) {
    ...
    fn list_active_bans(&mut self) -> Option<ActiveBanList> {
        match self.block_on_database(Self::read_active_bans(self.config.clone())) {
    ...
    fn is_between_ip(&mut self, check_enabled: bool, account: &[u8], raw_ipv4: u32) -> bool {
        ...
        let account = decode_ansi(account);
        match self.block_on_database(Self::read_ip_list(self.config.clone(), account, raw_ipv4)) {
    ...
    fn matrix_used(&mut self, account: &[u8]) -> bool {
        let account = decode_ansi(account);
        match self.block_on_database(Self::has_current_matrix(self.config.clone(), account)) {
            Ok(used) => used,
            Err(error) => {
    ...
    fn matrix_validate(
        ...
        let account = decode_ansi(account);
        let matrix_card =
            match self.block_on_database(Self::read_matrix_card(self.config.clone(), account)) {
