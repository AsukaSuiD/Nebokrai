...
use crate::access::message_func::{AuthMessageHandlers, LoginServerNotice};
use crate::access::configreader::{ConfigLoadError, ConfigReader};
use crate::access::dbqueue::{
    AuthExResultData, AuthResultData, DbQuest, DbResult, LockResultData, ServerInfo,
};
use crate::access::kl_ipfilter::{IpFilterLoadError, load_ip_patterns};
use crate::access::authproc::{
    AuthDatabaseNotice, AuthDatabaseWorkerJoinError, AuthDatabaseWorkerStartError,
    AuthDatabaseWorkers,
};
use nebokrai_shared::network::{DEFAULT_SOCKET_TYPE, legacy_ipv4_word};
use crate::app::auth_message::{
    AuthMessageHandler, CMessage, DispatchError, SendMessageError,
};
use crate::app::auth_server::CMyNetServerAuth;
use crate::app::auth_server_client::AuthReceiveError;
...
pub use crate::access::networkconfig::AuthNetworkConfig;

#[derive(Default)]
pub struct AuthRuntimeStep {
    pub processed_network_commands: i32,
    pub processed_messages: i32,
    pub processed_database_results: i32,
    pub server_info_requested: bool,
    pub server_info_write_due: bool,
    pub server_info_write_queued: bool,
    pub admissions: Vec<AdmissionOutcome>,
    pub io_completions: Vec<ServerIoCompletion>,
    pub network_errors: Vec<ServerSnapshotError<AuthReceiveError>>,
    pub accept_errors: Vec<io::Error>,
    pub login_server_notices: Vec<LoginServerNotice>,
    pub database_notices: Vec<AuthDatabaseNotice>,
}
...
#[derive(Debug)]
pub enum AuthInitializationNotice {
...
pub struct AuthInitializationReport {
    pub notices: Vec<AuthInitializationNotice>,
    pub update_server_info_enabled: bool,
}

#[derive(Debug)]
pub enum AuthInitializationError {
...
#[derive(Debug)]
pub struct AuthReleaseReport {
    pub database_worker_errors: Vec<AuthDatabaseWorkerJoinError>,
    pub network_error: Option<AuthRuntimeError>,
}

#[derive(Debug)]
pub struct AuthGameThreadReport {
    pub initialization: Result<AuthInitializationReport, AuthInitializationError>,
    pub runtime_error: Option<AuthRuntimeError>,
    pub release: AuthReleaseReport,
}

#[derive(Debug)]
pub struct AuthRuntimePaths {
    setup: PathBuf,
...
impl AuthRuntimePaths {
    pub fn from_runtime_directory(directory: impl AsRef<Path>) -> Self {
...
#[derive(Debug)]
pub struct AuthNetworkInitialization {
    pub allowed_clients_error: Option<io::Error>,
}
...
pub use crate::access::dbcontext::AuthDbContext;

pub struct CGame<Handler> {
    db: AuthDbContext,
...
{
    pub fn new(config: ConfigReader, message_handler: Handler) -> Self {
    ...
    pub const fn message_handler(&self) -> &Handler {
        &self.message_handler
    }

    pub fn message_handler_mut(&mut self) -> &mut Handler {
    ...
    /// создаёт результата.
    pub fn push_db_quest(&self, quest: DbQuest) -> bool {
        self.db.push_quest(quest)
    }

    pub fn db_quest_count(&self) -> u32 {
        self.db.quest_count()
    }

    pub fn pop_db_quest_wait(&self) -> DbQuest {
        self.db.quests.pop_front_wait()
    }

    pub fn push_db_result(&self, result: DbResult) {
        self.db.push_result(result);
    }

    pub fn db_result_count(&self) -> u32 {
        self.db.results.size()
    }

    pub fn pop_db_result_wait(&self) -> DbResult {
    ...
    pub fn replace_client_forbid_patterns(&mut self, patterns: Vec<[u8; 4]>) {
        self.db
            .client_ip_forbider
            .write()
    ...
    pub fn start_auth_database_workers(
        &mut self,
    ...
    pub fn release_auth_database_workers(&mut self) -> Vec<AuthDatabaseWorkerJoinError> {
        self.db_workers.stop_and_join()
    }

    pub fn pop_auth_database_notice(&self) -> Option<AuthDatabaseNotice> {
    ...
    pub async fn init_auth_network(
        &mut self,
    ...
    pub fn auth_local_ip(&self) -> Option<&[u8]> {
        self.net_server_auth
            .as_ref()
            .map(CMyNetServerAuth::local_ip)
    }

    pub fn auth_local_ipv4_word(&self) -> Option<u32> {
        self.net_server_auth
            .as_ref()
            .map(CMyNetServerAuth::local_ipv4_word)
    }

    pub fn process_message(&mut self) -> Result<i32, DispatchError> {
    ...
    pub fn process_database_results(&mut self) -> Result<i32, AuthRuntimeError> {
        let sender = self
            .net_server_auth
            .as_ref()
    ...
    pub async fn run_network_turn(&mut self) -> Result<AuthRuntimeStep, AuthRuntimeError> {
    ...
    pub async fn release_auth_network(&mut self) -> Result<(), AuthRuntimeError> {
    ...
    pub async fn release_auth_runtime(&mut self) -> AuthReleaseReport {
    ...
    pub fn active_network_tasks(&self) -> usize {
    ...
    fn spawn_io_actions(
        ...
        commands: nebokrai_shared::network::ServerCommandHandle,
    ) {
...
impl CGame<AuthMessageHandlers> {
    pub fn configured(config: ConfigReader, allowed_patterns: Vec<[u8; 4]>) -> Self {
    ...
    pub async fn initialize_auth_runtime(
        &mut self,
    ...
    pub async fn run_main_loop_turn(&mut self) -> Result<AuthRuntimeStep, AuthRuntimeError> {
    ...
    pub async fn init_configured_auth_network(
        &mut self,
        allowed_clients_path: impl AsRef<Path>,
    ) -> Result<(), AuthRuntimeError> {
        let network_config = self.db.config().auth_network_config();
    ...
    pub fn reload_config_and_login_filter(
        &mut self,
        setup_path: impl AsRef<Path>,
        allowed_clients_path: impl AsRef<Path>,
...
fn send_auth_result(
    sender: &nebokrai_shared::network::ServerCommandHandle,
...
fn add_legacy_string(message: &mut nebokrai_shared::network::CBaseMessage, value: &[u8]) {
    let end = value
        .iter()
