...
pub const VALID_CODE_BITMAP_LEN: usize = 0x70B6;
...
pub struct CValidCode {
    valid_code: Vec<u8>,
    bitmap: Box<[u8; VALID_CODE_BITMAP_LEN]>,
}

impl CValidCode {
    pub fn generate(runtime_directory: &Path) -> Result<Self, ValidCodeError> {
    ...
    pub fn valid_code(&self) -> &[u8] {
        &self.valid_code
    }

    pub fn bitmap(&self) -> &[u8; VALID_CODE_BITMAP_LEN] {
        &self.bitmap
    }
}
