...
//! нулями, но отдельно отмечаются, чтобы reload не удалял чужое событие.
//!
//! `Initialize` читает weekly setup, funds, country names и nation regions,
//! регистрируя доступные calendar events в исходном порядке. Фазы signup/end
//! сохраняют state, broadcasts, localized logs и перенос девяти timers на
//! следующую неделю только после успешного region lookup.
//!
//! Morale обновляет пять slots, но рассылает только countries 1..=4.
//! Exploit либо отправляется online player, либо с wrapping добавляется в
//! состояние после единственной DB-подгрузки. War-time и fail notices сохраняют
//! исходные region routes и `0x7FA04`; внутренние stack buffers ограничены без
//! изменения нормальных C-строк.
...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct FourNationWarFund {
    pub morale: i32,
    pub money: i32,
}

#[derive(Clone, Copy, Debug)]
pub struct FourNationWarCallbacks<Callback> {
    pub sign_up_start: Callback,
    pub sign_up_end: Callback,
    pub war_start: Callback,
    pub war_end: Callback,
    pub war_end_info: Callback,
    pub enter_start: Callback,
    pub enter_end: Callback,
    pub refresh_region: Callback,
    pub clear_war: Callback,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum FourNationWarCallbackKind {
    SignUpStart,
    SignUpEnd,
    WarStart,
    WarEnd,
...
impl<Callback: PartialEq> FourNationWarCallbacks<Callback> {
    pub fn kind(&self, callback: &Callback) -> Option<FourNationWarCallbackKind> {
        [
...
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct FourNationWarLoadReport {
    pub setup_resource_found: bool,
    pub nation_resource_found: bool,
    pub setup_rows_read: u32,
    pub setup_rows_accepted: u32,
    pub setup_rows_ignored_order: u32,
    pub funds_loaded: u32,
    pub rects_loaded: u32,
    pub rects_ignored_country: u32,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum FourNationWarLoadError {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct FourNationWarRegionIndexBlock {
    pub index: i32,
    pub setup_count: usize,
}

pub trait FourNationWarCallbackContext {
...
#[derive(Debug)]
pub enum FourNationWarReloadDisposition {
    BlockedActive {
        index: i32,
        kill_requests: u32,
        killed_events: u32,
...
#[derive(Debug)]
pub struct FourNationWarReloadReport {
    pub previous_setups: usize,
    pub kill_requests: u32,
    pub killed_events: u32,
    pub unregistered_events: u32,
    pub load: Result<FourNationWarLoadReport, FourNationWarLoadError>,
}
...
pub trait FourNationWarResultContext {
...
pub trait FourNationExploitContext {
...
pub trait FourNationCountryFailContext {
...
pub enum FourNationExploitLoadedDisposition {
    PlayerMissing,
...
#[derive(Debug, Eq, PartialEq)]
pub struct FourNationExploitLoadedReport {
    pub player_id: i32,
    pub increment: i32,
    pub disposition: FourNationExploitLoadedDisposition,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum FourNationSignUpDisposition {
    CountryIgnored,
    ValidCountryNoExternalEffect,
}

#[derive(Debug, Eq, PartialEq)]
pub enum FourNationWarTimeDisposition {
    CountryIgnored,
    RouteRejected {
        region_id: i32,
        map_id: i32,
...
#[derive(Debug, Eq, PartialEq)]
pub struct FourNationWarTimeReport {
    pub player_id: i32,
    pub war_time: u32,
    pub country: i32,
    pub disposition: FourNationWarTimeDisposition,
}

#[derive(Debug, Eq, PartialEq)]
pub struct FourNationCountryFailReport {
    pub country: i32,
    pub failed_country: i32,
    pub string_id: &'static [u8],
    pub country_name: Vec<u8>,
    pub failed_country_name: Option<Vec<u8>>,
    pub text: Vec<u8>,
    pub delivery: Result<i32, SendMessageError>,
}

#[derive(Debug, Eq, PartialEq)]
pub enum FourNationMoralePublicationDisposition {
    RouteRejected,
    Sent {
        wire: Vec<u8>,
        delivery: Result<i32, SendMessageError>,
...
#[derive(Debug, Eq, PartialEq)]
pub struct FourNationMoralePublication {
    pub country: u8,
    pub region_id: i32,
    pub map_id: i32,
    pub disposition: FourNationMoralePublicationDisposition,
}

#[derive(Debug, Eq, PartialEq)]
pub struct FourNationWarResultReport {
    pub previous_morale: [i32; FOUR_NATION_RECT_COUNT as usize],
    pub applied_morale: [i32; FOUR_NATION_RECT_COUNT as usize],
    pub values_complete: [bool; FOUR_NATION_RECT_COUNT as usize],
    pub publications: Vec<FourNationMoralePublication>,
}
...
impl CFourNationWarSys {
    pub fn on_refresh_region<SendAll, Delivery>(region_id: i32, send_all: &mut SendAll) -> Delivery
    where
        SendAll: FnMut(&CMessage) -> Delivery,
    {
    ...
    pub fn on_clear_war<SendAll, Delivery>(region_id: i32, send_all: &mut SendAll) -> Delivery
    where
        SendAll: FnMut(&CMessage) -> Delivery,
    {
    ...
    pub fn request_war_result_from_gs<SendAll, Delivery>(
        region_id: i32,
        send_all: &mut SendAll,
    ) -> Delivery
    where
    ...
    /// отсутствие завершает bool-owner после регистрации таймеров принятого
    /// префикса.
    pub fn initialize<Callback: Copy, NationSource, Log>(
        &mut self,
        ...
        Log: FnMut(&[u8]),
    {
        ...
        let Some(nation_source) = nation_source(first_region_id) else {
            add_log_text(
                format!("file 'regions/{first_region_id}.nation' can't found!").as_bytes(),
            );
    ...
    pub fn reload<Callback: Copy, NationSource, Log>(
        &mut self,
        country_names: [Vec<u8>; FOUR_NATION_RECT_COUNT as usize],
        source: Option<&[u8]>,
        now: TagTime,
    ...
    pub fn funds(&self) -> &[FourNationWarFund] {
        &self.funds
    }
    ...
    pub fn war_region_id_by_time(&self, index: i32) -> Result<i32, FourNationWarRegionIndexBlock> {
        if self.setups.is_empty() {
            return Ok(0);
        }
        let setup_count = self.setups.len();
        let index = usize::try_from(index)
            .map_err(|_| FourNationWarRegionIndexBlock { index, setup_count })?;
        self.setups
            .get(index)
            .map(|setup| setup.region_id)
    ...
    pub fn on_war_start<Context: FourNationWarCallbackContext + ?Sized>(
        &mut self,
        index: i32,
        context: &mut Context,
    ...
    pub fn on_enter_start<Context: FourNationWarCallbackContext + ?Sized>(
        &mut self,
        index: i32,
        context: &mut Context,
    ) -> Result<(), FourNationWarRegionIndexBlock> {
    ...
    pub fn on_enter_end<Context: FourNationWarCallbackContext + ?Sized>(
        &self,
        index: i32,
        context: &mut Context,
    ) -> Result<(), FourNationWarRegionIndexBlock> {
    ...
    pub fn on_sign_up_war_start<Context: FourNationWarCallbackContext + ?Sized>(
        &mut self,
        index: i32,
        context: &mut Context,
        ...
        let (region_id, start_time) = {
            let setup =
                self.setups
                    .get_mut(setup_index)
                    .ok_or(FourNationWarCalendarBlock::RegionIndex(
        ...
        let milliseconds =
            ((i32::from(difference.minute) * 60) + i32::from(difference.second)) * 1000;
    ...
    pub fn on_sign_up_war_end<Context: FourNationWarCallbackContext + ?Sized>(
        &self,
        index: i32,
        context: &mut Context,
    ) -> Result<(), FourNationWarRegionIndexBlock> {
    ...
    pub fn on_war_end_info<Context: FourNationWarCallbackContext + ?Sized>(
        &self,
        index: i32,
        context: &mut Context,
    ) -> Result<(), FourNationWarRegionIndexBlock> {
        ...
        let setup = self
            .setups
            .get(
                usize::try_from(index).map_err(|_| FourNationWarRegionIndexBlock {
                    index,
                    setup_count: self.setups.len(),
                })?,
            )
            .ok_or(FourNationWarRegionIndexBlock {
                index,
        ...
        if setup.region_id == 0
            || setup.region_state != 3
            || !current_time.legacy_lt(setup.end_time)
        {
            return Ok(());
        }
        let Some(region_name) = context.region_name(setup.region_id) else {
        ...
        let difference = setup
            .end_time
            .get_time_difference(current_time)
            .map_err(|_| FourNationWarRegionIndexBlock {
                index,
                setup_count: self.setups.len(),
            })?;
        let milliseconds =
    ...
    pub fn on_war_end<Callback: Copy, Context: FourNationWarCallbackContext + ?Sized>(
        &mut self,
        index: i32,
        timer: &mut CTimer<Callback>,
        callbacks: FourNationWarCallbacks<Callback>,
        ...
        if self.setups.get(setup_index).is_none() {
            return Err(FourNationWarCalendarBlock::RegionIndex(
                FourNationWarRegionIndexBlock { index, setup_count },
            ));
        }
    ...
    pub const fn one_country_sign_up(country: i32) -> FourNationSignUpDisposition {
    ...
    pub fn send_player_war_time_to_game_server<Context: FourNationWarResultContext + ?Sized>(
        &mut self,
        player_id: i32,
        war_time: u32,
        country: i32,
    ...
    pub fn one_country_fail<Context: FourNationCountryFailContext + ?Sized>(
        &self,
        country: i32,
        failed_country: i32,
        context: &mut Context,
        ...
        let (string_id, failed_country_name, formatted) = if country == failed_country {
            ...
            let formatted = context.format_world_string(string_id, &[&country_name, &failed_name]);
    ...
    pub fn push_setup(&mut self, setup: FourNationWarSetup) {
        self.setups.push(setup);
    }

    pub fn setups(&self) -> &[FourNationWarSetup] {
        &self.setups
    }

    pub fn rects(&self) -> &[FourNationRect; FOUR_NATION_RECT_COUNT as usize] {
        &self.rects
    }

    pub const fn morale(&self) -> &[i32; FOUR_NATION_RECT_COUNT as usize] {
        &self.morale
    }

    pub fn set_rect(&mut self, country: usize, rect: FourNationRect) -> bool {
    ...
    pub fn convert_loaded_morale_to_exploit<Context: FourNationExploitContext + ?Sized>(
        &mut self,
        player_id: i32,
        increment: i32,
        context: &mut Context,
    ...
    pub fn receive_result_from_game_server<Context: FourNationWarResultContext + ?Sized>(
        &mut self,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum FourNationWarSerializationBlock {
...
fn read_weekly_setup<'a>(
    ...
    let _ = start_time.add_day(if day_delta < 0 {
        day_delta + 7
    } else {
        day_delta
    })?;
