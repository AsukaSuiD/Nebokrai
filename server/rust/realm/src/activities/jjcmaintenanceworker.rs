...
use super::rsjjcsys::{RsJjcSysNotice, RsJjcSysOwner, TiberiusRsJjcSys};
use crate::persistence::rssetup::WorldDatabaseSettings;

#[derive(Debug)]
pub enum WorldJjcWeekClearWorkerEvent {
    WeekStarted,
    WeekFinished {
        returned_true: bool,
...
pub struct WorldJjcWeekClearWorker {
...
impl WorldJjcWeekClearWorker {
    ...
    pub fn dispatch(&self, runtime: Handle) -> Result<(), io::Error> {
 ...
 /// join оставляет внешний blocking/timing контракт исходного caller-а.
    pub fn clear_season(&self, runtime: Handle) -> bool {
        let settings = self.settings.clone();
    ...
    pub fn try_next_event(&self) -> Option<WorldJjcWeekClearWorkerEvent> {
