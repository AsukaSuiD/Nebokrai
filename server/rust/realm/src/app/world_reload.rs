...
//! (см. [`crate::app::world_game`]). Статус тела `reload` — PARTIAL: покрытие
//! статических `Load*`-внутренностей машинной досверкой не проверялось.
//!
//! Диспетчер `reload_profiles` и таблица [`WORLD_RELOAD_ACTIONS`] досверены
//! по машинному коду; закрытые расхождения: DIFF-3 (обработка любого
//! low-бита выполняет `low &= ~mask; high = 0` — первый обработанный
//! low-профиль гасит все pending high-флаги), DIFF-4 (ChangeBodyConf, lo
//! 0x80000000; SynthesisList, lo 0x50000000; Allthing, lo 0x100 — машинный
//! `ReLoad` вызывается с `(send=1, resources=1)`), DIFF-5 (Broadcast: при
//! отсутствии `setup/sysboardcast.ini` `return 0` без записи conf-log;
//! машинный MessageBox — единственный UI-side-effect ветки, без
//! wire/DB-эффекта — в headless-сервисе осознанно не воспроизводится; при
//! успехе лог `Load System Broadcast List...OK!`, result остаётся 0) и
//! DIFF-6 (conf-log пишет профиль машинным написанием `godsBattle`).
//!
//! DIFF-1/DIFF-2 сознательно оставлены как есть: расхождение касается только
//! внутреннего `legacy_result`, значение которого ни один машинный caller не
//! читает, — наблюдаемости нет.
//!
