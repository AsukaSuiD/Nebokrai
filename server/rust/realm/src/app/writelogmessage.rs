...
//! уже закрыты [`WorldGameView`]. Ни одна ветвь не исполняет DB-запрос: SQL
//! собирает worker (`persistence/writelogworker.rs`) при извлечении команды
//! из того же FIFO. Live-публикации владельцев `CIncrementLog` и `CAuctionLog`
//! приходят параметрами от dispatcher-а процесса, как и operator-log callback.
//!
...
use crate::auction::auctionlog::{AuctionLogNode, AuctionLogSystemTime, CAuctionLog};
use crate::billing::incrementlog::CIncrementLog;
use crate::persistence::writelog::{
    ...
    WorldFairyLogEvent, WorldFairyLogWrite, WorldGoodsCraftLogEvent, WorldGoodsCraftLogWrite,
    WorldGoodsLogWrite, WorldGoodsTradeLogWrite, WorldIncrementLogWrite, WorldPlainLogWrite,
    WorldPlayerProgressLogEvent, WorldPlayerProgressLogWrite, WorldPlayerRelationLogEvent,
    WorldPlayerRelationLogWrite, WorldWriteLogCommand,
};
...
#[derive(Debug)]
pub enum WorldIncrementLogMessageOutcome {
    Queued {
...
#[derive(Debug)]
pub struct WorldCarriageLogMessageOutcome {
    pub record: WorldCarriageLogWrite,
    pub payload_complete: [bool; 6],
    pub queue_length_after: usize,
}

#[derive(Debug)]
pub struct WorldPlainLogMessageOutcome {
    pub record: WorldPlainLogWrite,
    pub player_found: bool,
    pub payload_complete: [bool; 3],
    pub queue_length_after: usize,
}

#[derive(Debug)]
pub struct WorldCiqingLogMessageOutcome {
    pub record: WorldCiqingLogWrite,
    pub payload_complete: [bool; 5],
    pub queue_length_after: usize,
}

#[derive(Debug)]
pub enum WorldFairyPayloadCompleteness {
    Grow([bool; 4]),
    Take([bool; 6]),
    Implantation([bool; 5]),
    Incubate([bool; 2]),
...
#[derive(Debug)]
pub enum WorldFairyLogMessageOutcome {
    Queued {
...
#[derive(Debug)]
pub enum WorldAuctionLogMessageOutcome {
    Queued {
...
#[derive(Debug)]
pub enum WorldAuctionSalePayloadCompleteness {
    Oper([bool; 7]),
    Cancel([bool; 2]),
    Receive([bool; 3]),
}
...
#[derive(Debug)]
pub struct WorldAuctionSaleLogMessageOutcome {
    pub write: WorldAuctionSaleLogWrite,
    pub payload_complete: WorldAuctionSalePayloadCompleteness,
    pub queue_length_after: usize,
}

#[derive(Debug)]
pub enum WorldPlayerProgressPayloadCompleteness {
    Level([bool; 7]),
    Experience([bool; 6]),
    Died([bool; 4]),
}
...
#[derive(Debug)]
pub struct WorldPlayerProgressLogMessageOutcome {
    pub write: WorldPlayerProgressLogWrite,
    pub player_found: bool,
    pub payload_complete: WorldPlayerProgressPayloadCompleteness,
    pub queue_length_after: usize,
}

#[derive(Debug)]
pub struct WorldPlayerRelationLogMessageOutcome {
    pub write: WorldPlayerRelationLogWrite,
    pub players_found: [bool; 2],
    pub payload_complete: [bool; 6],
    pub queue_length_after: usize,
}

#[derive(Debug)]
pub struct WorldGoodsTradeLogMessageOutcome {
    pub write: WorldGoodsTradeLogWrite,
    pub players_found: [bool; 2],
    pub payload_complete: [bool; 17],
    pub queue_length_after: usize,
}

#[derive(Debug)]
pub struct WorldGoodsLogMessageOutcome {
    pub write: WorldGoodsLogWrite,
    pub player_found: bool,
    pub payload_complete: [bool; 13],
    pub queue_length_after: usize,
}

#[derive(Debug)]
pub enum WorldGoodsCraftPayloadCompleteness {
    Upgrade([bool; 15]),
    GemExchange([bool; 9]),
    JewelryMade([bool; 11]),
}
...
#[derive(Debug)]
pub struct WorldGoodsCraftLogMessageOutcome {
    pub write: WorldGoodsCraftLogWrite,
    pub player_found: bool,
    pub payload_complete: WorldGoodsCraftPayloadCompleteness,
    pub queue_length_after: usize,
}

#[derive(Debug)]
pub enum WorldChatPayloadCompleteness {
    FixedReceiver([bool; 6]),
    PrivateReceiver([bool; 7]),
}

#[derive(Debug)]
pub enum WorldChatLogMessageOutcome {
    Queued {
...
#[derive(Debug)]
pub struct WorldChangeMapLogMessageOutcome {
    pub write: WorldChangeMapLogWrite,
    pub player_found: bool,
    pub payload_complete: [bool; 10],
    pub queue_length_after: usize,
}

#[derive(Debug)]
pub enum WorldWriteLogMessageOutcome {
...
pub enum WorldWriteLogMessageDispatch {
...
fn on_goods_log_message(game: &dyn WorldGameView, mut message: CMessage) -> WorldGoodsLogMessageOutcome {
    let log_type = message.base_mut().get_char();
...
fn on_chat_log_message(game: &dyn WorldGameView, mut message: CMessage) -> WorldChatLogMessageOutcome {
...
fn on_ciqing_log_message(game: &dyn WorldGameView, mut message: CMessage) -> WorldCiqingLogMessageOutcome {
    let player_id = message.base_mut().get_long();
...
fn on_plain_log_message(game: &dyn WorldGameView, mut message: CMessage) -> WorldPlainLogMessageOutcome {
    let player_id = message.base_mut().get_long();
