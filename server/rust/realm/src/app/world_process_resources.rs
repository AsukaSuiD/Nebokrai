...
//! перенесённый из `src/worldserver/worldserver/runtime.rs` в Realm `app/`
//! волной C5-D.
//!
//! Источник контракта — те же свидетельства, что у [`crate::app::world_reload`]
//! (диспетчер ReLoad там). Это рантайм-проводка: форматы самих ресурсов
//! подтверждены модулями `nebokrai_shared::resources` и `content/`; сам перенос
//! новой машинной сверки не требует, тела перенесены буквально.
//!
//! transition: посадка этого держателя в `app/` переходная — целевое место
//! по [карте владельцев] `content/`, так как это состав reloadable-контента,
//! а не сетевого края. Пересадка отложена до отдельной волны вместе с
//! resource-семьёй `world_reload`.
//!
