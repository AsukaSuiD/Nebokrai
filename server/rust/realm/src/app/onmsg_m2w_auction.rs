...
pub struct WorldMiscAuctionMessageOutcome {
    pub request_type: i32,
    pub response_type: Option<i32>,
    pub route_map_id: Option<i32>,
    pub wire: Vec<u8>,
    pub delivery: Option<Result<i32, SendMessageError>>,
    pub queue_operation: Option<OperatorType>,
    pub unserialize_block: Option<GoodsNodeUnserializeError>,
    pub serialize_block: Option<GoodsNodeSerializeError>,
}

/// Результат M2W auction-dispatcher-а; все literal case текущего owner-а уже
/// обработаны, а default switch является no-op.
pub enum WorldMiscAuctionMessageDispatch {
    Handled(WorldMiscAuctionMessageOutcome),
    Pending(CMessage),
}

pub fn on_msg_m2w_auction(
