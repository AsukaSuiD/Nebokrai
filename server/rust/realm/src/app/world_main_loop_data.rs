//! Данные хода `CGame::MainLoop` WorldServer без самой игры (hub-data
//! уровень): stage-отчёты (timer/AI/session-factory/faction-war/db-misc/
//! net-session/minute/bai-tan-jjc/save/tail/ping), конфигурация хода и
//! связка `StateOwners`/`Owners`/`Callbacks`/`Block`/`Report`.
...
//! `route_loaded_player`) держат конкретную декларацию на самой границе
//! процесса. Глубокие timer-контексты (`WorldTimerHandler` и effect-адаптеры
...
//! (`WorldJjcRuntimeContext`/`WorldLeiTingRuntimeContext`) живут в
//! [`crate::app::world_main_loop_contexts`] вместе с impl-ами
//! process-контекстов (orphan-rule).
