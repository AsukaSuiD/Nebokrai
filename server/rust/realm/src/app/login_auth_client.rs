...
//! наследование `CClient` выражено композицией над общими операциями shared
//! network.
//!
//! Живой receive-контракт — поток envelope
//! `[total_len, crc(total_len), crc(message), message]`: length CRC после
//! накопления 12 bytes, второй CRC по нормализованному внутреннему сообщению,
//! неполный хвост сохраняется, доказанная ошибка очищает вход без отката уже
//! опубликованных кадров. Явный `Close` только закрывал общий client и
//! сбрасывал connect-флаг, synthetic сообщения не создавая; transport-close
//! публикует пустое Login-сообщение `0xCF301`. Два пути разведены в `close` и
//! `handle_transport_close`.
//!
//! Успешный `ReconnectAS` передавал новый client указателем внутри synthetic
//! `0xCF302` — внутрипроцессный handoff, не выходивший в wire: Rust заменяет
//! его типизированным `AuthClientEvent::Reconnected` в той же FIFO с тем же
//! порядком относительно принятых сообщений.
//!
...
//! safe Rust локально отвергает эти границы отдельными ошибками, не выдавая
//! выбор за контракт исходного процесса. Неизвестный Auth opcode `0x10F101`
//! относится к доменному обработчику, а не к этому framing-owner.
//!
//! Доказательства: docs/reconstruction/realm-services.md#login-исходящее-auth-соединение
