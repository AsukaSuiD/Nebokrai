...
//! Generic-параметр `PlayerT` заменяет прямую ссылку на игрока: реализация
//! `TiberiusRsPlayer` связывает его с `CPlayer` (`characters/player`), а
//! block-типы загрузки — associated types goods-владельца. Generic
...
use crate::persistence::row::{get_integer as read_ado_integer, get_value};
...
pub fn decode_honor_ranks_blob(
    rank_type: HonorRanksType,
    blob: Option<&[u8]>,
...
pub struct TiberiusRsPlayer {
    settings: WorldDatabaseSettings,
    notices: VecDeque<RsPlayerNotice>,
}

pub struct TiberiusPlayerLoadData<'owner, J, G, WeekDay> {
    pub player_owner: &'owner mut TiberiusRsPlayer,
    pub active_transaction: Option<&'owner mut WorldTdsClient>,
    pub thing_setup: &'owner CThingSetup,
    pub get_week_day: &'owner mut WeekDay,
    pub jjc_owner: &'owner mut J,
    pub goods_owner: &'owner mut G,
    pub goods_registry: &'owner GoodsBasePropertiesRegistry,
    pub changed_goods_indices: &'owner BTreeMap<u32, u32>,
    pub dakong_addon_types: &'owner BTreeSet<i32>,
}

#[derive(Debug)]
pub enum TiberiusPlayerLoadDataBlock {
    Reconstruction(PlayerLoadBlock<GoodsLoadedAddonBlock, PlayerLoadedGoodsInsertBlock>),
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PlayerAbilityScalarField {
    Id,
    SaveTime,
    Name,
    RegionId,
...
pub enum PlayerAbilityScalarValue<'a> {
    I4(i32),
    Ui4(u32),
    Ui2(u16),
    Ui1(u8),
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct PlayerAbilityScalarAssignment<'a> {
    pub field: PlayerAbilityScalarField,
    pub value: PlayerAbilityScalarValue<'a>,
}

pub fn player_ability_scalar_assignments<'a>(
...
pub fn encode_player_quest_data(snapshot: &PlayerQuestSaveSnapshot<'_>) -> Vec<u8> {
...
pub fn player_ability_save_scalar_assignments<'a>(
...
pub trait PlayerAbilityFieldSink {
    type Error;
...
pub fn save_hot_key_field<S: PlayerAbilityFieldSink>(
    hot_keys: &[u32; 24],
    sink: &mut S,
) -> Result<(), S::Error> {
...
pub fn save_skill_field<S: PlayerAbilityFieldSink>(
...
pub fn save_script_flag<S: PlayerAbilityFieldSink>(
...
pub fn save_state_field<S: PlayerAbilityFieldSink>(
    ex_states: &[u8],
    sink: &mut S,
) -> Result<(), S::Error> {
...
pub fn save_friend_field<S: PlayerAbilityFieldSink>(
...
pub fn save_ci_qing_field<S: PlayerAbilityFieldSink>(
...
pub fn save_thing_field<S: PlayerAbilityFieldSink>(
    things: &[PlayerThing],
    sink: &mut S,
) -> Result<(), S::Error> {
...
pub fn load_skill_field(blob: &[u8]) -> Vec<PlayerAbilitySkill> {
    blob.chunks_exact(4)
...
pub fn load_state_field(blob: &[u8]) -> Option<Vec<u8>> {
    (!blob.is_empty()).then(|| blob.to_vec())
}

pub fn load_friend_field(
    blob: &[u8],
...
pub fn load_ci_qing_field(blob: &[u8]) -> BTreeSet<u32> {
    blob.chunks_exact(4)
        .map(|entry| u32::from_le_bytes(entry.try_into().expect("полный tattoo ID")))
        .collect()
}

pub fn load_thing_field(blob: &[u8]) -> Vec<PlayerThing> {
    blob.chunks_exact(8)
...
pub fn load_quest_data(blob: &[u8]) -> Vec<PlayerQuestSaveEntry> {
    blob.chunks_exact(3)
...
pub fn materialize_player_ability_scalar_row(
    row: &Row,
    player: &mut CPlayer,
...
pub fn materialize_player_ability_binary_row(
    row: &Row,
    player: &mut CPlayer,
    thing_setup: &CThingSetup,
...
impl TiberiusRsPlayer {
    ...
    pub async fn load_player_ability_row(
        &mut self,
        player: &mut CPlayer,
        active_transaction: Option<&mut WorldTdsClient>,
        thing_setup: &CThingSetup,
    ...
    pub async fn load_player_quest_data(
        &mut self,
        player: &mut CPlayer,
        active_transaction: Option<&mut WorldTdsClient>,
...
impl crate::app::world_game_view::WorldRenameDbView for TiberiusRsPlayer {
    fn is_name_exist<'a>(
        &'a mut self,
        player_name: &'a [u8],
...
impl crate::app::world_game_view::WorldExploitDbView for TiberiusRsPlayer {
...
impl crate::app::world_game_view::WorldDeleteRoleDbView for TiberiusRsPlayer {
...
impl crate::app::world_game_view::WorldCreateRoleDbView for TiberiusRsPlayer {
...
impl crate::app::world_game_view::WorldPlayerSelectDbView for TiberiusRsPlayer {
