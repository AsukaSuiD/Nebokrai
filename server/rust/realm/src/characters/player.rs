...
use nebokrai_shared::resources::CDupliRegionSetup;
use nebokrai_shared::resources::{
    CPlayerList, CThingSetup, GlobePlayerPropertyCoefficients, GlobeSetupSnapshot,
    LeiTingDailyThing, LeiTingLocalTime, PlayerCreationPropertiesLookup, PlayerOriginEquipment,
};
use nebokrai_shared::values::CGuid;
use nebokrai_shared::values::{TagTime, TagTimeArithmeticBlock};

use crate::activities::rsjjcsys::{PlayerJjcDataSnapshot, RsJjcSysOwner};
use crate::app::world_message::{CMessage, SendMessageError};
use crate::content::countryparam::{CCountryParam, CountryRect};
use crate::content::cgoods::{CGoods, GoodsCodecError, GoodsDbSnapshotBlock};
use crate::content::cgoodsfactory::{
    GoodsOriginalNameIndex, create_goods, query_goods_id_by_original_name_bytes,
};
use crate::content::dbgoods::{DbGoodsOwner, PlayerGoodsFiledSnapshot};
use crate::content::goods::GAP_WEAPON_LEVEL;
use crate::content::goods::GoodsBasePropertiesRegistry;
use crate::content::goodslistener::{GoodsContainerTraversalSnapshot, TraversedGoods};
...
use crate::items::cequipmentcontainer::{CEquipmentContainer, EquipmentContainerCodecError};
use crate::items::cfairycontainer::CFairyContainer;
use crate::items::cjifen::CJiFen;
use crate::items::cseekgoodslistener::CSeekGoodsListener;
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PlayerMurderCounterUpdate {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PlayerMurderCounterReset {
    pub previous_kill_count: u32,
    pub previous_pk_count: u16,
}

pub use crate::characters::playerexploit::PlayerExploitUpdate;
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PlayerDefaultPropertyReport {
    pub selected_region_id: i32,
    pub tile_x: i32,
    pub tile_y: i32,
    pub direction: i32,
    pub creation_property_key: u32,
    pub creation_property_inserted: bool,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PlayerDefaultPropertyBlock {
    RandomPosition(RegionRandomPositionBlock),
    Property(PlayerCodecError),
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PlayerOriginEquipmentOutcome {
    OccupationMismatch,
...
#[derive(Debug)]
pub enum PlayerOriginEquipmentBlock {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PlayerLoadedLeiTingResetReport {
    pub reset: bool,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PlayerHonorEliminateResetReport {
    pub day_reset: bool,
    pub week_reset: bool,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PlayerLoadedGoodsInsertBlock {
...
#[derive(Clone, Copy)]
pub struct PlayerPropertyCoefficients {
...
pub trait PlayerLoadDataOwner {
    type Block;

    fn load_player(
        &mut self,
        player: &mut CPlayer,
    ) -> impl std::future::Future<Output = Result<bool, Self::Block>>;
}

#[derive(Clone, Copy, Debug, PartialEq)]
pub struct PlayerLoadDataReport {
    pub base_max_rp: u16,
    pub player_property_key: u32,
    pub player_property_inserted: bool,
...
#[derive(Debug)]
pub enum PlayerLoadDataOutcome<LoadBlock> {
    ReturnedFalse,
...
pub struct PlayerOrganizingState {
    pub faction_id: i32,
    pub faction_logo_id: i32,
    pub faction_name: Vec<u8>,
    pub faction_title: Vec<u8>,
    ...
    pub force: i32,
    pub faction_contribute: bool,
    pub enemy_factions: BTreeSet<i32>,
    pub city_war_enemy_factions: BTreeSet<i32>,
    owned_regions: VecDeque<PlayerOwnedRegion>,
}

impl PlayerOrganizingState {
    pub fn clear_owned_regions(&mut self) {
        self.owned_regions.clear();
    }

    pub fn add_owned_region(&mut self, region_id: i32, region_type: u16) {
        if self
            .owned_regions
            .iter()
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PlayerOrganizingUpdateError {
...
pub trait PlayerOrganizingUpdater {
...
pub trait PlayerFactionInfoContext: PlayerOrganizingUpdater {
...
pub struct PlayerFactionInfoDelivery {
    pub game_server_id: i32,
    pub result: Result<i32, SendMessageError>,
}

#[derive(Debug, Eq, PartialEq)]
pub struct PlayerFactionInfoUpdateReport {
    pub player_id: i32,
    pub faction_id_after_initial_update: i32,
    pub faction_data_reset: bool,
    pub wire: Vec<u8>,
    pub delivery: PlayerFactionInfoDelivery,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PlayerFactionInfoUpdateBlock {
    InitialOrganizing(PlayerOrganizingUpdateError),
    Serialization(PlayerCodecError),
}

pub struct PlayerGoodsDbProjection {
...
pub enum PlayerDbProjectionBlock {
...
pub struct PlayerBaseWireSnapshot {
    pub id: i32,
    pub name: Vec<u8>,
    pub level: u8,
    pub occupation: u8,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PlayerEquipmentWireSnapshot {
    pub ids: [u32; 11],
    pub levels: [u8; 11],
}
...
pub struct PlayerDbProjection<'player> {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum PlayerCountryChangeDisposition {
    SameCountry,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct PlayerCountryChangeReport {
    pub requested_country: u8,
    pub legacy_result: i32,
    pub disposition: PlayerCountryChangeDisposition,
}

impl PlayerGoodsDbProjection {
    pub fn snapshot(&self, player_id: i32) -> PlayerGoodsFiledSnapshot<'_> {
...
impl CPlayer {
    pub fn reset_goods_for_db_load(&mut self) {
        self.packet.clear();
    ...
    pub fn loaded_goods_mut(&mut self, place: i32, position: u32) -> Option<&mut CGoods> {
        match place {
    ...
    pub fn equipment_wire_snapshot(
        &self,
    ...
    pub fn player_base_wire_snapshot(
        &self,
 ...
 /// `None`, а не придуманному нулю.
    pub fn with_clone_decode_constructor_state() -> Self {
 ...
 /// заменами process-global setup/game owners и platform callbacks.
    ...
    pub fn load_default_property<'region, FindRegion, Random, WeekDay, Timestamp>(
        &mut self,
        sex: u8,
        occupation: u8,
        country: u8,
 ...
 /// и modifier roll-ы остаются на общем legacy `random(bound)` callback-е.
    pub fn add_origin_equipment<Random>(
        &mut self,
    ...
    pub fn set_creation_service_defaults(&mut self) {
        self.base_property
    ...
    pub fn set_database_load_identity(&mut self, player_id: i32, account: &[u8]) {
        self.set_id(player_id);
        self.base_property.account.clear();
        self.base_property.account.extend_from_slice(account);
    }

    pub fn mark_largess_charged(&mut self) {
    ...
    pub const fn largess_depot_limit(&self) -> u32 {
        self.depot.get_goods_amount_limit()
    }

    pub fn add_largess_to_depot(
        &mut self,
        position: u32,
        goods: Box<CGoods>,
        registry: &GoodsBasePropertiesRegistry,
    ...
    pub fn increment_murder_counters(&mut self) -> PlayerMurderCounterUpdate {
    ...
    pub fn reset_murder_counters(&mut self) -> PlayerMurderCounterReset {
        let previous_pk_count = self.base_property.read_u16(BASE_PROPERTY_PK_COUNT_OFFSET);
        self.base_property.write_u16(BASE_PROPERTY_PK_COUNT_OFFSET, 0);
    ...
    pub fn add_exploit_wrapping(&mut self, increment: i32) -> PlayerExploitUpdate {
    ...
    pub fn faction_data_received(&self) -> bool {
        self.faction_data_received.get()
    }

    pub fn set_faction_data_received(&self, received: bool) {
        self.faction_data_received.set(received);
    }

    pub fn clear_uncreated_pets(&mut self) {
        self.uncreated_pets.clear();
    }

    pub fn faction_war_operator(&self) -> bool {
        self.faction_runtime.borrow().faction_war_operator
    }

    pub fn set_faction_war_operator(&self, enabled: bool) {
    ...
    pub fn get_net_exchange_id(&self) -> i32 {
 ...
 /// игровая смена имени по-прежнему должна проходить `set_validated_name`.
    ...
    pub fn set_validated_name(&mut self, name: &[u8]) {
        self.move_shape_base.set_name(name);
    }

    pub fn set_fairy_container_enabled(&mut self, enabled: bool) {
        self.base_property
            .write_u8(BASE_PROPERTY_FAIRY_ENABLED_OFFSET, u8::from(enabled));
    }

    pub fn set_foster_num(&mut self, value: u32) {
        self.base_property
            .write_u32(BASE_PROPERTY_FOSTER_NUM_OFFSET, value.min(5));
    }

    pub fn set_hatcher_num(&mut self, value: u32) {
        self.base_property
            .write_u32(BASE_PROPERTY_HATCHER_NUM_OFFSET, value.min(5));
    }

    pub fn apply_loaded_ability_scalars(
        &mut self,
    ...
    pub fn apply_loaded_hot_keys(&mut self, hot_keys: &[u32; 24]) {
    ...
    pub fn append_loaded_skills(&mut self, skills: &[PlayerAbilitySkill]) {
        self.new_skills
    ...
    pub fn apply_loaded_script_flag(&mut self, script: LoadedPlayerScriptFlag) {
    ...
    pub fn apply_loaded_ex_states(&mut self, ex_states: Vec<u8>) {
        debug_assert!(!ex_states.is_empty());
        self.move_shape_base.set_ex_states(&ex_states);
    }

    pub fn append_loaded_friends(&mut self, names: Vec<Vec<u8>>) {
    ...
    pub fn extend_loaded_ci_qing(&mut self, ids: BTreeSet<u32>) {
        self.ci_qing_ids.extend(ids);
    }

    pub fn apply_loaded_things(
        &mut self,
 ...
 /// с последующей полной записью трёх значимых байт `tagPlayerQuest`.
    pub fn add_quest_from_db(&mut self, quest_id: u16, complete: u8) {
    ...
    pub fn replace_silience_time(&mut self, silience_time: i32) -> i32 {
    ...
    pub const fn get_team_id(&self) -> i32 {
        self.team_id
    }
    ...
    pub fn get_occupation(&self) -> u8 {
        self.base_property
            .read_u8(BASE_PROPERTY_OCCUPATION_OFFSET)
    }

    pub fn get_appellation_id(&self) -> u32 {
        self.base_property
            .read_u32(BASE_PROPERTY_APPELLATION_OFFSET)
    }

    pub fn reset_loaded_lei_ting_if_needed(
        &mut self,
        local_day: u16,
    ...
    pub fn reset_honor_eliminate_num(
        &mut self,
    ...
    pub fn reset_honor_eliminate_info(&mut self, rank_mask: u32) {
        if rank_mask & 0x02 != 0 {
            self.base_property
                .write_u32(BASE_PROPERTY_WEEKS_HONOR_OFFSET, 0);
        }
    ...
    pub fn friend_name(&self, index: usize) -> Option<&[u8]> {
        self.friends.get(index).map(|friend| friend.name.as_slice())
    }

    pub fn set_friend_online(&mut self, index: usize, online: bool) -> bool {
    ...
    pub fn reset_selected_login_flags(&mut self) {
        self.faction_data_received.set(false);
        self.login = false;
    }

    pub fn set_player_organizing<U: PlayerOrganizingUpdater>(
        &self,
        updater: &mut U,
    ) -> Result<(), PlayerOrganizingUpdateError> {
 ...
 /// снимок и только затем публикует `0x7FE06`.
    pub fn update_faction_info<Context>(
        &self,
        context: &mut Context,
    ...
    pub fn add_to_byte_array<U: PlayerOrganizingUpdater>(
        &mut self,
    ...
    pub fn add_to_byte_array_before_containers(
        &self,
        destination: &mut Vec<u8>,
        include_child: bool,
    ) -> Result<bool, PlayerCodecError> {
    ...
    pub fn decord_from_byte_array_before_containers(
        &mut self,
        source: &[u8],
        cursor: &mut usize,
        include_child: bool,
    ...
    pub fn add_containers_to_byte_array(
        &mut self,
        destination: &mut Vec<u8>,
        registry: &GoodsBasePropertiesRegistry,
    ) -> Result<bool, PlayerCodecError> {
    ...
    pub fn decord_containers_from_byte_array(
        &mut self,
        source: &[u8],
        cursor: &mut usize,
        registry: &GoodsBasePropertiesRegistry,
    ...
    pub fn add_to_byte_array_after_containers<U: PlayerOrganizingUpdater>(
        &mut self,
        destination: &mut Vec<u8>,
        updater: &mut U,
    ) -> Result<bool, PlayerCodecError> {
    ...
    pub fn decord_from_byte_array_after_containers(
        &mut self,
        source: &[u8],
        cursor: &mut usize,
    ) -> Result<bool, PlayerCodecError> {
    ...
    pub fn add_org_sys_to_byte_array<U: PlayerOrganizingUpdater>(
        &self,
        destination: &mut Vec<u8>,
        updater: &mut U,
    ) -> Result<bool, PlayerCodecError> {
    ...
    pub fn add_byte_array_lei_ting(
        &self,
        destination: &mut Vec<u8>,
    ) -> Result<(), PlayerCodecError> {
        destination.extend_from_slice(
    ...
    pub fn update_lei_ting<Clock: PlayerLeiTingClock>(
        &mut self,
        update_kind: u32,
    ...
    pub fn decode_byte_array_lei_ting(
        &mut self,
        source: &[u8],
        cursor: &mut usize,
    ) -> Result<(), PlayerCodecError> {
    ...
    pub fn add_quest_data_to_byte_array(
        &self,
        destination: &mut Vec<u8>,
    ) -> Result<bool, PlayerCodecError> {
    ...
    pub fn decord_quest_data_from_byte_array(
        &mut self,
        source: &[u8],
        cursor: &mut usize,
    ) -> Result<bool, PlayerCodecError> {
    ...
    pub async fn load_data<L: PlayerLoadDataOwner>(
        &mut self,
        loader: &mut L,
        player_list: &mut CPlayerList,
        globe_setup: &GlobeSetupSnapshot,
...
impl crate::activities::rsjjcsys::RsJjcSysPlayerTarget for CPlayer {
...
impl crate::characters::playerdataqueue::PlayerDataResetHonorEliminate for CPlayer {
    fn reset_honor_eliminate_info(&mut self, rank_mask: u32) {
        CPlayer::reset_honor_eliminate_info(self, rank_mask);
    }
}

impl crate::activities::leiting::LeiTingPlayerCodec for CPlayer {
...
impl crate::activities::jjcsystem::JjcPlayerView for CPlayer {
    fn get_level(&self) -> u8 {
        self.get_level()
    }
...
impl crate::characters::playerloadworker::WorldPlayerLoadFactory for CPlayer {
