...
use crate::auction::auctionnode::{CGoodsNode, GoodsNodeSerializeError, GoodsState};
use crate::auction::auctionroom::PlayerOptNode;
use nebokrai_shared::values::CGuid;

/// Неопределённая граница старого неинициализированного `m_btGoodsType`.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct AddAuctionItemMissingGoodsType {
    pub guid: CGuid,
}

/// Неопределённая type-граница `ClearRecond` после доказанных time/owner эффектов.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ClearRecondMissingGoodsType {
    pub guid: CGuid,
}

/// Неопределённая duplicate-граница вставки owned-узла в конечный список.
pub struct AddTerminalGoodsDuplicate {
    pub guid: CGuid,
    pub item: Box<CGoodsNode>,
}

/// Локальные safe-границы полного прохода `DoneDelList`.
pub enum DoneDelListError {
...
#[derive(Debug)]
pub enum TerminalGoodsDelivery {
...
pub struct UnityGoodsBuild {
    pub messages: Vec<CMessage>,
    pub blocked: Option<GoodsNodeSerializeError>,
}

/// Один удалённый при World reconciliation GameServer auction GUID.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct GameAuctionRemoval {
    pub owner_id: u32,
    pub guid: CGuid,
}
...
impl CGameAuctionRoom {
    ...
    pub fn add_item_to_auction_room(&mut self, item: &mut CGoodsNode) -> bool {
        let guid = item.guid();
    ...
    pub fn count_goods(&self) -> Vec<CGuid> {
    ...
    pub fn commit_unity_removal(&mut self, removal: GameAuctionRemoval) {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum AuctionPageBuildError {
...
pub struct CAuctionRoom<GoodsNode> {
...
impl CAuctionRoom<CGoodsNode> {
    ...
    pub fn add_item_to_auction_room(
        &mut self,
        item: Box<CGoodsNode>,
    ...
    pub fn query_goods_node_info(&self, guid: CGuid) -> Option<&CGoodsNode> {
    ...
    pub fn legacy_auction_goods_count(&self) -> u32 {
    ...
    pub fn done_del_list(&mut self) -> Result<(), DoneDelListError> {
        loop {
    ...
    pub fn done_sucessed_goods_list(
        &mut self,
    ...
    pub fn ai(&mut self, sender: Option<&dyn MessageSender>) -> AuctionAiOutcome {
        self.done_auction();
    ...
    pub fn add_byte_auction_self_to_client(
        &self,
    ...
    pub fn modify_player_seach_condition(&mut self, condition: PlayerOptNode) {
    ...
    pub fn add_byte_at_page_by_time(
        &self,
        player_id: u32,
        message: &mut CMessage,
    ) -> Result<(), AuctionPageBuildError> {
