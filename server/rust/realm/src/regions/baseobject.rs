...
use crate::characters::monster::CMonster;
use crate::characters::npc::CNpc;
...
pub use crate::regions::shapetypes::BaseObjectDecodeError;
...
pub enum BaseObjectChildCreateError {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum BaseObjectFactoryType {
    Region,
    Player,
    Npc,
    Monster,
...
pub enum BaseObjectFactoryObject {
    Region(CRegion),
    Player(CPlayer),
    Npc(CNpc),
    Monster(CMonster),
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum BaseObjectTreeAttachError {
    WouldCreateCycle,
}
...
pub type SharedBaseObjectFactoryObject = Rc<RefCell<BaseObjectTreeNode>>;
...
impl BaseObjectTreeNode {
    ...
    pub fn object(&self) -> &BaseObjectFactoryObject {
        &self.object
    }

    pub fn object_mut(&mut self) -> &mut BaseObjectFactoryObject {
        &mut self.object
    }

    pub fn father(&self) -> Option<SharedBaseObjectFactoryObject> {
        self.father.as_ref().and_then(std::rc::Weak::upgrade)
    }

    pub fn children(&self) -> Vec<SharedBaseObjectFactoryObject> {
        self.children.clone()
    }
 ...
 /// и, как EXE, оставляет child.father неизменённым.
    ...
    pub fn find_child_by_type_and_id(
        parent: &SharedBaseObjectFactoryObject,
        object_type: i32,
        object_id: i32,
    ) -> Option<SharedBaseObjectFactoryObject> {
    ...
    pub fn recursive_find_by_type_and_id(
    ...
    pub fn recursive_find_by_type_and_name(
        root: &SharedBaseObjectFactoryObject,
        object_type: i32,
        name: &[u8],
    ) -> Option<SharedBaseObjectFactoryObject> {
 ...
 /// list-order hit; отсутствие child остаётся no-op.
    pub fn delete_child_by_type_and_id(
        parent: &SharedBaseObjectFactoryObject,
        object_type: i32,
        object_id: i32,
    ) {
 ...
 /// возможные внешние safe aliases не превращаются в dangling pointers.
    pub fn delete_all_children_except(
        parent: &SharedBaseObjectFactoryObject,
 ...
 /// рекурсивный base owner соответствует их подтверждённому dispatch.
    pub fn dg_find_objects_by_type(
        parent: &SharedBaseObjectFactoryObject,
        object_type: i32,
        parameter: i32,
...
impl BaseObjectFactoryObject {
    pub const fn object_type(&self) -> i32 {
        match self {
...
pub fn create_base_object_child<Attach, Load>(
    object_type: i32,
    object_id: i32,
    name: Option<&[u8]>,
    graphics_id: i32,
...
impl CBaseObject {
 ...
 /// `None`/empty и destruction-order materialизованы отдельным owner-ом.
    ...
    pub const fn factory_type(&self) -> Option<BaseObjectFactoryType> {
