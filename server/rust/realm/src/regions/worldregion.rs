...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum WorldRegionEnterBlock {
    Setup(WorldRegionSetupSerializationBlock),
...
pub struct WorldRegionOwnerRelationReport {
    pub initial_faction_id: i32,
    pub faction_cleared: bool,
    pub union_cleared: bool,
    pub owned_city: Option<OwnedCityAddOutcome>,
    pub country: Option<u8>,
}

#[derive(Debug, Eq, PartialEq)]
pub enum WorldRegionOwnerRelationBlock {
    OwnedCity {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct WorldRegionLoadedCounts {
    pub monsters: i32,
    pub npcs: i32,
    pub base_failure: Option<WorldRegionBaseLoadFailure>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum WorldRegionBaseLoadFailure {
    MissingRegionResource,
    Region(RegionLoadError),
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub enum WorldRegionParamDecodeError {
    UnexpectedEnd {
        offset: usize,
        needed: usize,
        available: usize,
...
impl CWorldRegion {
    pub const fn with_constructor_region_base() -> Self {
        Self {
    ...
    pub const fn creation_region_base(&self) -> &CRegion {
        &self.region
    }
    ...
    pub fn generate_save_data(&self) -> RegionSaveSnapshot {
    ...
    pub const fn region_base(&self) -> &CRegion {
        &self.region
    }

    pub const fn region_base_mut(&mut self) -> &mut CRegion {
        &mut self.region
    }

 /// Возвращает три значения City Load guard без выбора реакции для
 /// constructor-uninitialized setup.
    pub fn city_load_base_guard(
        &self,
    ...
    pub fn set_region_identity(&mut self, region_id: i32, name: &[u8]) {
        self.region.set_id(region_id);
        self.region.set_name(name);
    }

    pub fn set_region_list_base_fields(
        &mut self,
        resource_id: u32,
        exp_scale: f32,
        country: u8,
    ...
    pub const fn set_world_region_list_fields(
        &mut self,
        war_region_type: i32,
        no_pk: bool,
        no_contribute: bool,
    ...
    pub fn load_npc_bytes<ResolveName>(
        &mut self,
        bytes: &[u8],
 ...
 /// двух исходных process-global counters.
    ...
    pub fn add_to_byte_array_for_proxy(
        &self,
 ...
 /// которые присваивал исходный `DecordRegionParamFromByteArray`.
    pub fn decord_region_param_from_byte_array(
        &mut self,
        source: &[u8],
        cursor: &mut usize,
        _include_child: bool,
 ...
 // байта за caller-pointer; реакция на короткий источник неизвестна.
    ...
    pub const fn set_param_from_gs(
        &mut self,
        current_tax_rate: i32,
        today_total_tax: u32,
        total_tax: u32,
    ...
    pub fn set_owned_city_org(&self, owned_faction_id: i32, owned_union_id: i32) {
        self.owned_city_organizing
            .set((owned_faction_id, owned_union_id));
    }

    pub fn get_owned_city_faction(&self) -> i32 {
        self.owned_city_organizing.get().0
    }

    pub fn get_owned_city_union(&self) -> i32 {
        self.owned_city_organizing.get().1
    }

    pub fn init_owner_relation(
        &mut self,
        organizing: &mut dyn crate::app::world_organizing_view::WorldRegionOwnerOrganizingView,
    ...
    pub fn new_region(&mut self) -> Result<i32, RegionLoadError> {
        self.region.new_region()
    }

    pub const fn decord_from_byte_array(
        &mut self,
        _source: &[u8],
        _cursor: &mut usize,
        _include_child: bool,
 ...
 /// Отсутствие ресурса обрабатывает caller до вызова и не меняет состояние.
    pub fn load_setup_bytes(&mut self, bytes: &[u8]) {
        let mut tokens = RegionSetupTokens::new(bytes);
    ...
    pub fn add_setup_to_byte_array(
        &self,
