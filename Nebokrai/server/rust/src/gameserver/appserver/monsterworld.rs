//! Каноническое поколенческое хранилище монстров одного `CServerRegion`.
//!
//! `slotmap` владеет `CMonster` и инвалидирует ключ только при настоящем
//! удалении сущности. `BTreeMap` остаётся тонким индексом legacy ID и задаёт
//! подтверждённый порядок обхода; порядок `SlotMap` никогда не используется в
//! ИИ, выборе цели, рассылке или сериализации. Прирученный питомец остаётся тем
//! же объектом `CMonster`, поэтому смена роли жизненного цикла не меняет identity.
//! Временное изъятие для вызова, одновременно меняющего членство в регионе,
//! сохраняет ячейку и поколение, а не изображает удаление и повторное создание.
...
impl MonsterWorld {
    ...
    pub(crate) fn restore(&mut self, taken: TakenMonster) {
        let slot = self
            ...
            .expect("временное изъятие сохраняет поколенческую ячейку");
