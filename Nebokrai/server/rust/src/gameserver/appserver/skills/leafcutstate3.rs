...
//! Object Begin (0x005EBE80) не требует user/sufferer: base Begin,
//! visual SetRun(1) → Update(0) → base visual tail, затем обнуление attack-count.
//! restart_leaf_cut_3_state переносит Begin(NULL, holder) по точному ключу:
...
pub(crate) fn restart_leaf_cut_3_state(
    game: &mut CGame,
    region_id: i32,
    ...
    let Some(state) = resolve_state_move_shape(game, region_id, holder)
        .and_then(|shape| shape.applied_state::<LeafCutState3>(key)).copied()
    else { return false };
    ...
    if let Some(state) = resolve_state_move_shape_mut(game, region_id, holder)
        .and_then(|shape| shape.applied_state_mut::<LeafCutState3>(key))
    {
