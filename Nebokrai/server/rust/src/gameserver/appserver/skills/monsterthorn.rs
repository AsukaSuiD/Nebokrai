...
fn finish_player<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, runtime: &mut Runtime) {
    restore_player(game, player_id);
    finish_summon_skill(game, player_id, ai, runtime, |ai, now_ms| ai.mark_skill_used(MONSTER_THORN_SKILL_ID, now_ms));
}
...
fn end_player_monster_thorn<Runtime: GameMainLoopRuntime>(
    ...
    let Some(dispatch) = ai.player_skill_state::<PlayerMonsterThornExecutionState>(MONSTER_THORN_SKILL_ID)
        .map(|state| state.kernel().dispatch())
    else { return false };
    if successful {
        finish_player(game, player_id, ai, runtime);
    } else {
        restore_player(game, player_id);
    }
    ai.finish_player_skill(dispatch, if successful {
