...
//! нулю при записи результата в `i32`. После каждого разрешённого попадания
//! источник получает `IncreaseRp(1, 0)`; износ оружия принадлежит единственному
//! `AfterUseSkill` владельца и здесь не повторяется.
