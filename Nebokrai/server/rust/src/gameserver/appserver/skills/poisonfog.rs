...
//! Reuse проверяется exact `CSkill::IsRestored`, отдельно от cast/lifetime.
//! Выбранная в Begin точка хранится вместе с kernel в зарегистрированном экземпляре навыка
//! и удаляется общим End; движение исходной цели её не пересчитывает.
...
pub(crate) fn execute_player_poison_fog<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, _ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    if game.player_skill_execution(player_id, POISON_FOG_SKILL_ID).is_none() { let started = runtime.now_milliseconds(); if !skill_is_restored(game.player_skill_last_used_ms(player_id, POISON_FOG_SKILL_ID), reuse, runtime.now_milliseconds()) { fail(game, player_id, 0x0d, b"GS0278", None); return terminal(QueuedSkillExecutionState::Rejected) } let Some((x, y)) = destination(game, region_id, dispatch) else { return terminal(QueuedSkillExecutionState::Rejected) }; let path = game.base_magic_path(region_id, source_x, source_y, x, y, None); if max_distance != 0 && path.len() > max_distance as usize { fail(game, player_id, 0x0b, b"GS0290", None); return terminal(QueuedSkillExecutionState::Rejected) } if path.iter().any(|cell| cell.2 == 2) { fail(game, player_id, 0x0f, b"GS0282", None); return terminal(QueuedSkillExecutionState::Rejected) } let weapon_ok = game.find_player(player_id).is_some_and(|player| weapon_valid(game, player)); if !weapon_ok { fail(game, player_id, 0x0e, b"GS0293", None); return terminal(QueuedSkillExecutionState::Rejected) } if mp_loss == 0 { return terminal(QueuedSkillExecutionState::Rejected) } if (mana.wrapping_sub(mp_loss) as i32) < 0 { fail(game, player_id, 7, b"GS0288", Some(mp_loss)); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(false); player.set_current_skill_id(Some(POISON_FOG_SKILL_ID)); } game.begin_poison_fog(player_id, SkillExecutionKernel::begin(dispatch, started), (x, y)); return terminal(QueuedSkillExecutionState::Begun); } else if game.player_skill_execution(player_id, POISON_FOG_SKILL_ID).is_none_or(|state| state.dispatch() != dispatch) { return terminal(QueuedSkillExecutionState::Rejected) }
    let Some((x, y)) = game.poison_fog_destination(player_id) else { abort_player_poison_fog(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) };
    if game.player_skill_execution(player_id, POISON_FOG_SKILL_ID).is_some_and(|state| state.stage() == SkillStage::Begin) { let mana = game.find_player(player_id).map_or(0, CPlayer::mana); if (mana.wrapping_sub(mp_loss) as i32) < 0 { fail(game, player_id, 7, b"GS0288", Some(mp_loss)); abort_player_poison_fog(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_mana(mana.wrapping_sub(mp_loss)); } let _ = game.update_player_current_state(player_id, GamePlayerFightStatePhase::MoveShapeAi); if !game.find_player(player_id).is_some_and(|player| weapon_valid(game, player)) { fail(game, player_id, 0x0e, b"GS0293", None); abort_player_poison_fog(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } let _can_break = can_break; if let Some(player) = game.find_player_mut(player_id) { player.movement_shape_mut().set_direction(get_line_direction(source_x, source_y, x, y)); } visual(game, player_id, level, 1, None); if let Some(state) = game.player_skill_execution_mut(player_id, POISON_FOG_SKILL_ID) { let _ = state.advance(SkillStage::Begin, SkillStage::Check); } }
    let started = game.player_skill_execution(player_id, POISON_FOG_SKILL_ID).map(SkillExecutionKernel::started_at_ms).expect("выполнение ядовитого тумана создано"); if !time_reached(runtime.now_milliseconds(), started, delay) { return terminal(QueuedSkillExecutionState::Pending) }
    visual(game, player_id, level, 2, Some((x, y)));
    ...
    if let Some(state) = game.player_skill_execution_mut(player_id, POISON_FOG_SKILL_ID) { let _ = state.advance(SkillStage::Check, SkillStage::Calculate); let _ = state.advance(SkillStage::Calculate, SkillStage::Attack); let _ = state.advance(SkillStage::Attack, SkillStage::Apply); } finish_player_poison_fog(game, player_id, runtime); terminal(if summoned { QueuedSkillExecutionState::Completed } else { QueuedSkillExecutionState::Rejected })
}
