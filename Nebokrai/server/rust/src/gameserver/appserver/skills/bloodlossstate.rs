...
//! Object Begin (0x005E3BD0) не требует user/sufferer: base Begin,
//! visual SetRun(1) → Update(0) → base visual tail, затем обнуление attack-count.
//! restart_blood_loss_state переносит Begin(NULL, holder) по точному ключу:
...
pub(crate) fn restart_blood_loss_state(
    game: &mut CGame,
    region_id: i32,
    ...
    let Some(state) = resolve_state_move_shape(game, region_id, holder)
        .and_then(|shape| shape.applied_state::<BloodLossState>(key)).copied()
    else { return false };
    ...
    if let Some(state) = resolve_state_move_shape_mut(game, region_id, holder)
        .and_then(|shape| shape.applied_state_mut::<BloodLossState>(key))
    {
...
pub(crate) fn update_player_blood_loss_state<Runtime: GameMainLoopRuntime>(
    ...
    let prepared = game.with_player_state_random::<BloodLossState, _>(player_id, key, |state, random| {
...
pub(crate) fn update_monster_blood_loss_state<Runtime: GameMainLoopRuntime>(
    ...
    let prepared = owner.base_mut().find_monster_by_id_mut(monster_id).and_then(|monster| {
        let state = monster.move_shape_mut().applied_state_mut::<BloodLossState>(key)?;
