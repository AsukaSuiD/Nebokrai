//! Общий visual CLightingArrow, CLightingArrow2, MeteorArrow/FallingStar и RainArrow.
//! Источник: gameserver.exe/GameServer.pdb, appserver/skills/lightingarrow.cpp
//! lightingarrow2.cpp, meteorarrow.cpp, fallingstar.cpp и rainarrow.cpp.
//! Подготовка передаёт направление свежего U; выпуск
//! заново разрешает S, её X/Y либо базовую точку. Очистка сохранённой identity
//! не запрещает GetS найти фигуру по координатам. E7 добавляет время полёта;
//! только он поддерживает mode3 с направлением U. MeteorArrow/FallingStar
//! передают нулевые type/id цели, сохраняя свежие координаты, и добавляют ошибку 4.
//! RainArrow добавляет правую и левую конечные клетки (либо свежие X/Y U)
//! и время полёта. Ошибки — BYTE0/BYTEcode.
...
pub(crate) fn publish_arrow_cast_visual(game: &CGame, skill: &MoveShapeSkill, mode: u32) {
    if !matches!(skill.owner(), SkillOwner::CLightingArrow | SkillOwner::CLightingArrow2
        | SkillOwner::CMeteorArrow | SkillOwner::CFallingStar | SkillOwner::CRainArrow)
    ...
    if matches!(mode, 2 | 7 | 10 | 11 | 13 | 14 | 15)
        || (matches!(skill.owner(), SkillOwner::CMeteorArrow | SkillOwner::CFallingStar) && mode == 4)
    {
    ...
    if let Some((kind, id, x, y)) = target {
        let anonymous_target = matches!(skill.owner(), SkillOwner::CMeteorArrow | SkillOwner::CFallingStar);
        ...
        if skill.owner() == SkillOwner::CRainArrow {
            let Some(state) = skill.player_state::<RainArrowExecutionState>() else { return; };
            let right = state.right_impact().unwrap_or_else(||
                (source.get_tile_x().unwrap_or(i32::MIN), source.get_tile_y().unwrap_or(i32::MIN)));
            message.add_long(right.0);
            message.add_long(right.1);
            let left = state.left_impact().unwrap_or_else(||
                (source.get_tile_x().unwrap_or(i32::MIN), source.get_tile_y().unwrap_or(i32::MIN)));
            message.add_long(left.0);
            message.add_long(left.1);
