//! Второй периодический удар листвы `CLeafCut2` (`0x80`).
//!
//! Источник: `gameserver.exe` + `GameServer.pdb`, исходный владелец
//! `appserver/skills/leafcut2.cpp`. Владелец проверяет объектную цель, путь и
//! меч до начала, после необратимого расхода MP повторно проверяет оружие, а
...
pub(crate) fn is_leaf_cut_2_dispatch(dispatch: PlayerSkillDispatch) -> bool { skill_id(dispatch) == LEAF_CUT_2_SKILL_ID }
...
fn path_block(game: &CGame, region_id: i32, source_x: i32, source_y: i32, target_x: i32, target_y: i32, maximum: u32) -> Option<bool> {
    ...
    if maximum != 0 && maximum < path.len() as u32 { return None }
...
fn send_failure(game: &CGame, player_id: i32, code: u8, amount: u32, text: Option<(&[u8], &[u8])>) {
    ...
    match code { 7 => game.send_skill_system_info_with_unsigned(player_id, b"GS0288", amount), 10 => game.send_skill_system_info(player_id, b"GS0286"), 0x0b => game.send_skill_system_info(player_id, b"GS0290"), 0x0d => game.send_skill_system_info(player_id, b"GS0278"), 0x0e => game.send_skill_system_info(player_id, b"GS0292"), _ => {} }
}

fn reject_initial(game: &CGame, player_id: i32, code: u8, amount: u32, text: Option<(&[u8], &[u8])>) -> QueuedSkillExecutionOutcome { send_failure(game, player_id, code, amount, text); game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 2); terminal(QueuedSkillExecutionState::Rejected) }

fn send_visual(game: &mut CGame, player_id: i32, level: i32, action: u8, target: Option<(ShapeIdentity, i32, i32)>) {
    let Some(player) = game.find_player(player_id) else { return }; let mut message = CMessage::new(EFFECT_MESSAGE); message.add_byte(action); message.add_long(LEAF_CUT_2_SKILL_ID as i32); message.add_short(level as i16); message.add_long(PLAYER_TYPE); message.add_long(player_id); if action == 1 { message.add_long(player.shape().get_direction()); } else if let Some((target, x, y)) = target { message.add_long(target.object_type); message.add_long(target.id); message.add_long(x); message.add_long(y); } let _ = game.send_player_shape_around(player_id, None, &message);
}
...
pub(crate) fn execute_player_leaf_cut_2<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some(properties) = game.skill_base_properties(LEAF_CUT_2_SKILL_ID, level) else { if ai.leaf_cut_2().is_some() { finish(game, player_id); } return terminal(QueuedSkillExecutionState::Rejected) };
    let mp_loss = properties.query_property(USER_MP_LOSE); let reuse = properties.query_property(SKILL_USAGE_REUSE_DELAY_TIME); let delay = properties.query_property(SKILL_USAGE_DELAY_TIME); let maximum = properties.query_property(SKILL_USAGE_TARGET_MAX_DISTANCE); let keep = properties.query_property(STATE_PERSIST_TIME); let frequency = properties.query_property(TARGET_AFFECT_FREQUENCY); let factor = properties.query_property(TARGET_DAMAGE_FACTOR); let weapon_modifier = properties.query_property(WEAPON_DAMAGE_LEVEL_MODIFIER); let skill_name = properties.skill_name().to_vec(); let _breakable = properties.query_property(SKILL_USAGE_CAN_BE_BREAKED);
    if ai.leaf_cut_2().is_none() { let now = runtime.now_milliseconds(); if ai.leaf_cut_2_last_used_ms() != 0 && now < ai.leaf_cut_2_last_used_ms().wrapping_add(reuse) { return reject_initial(game, player_id, 0x0d, 0, None) } let Some(blocked) = path_block(game, region_id, source_x, source_y, target_x, target_y, maximum) else { return reject_initial(game, player_id, 0x0b, 0, None) }; if blocked { return reject_initial(game, player_id, 0x0f, 0, Some((b"GS0291", &target_name))) } let Some(player) = game.find_player(player_id) else { return terminal(QueuedSkillExecutionState::Rejected) }; if !weapon_is_sword(game, player) { return reject_initial(game, player_id, 0x0e, 0, None) } if mp_loss != 0 && (initial_mana.wrapping_sub(mp_loss) as i32) < 0 { return reject_initial(game, player_id, 7, mp_loss, None) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(false); player.set_current_skill_id(Some(LEAF_CUT_2_SKILL_ID)); } ai.begin_leaf_cut_2(SkillExecutionKernel::begin(dispatch, now)); }
    if target_dead { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 10); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }
    if ai.leaf_cut_2().is_some_and(|state| state.stage() == SkillStage::Begin) { let mana = game.find_player(player_id).map_or(0, CPlayer::mana); if (mana.wrapping_sub(mp_loss) as i32) < 0 { send_failure(game, player_id, 7, mp_loss, None); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_mana(mana.wrapping_sub(mp_loss)); } let _ = game.update_player_current_state(player_id, GamePlayerFightStatePhase::MoveShapeAi); if game.find_player(player_id).is_none_or(|player| !weapon_is_sword(game, player)) { send_failure(game, player_id, 0x0e, 0, None); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } let direction = get_line_direction(source_x, source_y, target_x, target_y); if let Some(player) = game.find_player_mut(player_id) { player.movement_shape_mut().set_direction(direction); } send_visual(game, player_id, level, 1, None); if let Some(state) = ai.leaf_cut_2_mut() { let _ = state.advance(SkillStage::Begin, SkillStage::Check); } }
    ...
    let Some((live_source_x, live_source_y)) = game.find_player(player_id).and_then(|player| Some((player.shape().get_tile_x().ok()?, player.shape().get_tile_y().ok()?))) else { finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; let Some((live_x, live_y, live_dead, _)) = target_facts(game, region_id, target) else { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 10); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; if live_dead { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 10); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } let Some(blocked) = path_block(game, region_id, live_source_x, live_source_y, live_x, live_y, maximum) else { send_failure(game, player_id, 0x0b, 0, None); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; if blocked { send_failure(game, player_id, 0x0f, 0, Some((b"GS0307", &skill_name))); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }
    ...
    let Some((master, combat, weapon_level)) = game.find_player(player_id).map(|player| (master_info(player), player.combat_properties(), player.weapon_damage_level(game.goods_factory()))) else { finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; let now = runtime.now_milliseconds(); let state = LeafCutState2::new(master, now, keep, frequency, unsigned_float(factor) * 0.01, unsigned_float(weapon_level as u32) * unsigned_float(weapon_modifier) * 0.01, combat.minimum_attack as u16, combat.maximum_attack as u16, combat.add_element_attack as u16, combat.add_soul_attack);
    let Some((previous, identity, x, y)) = game.replace_leaf_cut_2_state(region_id, target, state) else { finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; if let Some(previous) = previous { send_leaf_cut_2_state_visual(game, region_id, identity, x, y, previous, false, now); } send_leaf_cut_2_state_visual(game, region_id, identity, x, y, state, true, now); if let Some(player) = game.find_player_mut(player_id) { player.movement_shape_mut().set_action(1); } if let Some(kernel) = ai.leaf_cut_2_mut() { let _ = kernel.advance(SkillStage::Check, SkillStage::Calculate); let _ = kernel.advance(SkillStage::Calculate, SkillStage::Attack); let _ = kernel.advance(SkillStage::Attack, SkillStage::Apply); } ai.mark_leaf_cut_2_used(now); finish(game, player_id); terminal(QueuedSkillExecutionState::Completed)
}
