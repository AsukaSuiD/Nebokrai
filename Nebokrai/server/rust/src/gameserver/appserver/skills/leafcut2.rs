...
pub(crate) fn execute_player_leaf_cut_2<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some(properties) = game.skill_base_properties(LEAF_CUT_2_SKILL_ID, level) else { if ai.leaf_cut_2().is_some() { finish_player_leaf_cut_2(game, player_id, ai, runtime); } return terminal(QueuedSkillExecutionState::Rejected) };
