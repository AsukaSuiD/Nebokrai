...
fn finish_player_leaf_cut_2<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, _player_ai: &mut CPlayerAI, runtime: &mut Runtime) { if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(true); } finish_summon_skill(game, player_id, LEAF_CUT_2_SKILL_ID, runtime); }
pub(crate) fn cancel_player_leaf_cut_2<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> bool { let Some(dispatch) = game.player_skill_execution(player_id, LEAF_CUT_2_SKILL_ID).map(SkillExecutionKernel::dispatch) else { return false }; finish_player_leaf_cut_2(game, player_id, player_ai, runtime); game.finish_player_skill(player_id, player_ai, dispatch, SkillTermination::Cancelled) }
...
pub(crate) fn execute_player_leaf_cut_2<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some(properties) = game.skill_base_properties(LEAF_CUT_2_SKILL_ID, level) else { if game.player_skill_execution(player_id, LEAF_CUT_2_SKILL_ID).is_some() { finish_player_leaf_cut_2(game, player_id, ai, runtime); } return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    if game.player_skill_execution(player_id, LEAF_CUT_2_SKILL_ID).is_none() { let now = runtime.now_milliseconds(); if !skill_is_restored(game.player_skill_last_used_ms(player_id, LEAF_CUT_2_SKILL_ID), reuse, now) { return reject_initial(game, player_id, 0x0d, 0, None) } let Some(blocked) = path_block(game, region_id, source_x, source_y, target_x, target_y, maximum) else { return reject_initial(game, player_id, 0x0b, 0, None) }; if blocked { return reject_initial(game, player_id, 0x0f, 0, Some((b"GS0291", &target_name))) } let Some(player) = game.find_player(player_id) else { return terminal(QueuedSkillExecutionState::Rejected) }; if !weapon_is_sword(game, player) { return reject_initial(game, player_id, 0x0e, 0, None) } if mp_loss != 0 && (initial_mana.wrapping_sub(mp_loss) as i32) < 0 { return reject_initial(game, player_id, 7, mp_loss, None) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(false); player.set_current_skill_id(Some(LEAF_CUT_2_SKILL_ID)); } game.begin_player_skill_execution(player_id, ai, SkillExecutionKernel::begin(dispatch, now)); return terminal(QueuedSkillExecutionState::Begun); }
    if target_dead { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 10); finish_player_leaf_cut_2(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) }
    if game.player_skill_execution(player_id, LEAF_CUT_2_SKILL_ID).is_some_and(|state| state.stage() == SkillStage::Begin) { let mana = game.find_player(player_id).map_or(0, CPlayer::mana); if (mana.wrapping_sub(mp_loss) as i32) < 0 { send_failure(game, player_id, 7, mp_loss, None); finish_player_leaf_cut_2(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_mana(mana.wrapping_sub(mp_loss)); } let _ = game.update_player_current_state(player_id, GamePlayerFightStatePhase::MoveShapeAi); if game.find_player(player_id).is_none_or(|player| !weapon_is_sword(game, player)) { send_failure(game, player_id, 0x0e, 0, None); finish_player_leaf_cut_2(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) } let direction = get_line_direction(source_x, source_y, target_x, target_y); if let Some(player) = game.find_player_mut(player_id) { player.movement_shape_mut().set_direction(direction); } send_visual(game, player_id, level, 1, None); if let Some(state) = game.player_skill_execution_mut(player_id, LEAF_CUT_2_SKILL_ID) { let _ = state.advance(SkillStage::Begin, SkillStage::Check); } }
    let started = game.player_skill_execution(player_id, LEAF_CUT_2_SKILL_ID).map(SkillExecutionKernel::started_at_ms).unwrap_or_default(); if runtime.now_milliseconds() < started.wrapping_add(delay) { return terminal(QueuedSkillExecutionState::Pending) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(true); }
