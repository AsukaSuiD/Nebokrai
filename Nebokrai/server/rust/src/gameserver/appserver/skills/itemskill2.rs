...
pub(crate) fn execute_player_item_skill_2<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    if game.player_skill_execution(player_id, ITEM_SKILL_2_ID).is_none() {
    ...
    if !path.is_empty() && !path.iter().any(|cell|cell.2==2) { let path=path.into_iter().map(|(x,y,_)|(x,y)).collect::<Vec<_>>(); let (souls,variable)=consume_soul_collect_snapshot(game,(region_id,ShapeIdentity{object_type:PLAYER_TYPE,id:player_id,ex_id:Default::default()})); let master=game.find_player(player_id).map(master_info); if let Some(master)=master { let summon_id=game.allocate_summon_shape_id(); let now=runtime.now_milliseconds(); let mut phalanx=CThunderFirePhalanx::new(summon_id,master,now,lifetime,level,minimum_attack,maximum_attack,element_modifier,path.clone(),speed,souls,variable as u32); phalanx.shape_mut().set_region_id(region_id); if consume_item(game,player_id,item_position as u32,item_index,item_amount) { if let Some((x,y))=path.first().copied(){let result=game.add_thunder_fire_phalanx(region_id,phalanx,x,y,now,runtime);if result.as_ref().is_some_and(|result|result.is_ok()){let _=game.send_thunder_fire_phalanx_entry(region_id,summon_id,runtime);}tracing::trace!(region_id,player_id,summon_id,?result,"создан громовой огонь");}}}}
