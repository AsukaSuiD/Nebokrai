...
pub(crate) fn cancel_player_item_skill_2<Runtime: GameMainLoopRuntime>(
    ...
    let Some(dispatch) = player_ai.player_skill_execution(ITEM_SKILL_2_ID).map(SkillExecutionKernel::dispatch) else {
        return false;
    };
...
pub(crate) fn execute_player_item_skill_2<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    if player_ai.player_skill_execution(ITEM_SKILL_2_ID).is_none() {
    ...
    if player_ai.player_skill_execution(ITEM_SKILL_2_ID).is_some_and(|execution| execution.stage()==SkillStage::Begin) {
        ...
        let _=game.update_player_current_state(player_id,GamePlayerFightStatePhase::MoveShapeAi); send_visual(game,player_id,level,None); if let Some(execution)=player_ai.player_skill_execution_mut(ITEM_SKILL_2_ID){let _=execution.advance(SkillStage::Begin,SkillStage::Check);}
    }
    let started=player_ai.player_skill_execution(ITEM_SKILL_2_ID).map(SkillExecutionKernel::started_at_ms).expect("громовой огонь начат"); if !time_reached(runtime.now_milliseconds(),started,delay){return terminal(QueuedSkillExecutionState::Pending);} if let Some(player)=game.find_player_mut(player_id){player.set_skill_moveable(true);}
