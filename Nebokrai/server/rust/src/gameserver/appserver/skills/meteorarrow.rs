...
pub(crate) fn complete_player_meteor_arrow<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, runtime: &mut Runtime) -> bool {
    let Some(dispatch) = ai.meteor_arrow().map(|state| state.kernel().dispatch()) else { return false };
...
pub(crate) fn execute_player_meteor_arrow<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32,
    ...
    let Some(properties) = game.skill_base_properties(METEOR_ARROW_SKILL_ID, level) else { if ai.meteor_arrow().is_some() { abort_player_meteor_arrow(game, player_id); } return outcome(QueuedSkillExecutionState::Rejected) };
