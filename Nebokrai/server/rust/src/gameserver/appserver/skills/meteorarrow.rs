...
pub(crate) fn execute_player_meteor_arrow<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32,
    ...
    let Some(properties) = game.skill_base_properties(METEOR_ARROW_SKILL_ID, level) else { return outcome(QueuedSkillExecutionState::Rejected) };
    ...
    if ai.meteor_arrow().is_none() {
        ...
        ai.begin_meteor_arrow(MeteorArrowExecutionState::begin(dispatch, destination, target, now));
    } else if ai.meteor_arrow().is_none_or(|state| state.kernel().dispatch() != dispatch) { return outcome(QueuedSkillExecutionState::Rejected) }

    let state = ai.meteor_arrow().expect("выполнение метеорной стрелы создано"); let (mut destination, target) = (state.destination, state.target);
    ...
    if !state.condition_checked {
        ...
        if game.find_player(player_id).is_none_or(|p| p.meteor_arrow_state().is_none_or(|state| state.arrows() == 0)) {
            game.send_base_magic_failure(player_id, 4); game.send_skill_system_info(player_id, b"GS0300"); finish(game, player_id); return outcome(QueuedSkillExecutionState::Rejected)
        }
        ...
        if let Some(state) = ai.meteor_arrow_mut() { state.condition_checked = true; let _ = state.kernel_mut().advance(SkillStage::Begin, SkillStage::Check); }
    }
    let started = ai.meteor_arrow().expect("состояние сохранено").kernel().started_at_ms();
    ...
    if arrows == 0 { game.send_base_magic_failure(player_id, 4); game.send_skill_system_info(player_id, b"GS0300"); finish(game, player_id); return outcome(QueuedSkillExecutionState::Rejected) }
    ...
    if let Some(state) = ai.meteor_arrow_mut() { let _ = state.kernel_mut().advance(SkillStage::Check, SkillStage::Calculate);
    ...
    ai.mark_meteor_arrow_used(runtime.now_milliseconds()); finish(game, player_id); outcome(QueuedSkillExecutionState::Completed)
}
