...
pub(crate) fn execute_player_yunsheng_lightning<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some(properties) = game.skill_base_properties(YUNSHENG_LIGHTNING_SKILL_ID, level).cloned() else { if ai.player_skill_state::<PlayerYunShengLightningExecutionState>(YUNSHENG_LIGHTNING_SKILL_ID).is_some() { restore_player(game, player_id); abort_skill(game, player_id); } return player_terminal(QueuedSkillExecutionState::Rejected) };
    ...
    let fallback = ai.player_skill_state::<PlayerYunShengLightningExecutionState>(YUNSHENG_LIGHTNING_SKILL_ID).map(|state| state.destination); let Some(destination) = player_destination(game, region_id, dispatch, fallback) else { restore_player(game, player_id); abort_skill(game, player_id); return player_terminal(QueuedSkillExecutionState::Rejected) };
