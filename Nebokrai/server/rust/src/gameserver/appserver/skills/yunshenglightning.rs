...
pub(crate) fn execute_player_yunsheng_lightning<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let started = ai.player_skill_state::<PlayerYunShengLightningExecutionState>(YUNSHENG_LIGHTNING_SKILL_ID).map(|state| state.kernel().started_at_ms()).unwrap_or_default(); if ai.player_skill_state::<PlayerYunShengLightningExecutionState>(YUNSHENG_LIGHTNING_SKILL_ID).is_some_and(|state| !state.fired) { if !time_reached(now, started, delay) { return player_terminal(QueuedSkillExecutionState::Pending) } let path = game.base_magic_path(region_id, source_x, source_y, destination.0, destination.1, None); if maximum != 0 && path.len() > maximum as usize { send_player_failure(game, player_id, 0x0b); restore_player(game, player_id); return player_terminal(QueuedSkillExecutionState::Rejected) } let visual_target = player_target(dispatch).filter(|target| game.base_magic_target_view(region_id, *target).is_some()); send_player_fire(game, player_id, level, visual_target, destination); if let Some(state) = ai.player_skill_state_mut::<PlayerYunShengLightningExecutionState>(YUNSHENG_LIGHTNING_SKILL_ID) { state.destination = destination; state.fired = true; let _ = state.kernel_mut().advance(SkillStage::Check, SkillStage::Calculate); } }
