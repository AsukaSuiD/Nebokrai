...
pub(crate) fn execute_player_lighting_arrow<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32,
    ...
    if player_ai.lighting_arrow().is_none() {
        ...
        player_ai.begin_lighting_arrow(LightingArrowExecutionState::begin(dispatch, destination, target, now_ms));
    } else if player_ai.lighting_arrow().is_none_or(|s| s.kernel().dispatch() != dispatch) { return outcome(QueuedSkillExecutionState::Rejected); }

    let state = player_ai.lighting_arrow().expect("выполнение световой стрелы создано");
