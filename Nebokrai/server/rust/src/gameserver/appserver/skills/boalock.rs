...
pub(crate) fn execute_player_boa_lock<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let started = player_ai.player_skill_state::<BoaLockExecutionState>(BOA_LOCK_SKILL_ID).map(|state| state.kernel.started_at_ms()).unwrap_or_default(); if !player_ai.player_skill_state::<BoaLockExecutionState>(BOA_LOCK_SKILL_ID).is_some_and(|state| state.kernel().is_prepared()) { if !time_reached(runtime.now_milliseconds(), started, delay) { return terminal(QueuedSkillExecutionState::Pending) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(true); } let Some((x, y, _)) = target_snapshot(game, region_id, identity) else { return reject_runtime(game, player_id, None) }; let path = game.base_magic_path(region_id, source_x, source_y, x, y, None); if max_distance != 0 && path.len() as u32 > max_distance { return reject_runtime(game, player_id, Some((0x0b, mp, None))) } if path.iter().any(|cell| cell.2 == 2) { let name = target_name(game, region_id, identity).to_vec(); send_path_failure(game, player_id, b"GS0296", &name); abort_player_boa_lock(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } let missile_time = missile_per_cell.wrapping_mul(path.len() as u32); send_fire(game, player_id, level, identity, (x, y), missile_time); if let Some(state) = player_ai.player_skill_state_mut::<BoaLockExecutionState>(BOA_LOCK_SKILL_ID) { state.kernel_mut().mark_prepared(); state.missile_time = missile_time; let _ = state.kernel.advance(SkillStage::Check, SkillStage::Calculate); let _ = state.kernel.advance(SkillStage::Calculate, SkillStage::Attack); } }
