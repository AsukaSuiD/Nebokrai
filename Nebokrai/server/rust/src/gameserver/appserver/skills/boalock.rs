...
//! оглушение, после чего выполняется пустая контактная атака. Состояния
//! принадлежат `CanonicalStateStorage`; `CGame` только координирует владельцев.
...
fn finish_player_boa_lock<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, runtime: &mut Runtime) {
    ...
    finish_state_skill(game, player_id, ai, runtime, |ai, now_ms| ai.mark_boa_lock_used(now_ms));
}
fn abort_player_boa_lock(game: &mut CGame, player_id: i32) { restore_player_movement(game, player_id); abort_skill(game, player_id); }
pub(crate) fn complete_player_boa_lock<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, runtime: &mut Runtime) -> bool {
    let Some(dispatch) = ai.boa_lock().map(|state| state.kernel().dispatch()) else { return false };
...
pub(crate) fn execute_player_boa_lock<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    if player_ai.boa_lock().is_none() { let now = runtime.now_milliseconds(); let Some(identity) = target(dispatch) else { return reject_begin(game, player_id, Some((10, mp, Some(b"GS0294")))) }; if identity.object_type == PLAYER_TYPE && identity.id == player_id { return reject_begin(game, player_id, Some((10, mp, Some(b"GS0286")))) } if player_ai.boa_lock_last_used_ms() != 0 && !time_reached(now, player_ai.boa_lock_last_used_ms(), reuse) { return reject_begin(game, player_id, Some((0x0d, mp, None))) } let Some((x, y, _)) = target_snapshot(game, region_id, identity) else { return reject_begin(game, player_id, Some((10, mp, Some(b"GS0294")))) }; let path = game.base_magic_path(region_id, source_x, source_y, x, y, None); if max_distance != 0 && path.len() as u32 > max_distance { return reject_begin(game, player_id, Some((0x0b, mp, None))) } if path.iter().any(|cell| cell.2 == 2) { let name = target_name(game, region_id, identity).to_vec(); send_path_failure(game, player_id, b"GS0295", &name); send_failure(game, player_id, 2, 0, None); abort_player_boa_lock(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } if mp != 0 && (initial_mana.wrapping_sub(mp) as i32) < 0 { return reject_begin(game, player_id, Some((7, mp, None))) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(false); player.set_current_skill_id(Some(BOA_LOCK_SKILL_ID)); } player_ai.begin_boa_lock(BoaLockExecutionState::begin(dispatch, now)); }
    else if player_ai.boa_lock().is_none_or(|state| state.kernel.dispatch() != dispatch) { return terminal(QueuedSkillExecutionState::Rejected) }
    ...
    if player_ai.boa_lock().is_some_and(|state| !state.condition_checked) { let mana = game.find_player(player_id).map_or(0, CPlayer::mana); if (mana.wrapping_sub(mp) as i32) < 0 { return reject_runtime(game, player_id, Some((7, mp, None))) } if let Some(player) = game.find_player_mut(player_id) { player.set_mana(mana.wrapping_sub(mp)); player.movement_shape_mut().set_direction(get_line_direction(source_x, source_y, target_x, target_y)); } let _ = game.update_player_current_state(player_id, GamePlayerFightStatePhase::MoveShapeAi); send_start(game, player_id, level); if let Some(state) = player_ai.boa_lock_mut() { state.condition_checked = true; let _ = state.kernel.advance(SkillStage::Begin, SkillStage::Check); } }
    let started = player_ai.boa_lock().map(|state| state.kernel.started_at_ms()).unwrap_or_default(); if !player_ai.boa_lock().is_some_and(|state| state.attacking_started) { if !time_reached(runtime.now_milliseconds(), started, delay) { return terminal(QueuedSkillExecutionState::Pending) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(true); } let Some((x, y, _)) = target_snapshot(game, region_id, identity) else { return reject_runtime(game, player_id, None) }; let path = game.base_magic_path(region_id, source_x, source_y, x, y, None); if max_distance != 0 && path.len() as u32 > max_distance { return reject_runtime(game, player_id, Some((0x0b, mp, None))) } if path.iter().any(|cell| cell.2 == 2) { let name = target_name(game, region_id, identity).to_vec(); send_path_failure(game, player_id, b"GS0296", &name); abort_player_boa_lock(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } let missile_time = missile_per_cell.wrapping_mul(path.len() as u32); send_fire(game, player_id, level, identity, (x, y), missile_time); if let Some(state) = player_ai.boa_lock_mut() { state.attacking_started = true; state.missile_time = missile_time; let _ = state.kernel.advance(SkillStage::Check, SkillStage::Calculate); let _ = state.kernel.advance(SkillStage::Calculate, SkillStage::Attack); } }
    let missile_time = player_ai.boa_lock().map_or(0, |state| state.missile_time); if !time_reached(runtime.now_milliseconds(), started, delay.wrapping_add(missile_time)) { return terminal(QueuedSkillExecutionState::Pending) } let Some(target_level) = target_level(game, region_id, identity) else { finish_player_boa_lock(game, player_id, player_ai, runtime); return terminal(QueuedSkillExecutionState::Completed) }; let master = game.find_player(player_id).map(master_info); if let Some(master) = master && game.owned_player_skill_target_attackable(master, identity, region_id) { let keep = adjusted_time(source_level, target_level, persist); if keep != 0 { let now = runtime.now_milliseconds(); let lock = (target_level < source_level).then_some(BoaLockState::new(now, keep)); let _ = game.apply_boa_lock_control(region_id, identity, lock, KnockOutState::new(now, keep), now); } if let Some(contact) = game.find_player(player_id).map(contact) { game.apply_owned_skill_contact(master, identity, region_id, contact, runtime); } } if let Some(state) = player_ai.boa_lock_mut() { let _ = state.kernel.advance(SkillStage::Attack, SkillStage::Apply); } finish_player_boa_lock(game, player_id, player_ai, runtime); terminal(QueuedSkillExecutionState::Completed)
}
