...
pub(crate) fn execute_player_rage_break<Runtime: GameMainLoopRuntime>(
    ...
    let Some(properties) = game.skill_base_properties(RAGE_BREAK_SKILL_ID, level) else { if ai.rage_break().is_some() { finish(game, player_id); } return terminal(QueuedSkillExecutionState::Rejected) };
