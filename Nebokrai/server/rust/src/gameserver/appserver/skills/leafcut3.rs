...
//! путь и меч до начала, повторно расходует MP и проверяет оружие в первом
//! проходе ИИ, а после задержки заново строит путь. Частичная граница
//! `MP → OnChangeStates → weapon` сохранена. Формула и состояние принадлежат
//! `leafcutstate3.rs`; `CGame` только разрешает владельцев, PK и доставку.
...
fn reject_initial(game: &CGame, player_id: i32, code: u8, amount: u32, text: Option<(&[u8], &[u8])>) -> QueuedSkillExecutionOutcome {
    send_failure(game, player_id, code, amount, text);
...
fn send_visual(game: &mut CGame, player_id: i32, level: i32, action: u8, target: Option<(ShapeIdentity, i32, i32)>) {
    ...
    message.add_long(LEAF_CUT_3_SKILL_ID as i32);
    message.add_short(level as i16);
    message.add_long(PLAYER_TYPE);
    message.add_long(player_id);
    if action == 1 {
        message.add_long(player.shape().get_direction());
    } else if let Some((target, x, y)) = target {
...
pub(crate) fn execute_player_leaf_cut_3<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    if !is_leaf_cut_3_dispatch(dispatch) { return terminal(QueuedSkillExecutionState::Rejected) }
    ...
    let Some((region_id, level, initial_mana)) = game.find_player(player_id).and_then(|player| Some((player.server_region_id()?, player.learned_skill_level(LEAF_CUT_3_SKILL_ID, game.skill_factory()), player.mana()))) else { return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    let Some(properties) = game.skill_base_properties(LEAF_CUT_3_SKILL_ID, level) else { if game.player_skill_execution(player_id, LEAF_CUT_3_SKILL_ID).is_some() { finish_player_leaf_cut_3(game, player_id, ai, runtime); } return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    if game.player_skill_execution(player_id, LEAF_CUT_3_SKILL_ID).is_none() {
        let now = runtime.now_milliseconds();
        if !skill_is_restored(game.player_skill_last_used_ms(player_id, LEAF_CUT_3_SKILL_ID), reuse, now) { return reject_initial(game, player_id, 0x0d, 0, None) }
    ...
    if game.player_skill_execution(player_id, LEAF_CUT_3_SKILL_ID).is_some_and(|state| state.stage() == SkillStage::Begin) {
        ...
        if game.find_player(player_id).is_none_or(|player| !weapon_is_sword(game, player)) { send_failure(game, player_id, 0x0e, 0, None); finish_player_leaf_cut_3(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) }
        ...
        if let Some(state) = game.player_skill_execution_mut(player_id, LEAF_CUT_3_SKILL_ID) { let _ = state.advance(SkillStage::Begin, SkillStage::Check); }
    }

    let started = game.player_skill_execution(player_id, LEAF_CUT_3_SKILL_ID).map(|state| state.started_at_ms()).unwrap_or_default();
    ...
    let Some((live_x, live_y, live_dead, _)) = target_facts(game, region_id, target) else { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 10); finish_player_leaf_cut_3(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) };
    if live_dead { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 10); finish_player_leaf_cut_3(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) }
    let Some(blocked) = path_block(game, region_id, live_source_x, live_source_y, live_x, live_y, maximum) else { send_failure(game, player_id, 0x0b, 0, None); finish_player_leaf_cut_3(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) };
    if blocked { send_failure(game, player_id, 0x0f, 0, Some((b"GS0307", &skill_name))); finish_player_leaf_cut_3(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) }

    send_visual(game, player_id, level, 2, Some((target, live_x, live_y)));
    if target.object_type == PLAYER_TYPE { let _ = game.player_on_first_skill(player_id, target.id, Some(region_id), runtime); }
    let Some((master, combat, weapon_level)) = game.find_player(player_id).map(|player| (master_info(player), player.combat_properties(), player.weapon_damage_level(game.goods_factory()))) else { finish_player_leaf_cut_3(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) };
    let now = runtime.now_milliseconds();
    let (damage_factor, weapon_factor) = leaf_cut_factors(factor, weapon_level as u32, weapon_modifier);
    let state = LeafCutState3::new(master, now, keep, frequency, damage_factor, weapon_factor, combat.minimum_attack as u16, combat.maximum_attack as u16, combat.add_element_attack as u16, combat.add_soul_attack);
    let Some((previous, identity, x, y)) = game.replace_leaf_cut_3_state(region_id, target, state, now) else { finish_player_leaf_cut_3(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    if let Some(kernel) = game.player_skill_execution_mut(player_id, LEAF_CUT_3_SKILL_ID) { let _ = kernel.advance(SkillStage::Check, SkillStage::Calculate); let _ = kernel.advance(SkillStage::Calculate, SkillStage::Attack); let _ = kernel.advance(SkillStage::Attack, SkillStage::Apply); }
