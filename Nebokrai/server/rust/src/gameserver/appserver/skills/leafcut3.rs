...
pub(crate) fn execute_player_leaf_cut_3<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some(properties) = game.skill_base_properties(LEAF_CUT_3_SKILL_ID, level) else { if ai.leaf_cut_3().is_some() { finish(game, player_id); } return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    if ai.leaf_cut_3().is_some_and(|state| state.stage() == SkillStage::Begin) {
        ...
        if (mana.wrapping_sub(mp_loss) as i32) < 0 { send_failure(game, player_id, 7, mp_loss, None); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }
        if let Some(player) = game.find_player_mut(player_id) { player.set_mana(mana.wrapping_sub(mp_loss)); }
        let _ = game.update_player_current_state(player_id, GamePlayerFightStatePhase::MoveShapeAi);
        if game.find_player(player_id).is_none_or(|player| !weapon_is_sword(game, player)) { send_failure(game, player_id, 0x0e, 0, None); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }
    ...
    let Some((live_source_x, live_source_y)) = game.find_player(player_id).and_then(|player| Some((player.shape().get_tile_x().ok()?, player.shape().get_tile_y().ok()?))) else { finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) };
    let Some((live_x, live_y, live_dead, _)) = target_facts(game, region_id, target) else { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 10); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) };
    if live_dead { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 10); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }
    let Some(blocked) = path_block(game, region_id, live_source_x, live_source_y, live_x, live_y, maximum) else { send_failure(game, player_id, 0x0b, 0, None); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) };
    if blocked { send_failure(game, player_id, 0x0f, 0, Some((b"GS0307", &skill_name))); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }
    ...
    let Some((master, combat, weapon_level)) = game.find_player(player_id).map(|player| (master_info(player), player.combat_properties(), player.weapon_damage_level(game.goods_factory()))) else { finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) };
    let now = runtime.now_milliseconds();
    let state = LeafCutState3::new(master, now, keep, frequency, unsigned_float(factor) * 0.01, unsigned_float(weapon_level as u32) * unsigned_float(weapon_modifier) * 0.01, combat.minimum_attack as u16, combat.maximum_attack as u16, combat.add_element_attack as u16, combat.add_soul_attack);
    let Some((previous, identity, x, y)) = game.replace_leaf_cut_3_state(region_id, target, state, now) else { finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) };
