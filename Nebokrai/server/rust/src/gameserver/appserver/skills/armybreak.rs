...
//! Reuse проверяется exact `CSkill::IsRestored`; attack delay остаётся elapsed.
//!
...
fn finish_player_army_break<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, _player_ai: &mut CPlayerAI, skill_id: u32, runtime: &mut Runtime) { if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(true); } finish_summon_skill(game, player_id, skill_id, runtime); }
pub(crate) fn cancel_player_army_break<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, execution_skill_id: u32, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> bool { let Some(dispatch) = game.player_skill_state::<ArmyBreakExecutionState>(player_id, execution_skill_id).map(|state| state.kernel().dispatch()) else { return false }; finish_player_army_break(game, player_id, player_ai, skill_id(dispatch), runtime); game.finish_player_skill(player_id, player_ai, dispatch, SkillTermination::Cancelled) }
...
pub(crate) fn execute_player_army_break<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some((region_id, level, sx, sy, mana, rp)) = game.find_player(player_id).and_then(|player| Some((player.server_region_id()?, player.learned_skill_level(skill_id, game.skill_factory()), player.shape().get_tile_x().ok()?, player.shape().get_tile_y().ok()?, player.mana(), player.rp()))) else { return terminal(QueuedSkillExecutionState::Rejected) }; let Some(properties) = game.skill_base_properties(skill_id, level) else { if game.player_skill_state::<ArmyBreakExecutionState>(player_id, dispatch.skill_id()).is_some() { finish_player_army_break(game, player_id, ai, skill_id, runtime); } return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    if game.player_skill_state::<ArmyBreakExecutionState>(player_id, dispatch.skill_id()).is_none() { let now = runtime.now_milliseconds(); if !skill_is_restored(game.player_skill_last_used_ms(player_id, skill_id), reuse, now) { send_failure(game, player_id, 0x0d, 0); return terminal(QueuedSkillExecutionState::Rejected) } let Some(player) = game.find_player(player_id) else { return terminal(QueuedSkillExecutionState::Rejected) }; if !weapon_is_sword(game, player) { send_failure(game, player_id, 0x0e, 0); return terminal(QueuedSkillExecutionState::Rejected) } if mp_loss != 0 && (mana.wrapping_sub(mp_loss) as i32) < 0 { send_failure(game, player_id, 7, mp_loss); return terminal(QueuedSkillExecutionState::Rejected) } if rp_loss != 0 && (u32::from(rp).wrapping_sub(rp_loss) as i32) < 0 { send_failure(game, player_id, 8, rp_loss); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(false); player.set_current_skill_id(Some(skill_id)); } game.begin_player_skill_execution(player_id, ArmyBreakExecutionState::begin(dispatch, now)); return terminal(QueuedSkillExecutionState::Begun); } else if game.player_skill_state::<ArmyBreakExecutionState>(player_id, dispatch.skill_id()).is_none_or(|state| state.kernel().dispatch() != dispatch) { return terminal(QueuedSkillExecutionState::Rejected) }
    if game.player_skill_state::<ArmyBreakExecutionState>(player_id, dispatch.skill_id()).is_some_and(|state| state.kernel().stage() == SkillStage::Begin) { let mana = game.find_player(player_id).map_or(0, CPlayer::mana); if (mana.wrapping_sub(mp_loss) as i32) < 0 { send_failure(game, player_id, 7, mp_loss); finish_player_army_break(game, player_id, ai, skill_id, runtime); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_mana(mana.wrapping_sub(mp_loss)); } let rp = game.find_player(player_id).map_or(0, CPlayer::rp); if (u32::from(rp).wrapping_sub(rp_loss) as i32) < 0 { send_failure(game, player_id, 8, rp_loss); finish_player_army_break(game, player_id, ai, skill_id, runtime); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_rp(rp.wrapping_sub(rp_loss as u16)); } let _ = game.update_player_current_state(player_id, GamePlayerFightStatePhase::MoveShapeAi); if game.find_player(player_id).is_none_or(|player| !weapon_is_sword(game, player)) { send_failure(game, player_id, 0x0e, 0); finish_player_army_break(game, player_id, ai, skill_id, runtime); return terminal(QueuedSkillExecutionState::Rejected) } let (tx, ty) = destination(game, region_id, player_id, dispatch).unwrap_or((sx, sy)); let direction = get_line_direction(sx, sy, tx, ty); if let Some(player) = game.find_player_mut(player_id) { player.movement_shape_mut().set_direction(direction); } if let Some(state) = game.player_skill_state_mut::<ArmyBreakExecutionState>(player_id, dispatch.skill_id()) { state.direction = direction; let _ = state.kernel_mut().advance(SkillStage::Begin, SkillStage::Check); } send_visual(game, player_id, skill_id, level, direction, false); }
    let Some(state) = game.player_skill_state::<ArmyBreakExecutionState>(player_id, dispatch.skill_id()) else { return terminal(QueuedSkillExecutionState::Rejected) }; if !time_reached(runtime.now_milliseconds(), state.kernel().started_at_ms(), delay) { return terminal(QueuedSkillExecutionState::Pending) } let direction = state.direction; send_visual(game, player_id, skill_id, level, direction, true); if let Some(state) = game.player_skill_state_mut::<ArmyBreakExecutionState>(player_id, dispatch.skill_id()) { let _ = state.kernel_mut().advance(SkillStage::Check, SkillStage::Calculate); let _ = state.kernel_mut().advance(SkillStage::Calculate, SkillStage::Attack); }
    let owner = game.find_player(player_id).map(master).unwrap_or_default(); for (identity, central) in targets(game, region_id, player_id, direction) { if !game.owned_player_skill_target_attackable(owner, identity, region_id) { continue } let Some(target_level) = target_level(game, region_id, identity) else { continue }; let Some((owner, attack)) = calculate(game, player_id, target_level, skill_id, level, if central { major } else { minor }, hit) else { continue }; match identity.object_type { PLAYER_TYPE => game.with_published_player_ai(player_id, ai, |game| game.apply_owned_skill_attack_to_player(owner, identity.id, region_id, attack, runtime)), MONSTER_TYPE => game.with_published_player_ai(player_id, ai, |game| game.apply_owned_skill_attack_to_monster(owner, identity.id, region_id, attack, runtime)), _ => continue } }
    if let Some(state) = game.player_skill_state_mut::<ArmyBreakExecutionState>(player_id, dispatch.skill_id()) { let _ = state.kernel_mut().advance(SkillStage::Attack, SkillStage::Apply); } finish_player_army_break(game, player_id, ai, skill_id, runtime); terminal(QueuedSkillExecutionState::Completed)
}
