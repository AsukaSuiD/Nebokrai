...
pub(crate) fn cancel_player_army_break<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, execution_skill_id: u32, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> bool { let Some(dispatch) = player_ai.player_skill_state::<ArmyBreakExecutionState>(execution_skill_id).map(|state| state.kernel().dispatch()) else { return false }; finish_player_army_break(game, player_id, player_ai, skill_id(dispatch), runtime); player_ai.finish_player_skill(dispatch, SkillTermination::Cancelled) }
...
pub(crate) fn execute_player_army_break<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some((region_id, level, sx, sy, mana, rp)) = game.find_player(player_id).and_then(|player| Some((player.server_region_id()?, player.learned_skill_level(skill_id, game.skill_factory()), player.shape().get_tile_x().ok()?, player.shape().get_tile_y().ok()?, player.mana(), player.rp()))) else { return terminal(QueuedSkillExecutionState::Rejected) }; let Some(properties) = game.skill_base_properties(skill_id, level) else { if ai.player_skill_state::<ArmyBreakExecutionState>(dispatch.skill_id()).is_some() { finish_player_army_break(game, player_id, ai, skill_id, runtime); } return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    if let Some(state) = ai.player_skill_state_mut::<ArmyBreakExecutionState>(dispatch.skill_id()) { let _ = state.kernel_mut().advance(SkillStage::Attack, SkillStage::Apply); } finish_player_army_break(game, player_id, ai, skill_id, runtime); terminal(QueuedSkillExecutionState::Completed)
}
