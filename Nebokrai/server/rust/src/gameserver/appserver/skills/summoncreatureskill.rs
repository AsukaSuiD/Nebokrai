...
pub(crate) fn execute_player_summon_creature<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, _player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    if let Some(mut owner) = game.take_region_owner(region_id) {
        ...
        for _ in 0..amount {
            let mut tile_x = 0;
            let mut tile_y = 0;
            if let Ok(position) = game.random_region_position_owned(&owner.base().region, source_x.wrapping_sub(4), source_y.wrapping_sub(4), 8, 8) && position.found { tile_x = position.x; tile_y = position.y; }
    ...
    if let Some(cast) = cast {
        ...
        for _ in 0..amount {
            ...
            if let Ok(position) = game.random_region_position_owned(
                &region.region, source_x.wrapping_sub(4), source_y.wrapping_sub(4), 8, 8,
            ) && position.found {
                tile_x = position.x;
                tile_y = position.y;
            }
