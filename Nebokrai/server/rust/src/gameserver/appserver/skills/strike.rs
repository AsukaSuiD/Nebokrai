...
fn finish_player_strike<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, runtime: &mut Runtime) {
    restore_player_movement(game, player_id);
    finish_summon_skill(game, player_id, STRIKE_SKILL_ID, runtime);
}
...
pub(crate) fn complete_player_strike<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, runtime: &mut Runtime) -> bool {
    let Some(dispatch) = game.player_skill_state::<StrikeExecutionState>(player_id, STRIKE_SKILL_ID).copied().map(|state| state.kernel().dispatch()) else { return false };
    finish_player_strike(game, player_id, runtime);
...
pub(crate) fn execute_player_strike<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    if !is_strike_dispatch(dispatch) { return terminal(QueuedSkillExecutionState::Rejected) } let PlayerSkillDispatch::Object { target, .. } = dispatch else { unreachable!() }; let Some((region_id, source_x, source_y, source_level, level, mana)) = game.find_player(player_id).and_then(|player| Some((player.server_region_id()?, player.shape().get_tile_x().ok()?, player.shape().get_tile_y().ok()?, player.level(), player.learned_skill_level(STRIKE_SKILL_ID, game.skill_factory()), player.mana()))) else { return terminal(QueuedSkillExecutionState::Rejected) }; let Some(properties) = game.skill_base_properties(STRIKE_SKILL_ID, level) else { if game.player_skill_state::<StrikeExecutionState>(player_id, STRIKE_SKILL_ID).copied().is_some() { abort_player_strike(game, player_id); } return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    if game.player_skill_state::<StrikeExecutionState>(player_id, STRIKE_SKILL_ID).copied().is_none() { if target.object_type == PLAYER_TYPE && target.id == player_id { fail(game, player_id, 10, mp_loss, b"GS0286"); return terminal(QueuedSkillExecutionState::Rejected) } let now = runtime.now_milliseconds(); if !skill_is_restored(game.player_skill_last_used_ms(player_id, STRIKE_SKILL_ID), reuse, now) { fail(game, player_id, 0x0d, mp_loss, b"GS0278"); return terminal(QueuedSkillExecutionState::Rejected) } let Some(position) = target_view(game, region_id, target) else { fail(game, player_id, 10, mp_loss, b"GS0286"); return terminal(QueuedSkillExecutionState::Rejected) }; let path = game.base_magic_path(region_id, source_x, source_y, position.0, position.1, None); if maximum != 0 && path.len() as u32 > maximum { fail(game, player_id, 0x0b, mp_loss, b"GS0290"); return terminal(QueuedSkillExecutionState::Rejected) } if path.iter().any(|cell| cell.2 == BLOCK_UNFLY) { fail(game, player_id, 0x0f, mp_loss, b"GS0282"); return terminal(QueuedSkillExecutionState::Rejected) } if mp_loss != 0 && (mana.wrapping_sub(mp_loss) as i32) < 0 { fail(game, player_id, 7, mp_loss, b"GS0288"); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(false); player.set_current_skill_id(Some(STRIKE_SKILL_ID)); } game.begin_player_skill_execution(player_id, StrikeExecutionState::begin(dispatch, target, now)); return terminal(QueuedSkillExecutionState::Begun); }
    else if game.player_skill_state::<StrikeExecutionState>(player_id, STRIKE_SKILL_ID).copied().is_none_or(|state| state.kernel.dispatch() != dispatch) { return terminal(QueuedSkillExecutionState::Rejected) }
