...
pub(crate) fn execute_player_strike<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let started = ai.player_skill_state::<StrikeExecutionState>(STRIKE_SKILL_ID).copied().map(|state| state.kernel.started_at_ms()).unwrap_or_default(); if !ai.player_skill_state::<StrikeExecutionState>(STRIKE_SKILL_ID).copied().is_some_and(|state| state.attacking_started) { if !time_reached(runtime.now_milliseconds(), started, delay) { return terminal(QueuedSkillExecutionState::Pending) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(true); } let Some(position) = target_view(game, region_id, target) else { abort_player_strike(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; let path = game.base_magic_path(region_id, source_x, source_y, position.0, position.1, None); if maximum != 0 && path.len() as u32 > maximum { fail(game, player_id, 0x0b, mp_loss, b"GS0290"); abort_player_strike(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } if path.iter().any(|cell| cell.2 == BLOCK_UNFLY) { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 0x0f); let name = game.periodic_state_target_name(region_id, target).to_vec(); game.send_skill_system_info_with_text(player_id, b"GS0296", &name); abort_player_strike(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } let flying_time = missile_step.wrapping_mul(path.len() as u32); send_fire(game, player_id, level, target, position, flying_time); if let Some(state) = ai.player_skill_state_mut::<StrikeExecutionState>(STRIKE_SKILL_ID) { state.missile_flying_time_ms = flying_time; state.attacking_started = true; let _ = state.kernel.advance(SkillStage::Check, SkillStage::Calculate); } }
