...
//! владельцу цели, применение состояния и доставку.
...
pub(crate) fn execute_player_leaf_cut<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some((live_x, live_y, live_dead, live_name)) = target_facts(game, region_id, target) else { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 10); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; if live_dead { game.send_self_state_skill_failure(EFFECT_MESSAGE, player_id, 10); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } let Some(blocked) = path_block(game, region_id, source_x, source_y, live_x, live_y, maximum) else { send_failure(game, player_id, 0x0b, 0, None); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; if blocked { send_failure(game, player_id, 0x0f, 0, Some((b"GS0307", &live_name))); finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }
    ...
    let Some((master, combat, weapon_level)) = game.find_player(player_id).map(|player| (master_info(player), player.combat_properties(), player.weapon_damage_level(game.goods_factory()))) else { finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; let now = runtime.now_milliseconds(); let state = LeafCutState::new(master, now, keep, frequency, unsigned_float(factor) * 0.01, unsigned_float(weapon_level as u32) * unsigned_float(weapon_modifier) * 0.01, combat.minimum_attack as u16, combat.maximum_attack as u16, combat.add_element_attack as u16, combat.add_soul_attack, );
    let Some((previous, identity, x, y)) = game.replace_leaf_cut_state(region_id, target, state, now) else { finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; if let Some(previous) = previous { send_leaf_cut_state_visual(game, region_id, identity, x, y, previous, false, now); } send_leaf_cut_state_visual(game, region_id, identity, x, y, state, true, now); game.damage_player_weapon(player_id, runtime); if let Some(kernel) = ai.leaf_cut_mut() { let _ = kernel.advance(SkillStage::Check, SkillStage::Calculate); let _ = kernel.advance(SkillStage::Calculate, SkillStage::Attack); let _ = kernel.advance(SkillStage::Attack, SkillStage::Apply); } ai.mark_leaf_cut_used(now); finish(game, player_id); terminal(QueuedSkillExecutionState::Completed)
}
