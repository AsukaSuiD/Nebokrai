...
fn finish_player_leaf_cut<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, _player_ai: &mut CPlayerAI, runtime: &mut Runtime) { if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(true); } finish_summon_skill(game, player_id, LEAF_CUT_SKILL_ID, runtime); }
pub(crate) fn cancel_player_leaf_cut<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> bool { let Some(dispatch) = game.player_skill_execution(player_id, LEAF_CUT_SKILL_ID).map(SkillExecutionKernel::dispatch) else { return false }; finish_player_leaf_cut(game, player_id, player_ai, runtime); game.finish_player_skill(player_id, player_ai, dispatch, SkillTermination::Cancelled) }
...
pub(crate) fn execute_player_leaf_cut<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some((previous, identity, x, y)) = game.replace_leaf_cut_state(region_id, target, state, now) else { finish_player_leaf_cut(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) }; if let Some(previous) = previous { send_leaf_cut_state_visual(game, region_id, identity, x, y, previous, false, now); } send_leaf_cut_state_visual(game, region_id, identity, x, y, state, true, now); if let Some(kernel) = game.player_skill_execution_mut(player_id, LEAF_CUT_SKILL_ID) { let _ = kernel.advance(SkillStage::Check, SkillStage::Calculate); let _ = kernel.advance(SkillStage::Calculate, SkillStage::Attack); let _ = kernel.advance(SkillStage::Attack, SkillStage::Apply); } finish_player_leaf_cut(game, player_id, ai, runtime); terminal(QueuedSkillExecutionState::Completed)
}
