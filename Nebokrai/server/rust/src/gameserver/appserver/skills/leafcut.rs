...
pub(crate) fn execute_player_leaf_cut<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some((previous, identity, x, y)) = game.replace_leaf_cut_state(region_id, target, state, now) else { finish_player_leaf_cut(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Rejected) }; if let Some(previous) = previous { send_leaf_cut_state_visual(game, region_id, identity, x, y, previous, false, now); } send_leaf_cut_state_visual(game, region_id, identity, x, y, state, true, now); if let Some(kernel) = ai.player_skill_execution_mut(LEAF_CUT_SKILL_ID) { let _ = kernel.advance(SkillStage::Check, SkillStage::Calculate); let _ = kernel.advance(SkillStage::Calculate, SkillStage::Attack); let _ = kernel.advance(SkillStage::Attack, SkillStage::Apply); } finish_player_leaf_cut(game, player_id, ai, runtime); terminal(QueuedSkillExecutionState::Completed)
}
