...
pub(crate) fn update_rush_state(
    game: &mut CGame,
    region_id: i32,
    holder: ShapeIdentity,
    key: crate::gameserver::appserver::moveshape::StateKey,
    ...
    let expired = resolve_state_move_shape(game, region_id, holder)
        .and_then(|shape| shape.applied_state::<RushState>(key))
        .is_some_and(|state| state.expired(now_ms));
    expired && end_rush_state(game, region_id, holder, key)
}

pub(crate) fn end_rush_state(
    game: &mut CGame,
    region_id: i32,
    holder: ShapeIdentity,
    key: crate::gameserver::appserver::moveshape::StateKey,
    ...
    if shape.applied_state::<RushState>(key).is_none() { return false }
    shape.set_fightable(true);
    shape.set_moveable(true);
    let removed = shape.remove_applied_state_record::<RushState>(key, RUSH_STATE_BYTES).is_some();
    if removed {
