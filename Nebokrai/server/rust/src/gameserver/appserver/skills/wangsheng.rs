...
pub(crate) fn execute_battle_fairy_wangsheng<Runtime: GameMainLoopRuntime>(
    ...
    let starting = player_ai.battle_fairy_execution(WANGSHENG_SKILL_ID).is_none();
    ...
    if player_ai.battle_fairy_execution(WANGSHENG_SKILL_ID).is_none() {
        ...
        if !skill_is_restored(
            player_ai.battle_fairy_skill_last_used_ms(WANGSHENG_SKILL_ID),
            reuse_delay_ms,
            cooldown_now_ms,
        ) {
        ...
        .is_some_and(|state| state.stage() == SkillStage::Begin)
    {
        ...
        if let Some(state) = player_ai.battle_fairy_execution_mut(WANGSHENG_SKILL_ID) {
