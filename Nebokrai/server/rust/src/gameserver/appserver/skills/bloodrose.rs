...
fn calculate_attack(game: &mut CGame, player_id: i32, target_level: u8, level: i32, damage_factor: u32, hit_modifier: i32, element_addition: u32) -> Option<(MasterInfo, AttackInformation)> {
    ...
    if game.skill_random_below(100) < i32::from(combat.blast_attack) { attack.critical = true; let rate = game.globe_setup().critical_rate(); for power in &mut attack.damages { power.hp_damage = (power.hp_damage as f32 * rate).round_ties_even() as i32; } } Some((master, attack))
}
...
fn knock_back_owner<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, region_id: i32, back_steps: u32, move_speed: u32, runtime: &mut Runtime) {
    ...
    if position != original { let _ = game.force_move_player(player_id, position.x, position.y, move_speed.wrapping_mul(moved), runtime); }
}
...
pub(crate) fn execute_player_blood_rose<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    if !is_blood_rose_dispatch(dispatch) { return terminal(QueuedSkillExecutionState::Rejected) } let Some((region_id, source_x, source_y, level, initial_mana)) = game.find_player(player_id).and_then(|player| Some((player.server_region_id()?, player.shape().get_tile_x().ok()?, player.shape().get_tile_y().ok()?, player.learned_skill_level(BLOOD_ROSE_SKILL_ID), player.mana()))) else { return terminal(QueuedSkillExecutionState::Rejected) }; let Some(properties) = game.skill_base_properties(BLOOD_ROSE_SKILL_ID, level) else { finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    let Some((x, y, _)) = cell else { knock_back_owner(game, player_id, region_id, back_steps, move_speed, runtime); if let Some(state) = player_ai.blood_rose_mut() { let _ = state.kernel.advance(SkillStage::Attack, SkillStage::Apply); } player_ai.mark_blood_rose_used(runtime.now_milliseconds()); finish(game, player_id); return terminal(QueuedSkillExecutionState::Completed) }; let live_block = game.find_region(region_id).map_or(2, |owner| owner.base().skill_cell_block(x, y)); if let Some(state) = player_ai.blood_rose_mut() { state.end_tile = (x, y) }
