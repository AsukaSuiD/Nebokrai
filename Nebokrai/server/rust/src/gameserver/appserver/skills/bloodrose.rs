...
fn finish_player_blood_rose<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, runtime: &mut Runtime) {
    ...
    finish_summon_skill(game, player_id, ai, runtime, |ai, now_ms| ai.mark_skill_used(BLOOD_ROSE_SKILL_ID, now_ms));
}
...
pub(crate) fn complete_player_blood_rose<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, runtime: &mut Runtime) -> bool {
    let Some(dispatch) = ai.player_skill_state::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).map(|state| state.kernel().dispatch()) else { return false };
...
pub(crate) fn execute_player_blood_rose<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    if !is_blood_rose_dispatch(dispatch) { return terminal(QueuedSkillExecutionState::Rejected) } let Some((region_id, source_x, source_y, level, initial_mana)) = game.find_player(player_id).and_then(|player| Some((player.server_region_id()?, player.shape().get_tile_x().ok()?, player.shape().get_tile_y().ok()?, player.learned_skill_level(BLOOD_ROSE_SKILL_ID, game.skill_factory()), player.mana()))) else { return terminal(QueuedSkillExecutionState::Rejected) }; let Some(properties) = game.skill_base_properties(BLOOD_ROSE_SKILL_ID, level) else { if player_ai.player_skill_state::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).is_some() { abort_player_blood_rose(game, player_id); } return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    if player_ai.player_skill_state::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).is_none() { let now = runtime.now_milliseconds(); let self_target = match dispatch { PlayerSkillDispatch::SelfTarget { .. } => true, PlayerSkillDispatch::Object { target, .. } => target.object_type == PLAYER_TYPE && target.id == player_id, _ => false }; if self_target { send_failure(game, player_id, 10, mp_loss); return terminal(QueuedSkillExecutionState::Rejected) } if !skill_is_restored(player_ai.skill_last_used_ms(BLOOD_ROSE_SKILL_ID), reuse_delay, now) { send_failure(game, player_id, 0x0d, mp_loss); return terminal(QueuedSkillExecutionState::Rejected) } let Some(destination) = target_position(game, region_id, player_id, dispatch) else { return terminal(QueuedSkillExecutionState::Rejected) }; let path = game.base_magic_path(region_id, source_x, source_y, destination.0, destination.1, None); if max_distance != 0 && path.len() as u32 > max_distance { send_failure(game, player_id, 0x0b, mp_loss); return terminal(QueuedSkillExecutionState::Rejected) } let Some(player) = game.find_player(player_id) else { return terminal(QueuedSkillExecutionState::Rejected) }; if !weapon_is_crossbow(game, player) { send_failure(game, player_id, 0x0e, mp_loss); return terminal(QueuedSkillExecutionState::Rejected) } if mp_loss != 0 && (initial_mana.wrapping_sub(mp_loss) as i32) < 0 { send_failure(game, player_id, 7, mp_loss); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(false); player.set_current_skill_id(Some(BLOOD_ROSE_SKILL_ID)); } player_ai.begin_player_skill_execution(BloodRoseExecutionState::begin(dispatch, now, destination)); return terminal(QueuedSkillExecutionState::Begun); }
    else if player_ai.player_skill_state::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).is_none_or(|state| state.kernel.dispatch() != dispatch) { return terminal(QueuedSkillExecutionState::Rejected) }
    if player_ai.player_skill_state::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).is_some_and(|state| !state.condition_checked) { let mana = game.find_player(player_id).map_or(0, CPlayer::mana); if (mana.wrapping_sub(mp_loss) as i32) < 0 { send_failure(game, player_id, 7, mp_loss); abort_player_blood_rose(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_mana(mana.wrapping_sub(mp_loss)); } let _ = game.update_player_current_state(player_id, GamePlayerFightStatePhase::MoveShapeAi); if game.find_player(player_id).is_none_or(|player| !weapon_is_crossbow(game, player)) { send_failure(game, player_id, 0x0e, mp_loss); abort_player_blood_rose(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } let destination = player_ai.player_skill_state::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).map(|state| state.destination).unwrap_or_default(); if let Some(player) = game.find_player_mut(player_id) { player.movement_shape_mut().set_direction(get_line_direction(source_x, source_y, destination.0, destination.1)); } send_start(game, player_id, level); if let Some(state) = player_ai.player_skill_state_mut::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID) { state.condition_checked = true; let _ = state.kernel.advance(SkillStage::Begin, SkillStage::Check); } }
    let started = player_ai.player_skill_state::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).map(|state| state.kernel.started_at_ms()).unwrap_or_default(); if !player_ai.player_skill_state::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).is_some_and(|state| state.kernel().is_prepared()) { if !time_reached(runtime.now_milliseconds(), started, delay) { return terminal(QueuedSkillExecutionState::Pending) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(true); } let Some(destination) = target_position(game, region_id, player_id, dispatch) else { abort_player_blood_rose(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; let path = game.base_magic_path(region_id, source_x, source_y, destination.0, destination.1, (max_distance != 0).then_some(max_distance)); let index = path.iter().position(|cell| cell.2 == 2).unwrap_or(path.len()); let endpoint = path.get(index).or_else(|| path.last()).copied().unwrap_or((destination.0, destination.1, 2)); send_fire(game, region_id, player_id, level, dispatch, destination, cell_delay.wrapping_mul(index as u32)); if let Some(state) = player_ai.player_skill_state_mut::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID) { state.path = path; state.current_position = 1; state.end_tile = (endpoint.0, endpoint.1); state.visual_target = None; state.kernel_mut().mark_prepared(); let _ = state.kernel.advance(SkillStage::Check, SkillStage::Calculate); let _ = state.kernel.advance(SkillStage::Calculate, SkillStage::Attack); } }
    let Some((position, path_len, cell, end_sent)) = player_ai.player_skill_state::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).map(|state| (state.current_position, state.path.len(), state.path.get(state.current_position).copied(), state.end_sent)) else { return terminal(QueuedSkillExecutionState::Rejected) }; if !time_reached(runtime.now_milliseconds(), started, delay.wrapping_add(cell_delay.wrapping_mul(position as u32))) { return terminal(QueuedSkillExecutionState::Pending) }
    let Some((x, y, _)) = cell else { knock_back_owner(game, player_id, region_id, back_steps, move_speed); if let Some(state) = player_ai.player_skill_state_mut::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID) { let _ = state.kernel.advance(SkillStage::Attack, SkillStage::Apply); } finish_player_blood_rose(game, player_id, player_ai, runtime); return terminal(QueuedSkillExecutionState::Completed) }; let live_block = game.find_region(region_id).map_or(2, |owner| owner.base().skill_cell_block(x, y)); if let Some(state) = player_ai.player_skill_state_mut::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID) { state.end_tile = (x, y) }
    if live_block == 3 { let mut attacked = player_ai.player_skill_state_mut::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).map(|state| std::mem::take(&mut state.attacked)).unwrap_or_default(); let (any, visual) = attack_scope(game, player_id, region_id, level, damage_factor, hit_modifier, element_addition, x, y, &mut attacked, runtime); if let Some(state) = player_ai.player_skill_state_mut::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID) { state.attacked = attacked; if state.visual_target.is_none() { state.visual_target = visual; } } if any { let target = player_ai.player_skill_state::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID).and_then(|state| state.visual_target); send_end(game, player_id, level, (x, y), target); if let Some(state) = player_ai.player_skill_state_mut::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID) { state.end_sent = true; state.current_position = path_len.wrapping_add(1); } return terminal(QueuedSkillExecutionState::Pending) } }
    else if live_block == 2 { if !end_sent { send_end(game, player_id, level, (x, y), None); } if let Some(state) = player_ai.player_skill_state_mut::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID) { state.end_sent = true; state.current_position = path_len; } }
    if let Some(state) = player_ai.player_skill_state_mut::<BloodRoseExecutionState>(BLOOD_ROSE_SKILL_ID) { state.current_position = state.current_position.wrapping_add(1); } terminal(QueuedSkillExecutionState::Pending)
}
