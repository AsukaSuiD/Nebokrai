...
//! Прямой End: vtable 0x0066035C, слот +0x1C → 0x005FD420: visual
...
pub(crate) fn end_leaf_cut_2_state(
    ...
    let Some(state_id) = resolve_state_move_shape(game, region_id, holder)
        .and_then(|shape| shape.applied_state::<LeafCutState2>(key))
    ...
    let removed = resolve_state_move_shape_mut(game, region_id, holder)
        .and_then(|shape| shape.remove_applied_state_record::<LeafCutState2>(key, LEAF_CUT_2_STATE_BYTES))
        .is_some();
...
pub(crate) fn update_player_leaf_cut_2_state<Runtime: GameMainLoopRuntime>(
    ...
    match tick {
        ...
        LeafCutStateTick::Ended => {
            let _ = end_leaf_cut_2_state(game, region_id, identity, key);
