...
pub(crate) fn complete_player_lighting_arrow_2<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, runtime: &mut Runtime) -> bool {
    let Some(dispatch) = ai.player_skill_state::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID).map(|state| state.kernel().dispatch()) else { return false };
...
pub(crate) fn execute_player_lighting_arrow_2<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some(properties) = game.skill_base_properties(LIGHTING_ARROW_2_SKILL_ID, level) else { if ai.player_skill_state::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID).is_some() { abort_player_lighting_arrow_2(game, player_id) } return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    if ai.player_skill_state::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID).is_none() {
    ...
    if ai.player_skill_state::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID).is_some_and(|state| !state.condition_checked) {
        let mana = game.find_player(player_id).map_or(0, CPlayer::mana); if (mana.wrapping_sub(mp_loss) as i32) < 0 { failure(game, player_id, 7, mp_loss); abort_player_lighting_arrow_2(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_mana(mana.wrapping_sub(mp_loss)); } let _ = game.update_player_current_state(player_id, GamePlayerFightStatePhase::MoveShapeAi); if game.find_player(player_id).is_none_or(|player| !weapon_is_valid(game, player)) { failure(game, player_id, 0x0e, mp_loss); abort_player_lighting_arrow_2(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.movement_shape_mut().set_direction(get_line_direction(source_x, source_y, destination.0, destination.1)); } send_start(game, player_id, level); if let Some(state) = ai.player_skill_state_mut::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID) { state.condition_checked = true; let _ = state.kernel_mut().advance(SkillStage::Begin, SkillStage::Check); }
    }
    let started = ai.player_skill_state::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID).map(|state| state.kernel().started_at_ms()).unwrap_or_default();
    if ai.player_skill_state::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID).is_some_and(|state| !state.kernel().is_prepared()) {
        if !time_reached(runtime.now_milliseconds(), started, delay) { return terminal(QueuedSkillExecutionState::Pending) } restore_player_movement(game, player_id); let path = game.base_magic_path(region_id, source_x, source_y, destination.0, destination.1, Some(maximum)); if path.is_empty() { send_broken(game, player_id, level); abort_player_lighting_arrow_2(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } if maximum != 0 && path.len() > maximum.wrapping_add(1) as usize { failure(game, player_id, 0x0b, mp_loss); abort_player_lighting_arrow_2(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } let count = path.iter().position(|cell| cell.2 == 2).unwrap_or(path.len()); let endpoint = path.get(count).or_else(|| path.last()).copied().unwrap_or((destination.0, destination.1, 2)); let visual_destination = if matches!(dispatch, PlayerSkillDispatch::Object { .. }) { destination } else { (endpoint.0, endpoint.1) }; send_fire(game, player_id, level, dispatch, visual_destination, missile_time); if let Some(state) = ai.player_skill_state_mut::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID) { state.path = path; state.attack_cell_count = count; state.current_cell = 0; state.kernel_mut().mark_prepared(); let _ = state.kernel_mut().advance(SkillStage::Check, SkillStage::Calculate); let _ = state.kernel_mut().advance(SkillStage::Calculate, SkillStage::Attack); }
    }
    let Some((current, count, cell)) = ai.player_skill_state::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID).map(|state| (state.current_cell, state.attack_cell_count, state.path.get(state.current_cell).copied())) else { return terminal(QueuedSkillExecutionState::Rejected) }; if current >= count { if let Some(state) = ai.player_skill_state_mut::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID) { let _ = state.kernel_mut().advance(SkillStage::Attack, SkillStage::Apply); } finish_player_lighting_arrow_2(game, player_id, ai, runtime); return terminal(QueuedSkillExecutionState::Completed) } if !time_reached(runtime.now_milliseconds(), started, delay.wrapping_add(missile_time.wrapping_mul(current as u32))) { return terminal(QueuedSkillExecutionState::Pending) }
    if let Some((x, y, _)) = cell { let mut attacked = ai.player_skill_state_mut::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID).map(|state| std::mem::take(&mut state.attacked_creatures)).unwrap_or_default(); attack_cell(game, player_id, region_id, level, factor, hit, x, y, &mut attacked, runtime); if let Some(state) = ai.player_skill_state_mut::<LightingArrow2ExecutionState>(LIGHTING_ARROW_2_SKILL_ID) { state.attacked_creatures = attacked; state.current_cell = state.current_cell.wrapping_add(1); } }
