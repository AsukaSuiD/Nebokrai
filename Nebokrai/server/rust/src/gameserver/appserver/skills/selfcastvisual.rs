...
//! и X/Y. У ManaShield/MachineShield выпуск вместо повторной identity/X/Y
//! заканчивается двумя LONG0. S и точка Begin не используются.
//! Ошибки 2/7/13 общие; Roar добавляет 14, MeteorArrowMass — 4/14,
//! Pillar и Callosity — 8, ManaShield/MachineShield — 8/14.
//! Только у Pillar/Callosity ошибка 8 имеет LONG0/BYTE8 вместо BYTE0/BYTEcode.
//! Другие режимы не создают пакет.
...
pub(crate) fn publish_self_cast_visual(game: &CGame, skill: &MoveShapeSkill, mode: u32) {
    ...
    let shield = matches!(skill.owner(), SkillOwner::CManaShield | SkillOwner::CMachineShield);
    ...
    if matches!(mode, 2 | 7 | 13) || long_failure
        ...
        || (skill.owner() == SkillOwner::CMeteorArrowMass && matches!(mode, 4 | 14))
        || (shield && matches!(mode, 8 | 14))
    {
