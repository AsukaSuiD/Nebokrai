    ...
    match tick {
        ...
        LeafCutStateTick::Ended => {
            if let Some(player) = game.find_player_mut(player_id) {
                ...
                let _ = move_shape.remove_applied_state_record::<LeafCutState>(key, LEAF_CUT_STATE_BYTES);
            }
            send_leaf_cut_state_visual(game, region_id, identity, x, y, state, false, lifetime_now_ms);
