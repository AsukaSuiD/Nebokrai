...
fn finish_player_rush<Runtime: GameMainLoopRuntime>(
    ...
    finish_rush_owner(game, player_id, RUSH_SKILL_ID, runtime);
}
...
pub(crate) fn cancel_player_rush<Runtime: GameMainLoopRuntime>(
    ...
    let Some(dispatch) = game.player_skill_execution(player_id, RUSH_SKILL_ID).map(SkillExecutionKernel::dispatch) else {
        return false;
    };
    finish_player_rush(game, player_id, runtime);
...
pub(crate) fn execute_player_rush<Runtime: GameMainLoopRuntime>(
    ...
    let Some(properties) = game.skill_base_properties(RUSH_SKILL_ID, level) else {
        if game.player_skill_execution(player_id, RUSH_SKILL_ID).is_some() { finish_player_rush(game, player_id, runtime) }
        return terminal(QueuedSkillExecutionState::Rejected);
    };
    ...
    if game.player_skill_execution(player_id, RUSH_SKILL_ID).is_none() {
        let now_ms = runtime.now_milliseconds();
        if !skill_is_restored(game.player_skill_last_used_ms(player_id, RUSH_SKILL_ID), reuse, now_ms) {
    ...
    } else if game.player_skill_execution(player_id, RUSH_SKILL_ID).is_none_or(|execution| execution.dispatch() != dispatch) {
        return terminal(QueuedSkillExecutionState::Rejected);
    }
    ...
    if let Some(execution) = game.player_skill_execution_mut(player_id, RUSH_SKILL_ID) {
