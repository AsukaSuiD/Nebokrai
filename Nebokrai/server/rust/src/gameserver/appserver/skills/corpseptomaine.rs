...
fn finish_player<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, runtime: &mut Runtime) { restore_player(game, player_id); finish_summon_skill_without_weapon_wear(game, player_id, ai, runtime, |ai, now_ms| ai.mark_corpse_ptomaine_used(now_ms)); }
