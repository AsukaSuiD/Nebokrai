...
pub(crate) fn execute_player_scorpion<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, player_ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    if player_ai.player_skill_state::<ScorpionExecutionState>(SCORPION_SKILL_ID).is_none() { let now = runtime.now_milliseconds(); let Some(identity) = target(dispatch) else { return reject(game, player_id, level, Some((10, mp, Some(b"GS0286")))) }; if !skill_is_restored(player_ai.skill_last_used_ms(SCORPION_SKILL_ID), reuse, now) { return reject(game, player_id, level, Some((0x0d, mp, None))) } let Some((target_x, target_y, _)) = target_snapshot(game, region_id, identity) else { return reject(game, player_id, level, Some((10, mp, Some(b"GS0286")))) }; let path = game.base_magic_path(region_id, source_x, source_y, target_x, target_y, None); if max_distance != 0 && path.len() as u32 > max_distance { return reject(game, player_id, level, Some((0x0b, mp, None))) } let Some(player) = game.find_player(player_id) else { return terminal(QueuedSkillExecutionState::Rejected) }; if !weapon_is_crossbow(game, player) { return reject(game, player_id, level, Some((0x0e, mp, None))) } if mp != 0 && (initial_mana.wrapping_sub(mp) as i32) < 0 { return reject(game, player_id, level, Some((7, mp, None))) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(false); player.set_current_skill_id(Some(SCORPION_SKILL_ID)); } player_ai.begin_player_skill_execution(ScorpionExecutionState::begin(dispatch, now)); return terminal(QueuedSkillExecutionState::Begun); }
    else if player_ai.player_skill_state::<ScorpionExecutionState>(SCORPION_SKILL_ID).is_none_or(|state| state.kernel.dispatch() != dispatch) { return terminal(QueuedSkillExecutionState::Rejected) }
    ...
    if player_ai.player_skill_state::<ScorpionExecutionState>(SCORPION_SKILL_ID).is_some_and(|state| !state.condition_checked) { let mana = game.find_player(player_id).map_or(0, CPlayer::mana); if (mana.wrapping_sub(mp) as i32) < 0 { return reject(game, player_id, level, Some((7, mp, None))) } if let Some(player) = game.find_player_mut(player_id) { player.set_mana(mana.wrapping_sub(mp)); } let _ = game.update_player_current_state(player_id, GamePlayerFightStatePhase::MoveShapeAi); if game.find_player(player_id).is_none_or(|player| !weapon_is_crossbow(game, player)) { return reject(game, player_id, level, Some((0x0e, mp, None))) } if let Some(player) = game.find_player_mut(player_id) { player.movement_shape_mut().set_direction(get_line_direction(source_x, source_y, target_x, target_y)); } let path = game.base_magic_path(region_id, source_x, source_y, target_x, target_y, None); if max_distance != 0 && path.len() as u32 > max_distance { return reject(game, player_id, level, Some((0x0b, mp, None))) } if path.iter().any(|cell| cell.2 == 2) { let name = target_name(game, region_id, identity).to_vec(); return reject(game, player_id, level, Some((0x0f, mp, Some(&name)))) } send_start(game, player_id, level); if let Some(state) = player_ai.player_skill_state_mut::<ScorpionExecutionState>(SCORPION_SKILL_ID) { state.condition_checked = true; let _ = state.kernel.advance(SkillStage::Begin, SkillStage::Check); } }
    let started = player_ai.player_skill_state::<ScorpionExecutionState>(SCORPION_SKILL_ID).map(|state| state.kernel.started_at_ms()).unwrap_or_default(); let attacks = player_ai.player_skill_state::<ScorpionExecutionState>(SCORPION_SKILL_ID).map_or(0, |state| state.attacks); let deadline = match attacks { 0 => delay.wrapping_add(first), 1 => delay.wrapping_add(first).wrapping_add(second), _ => delay.wrapping_add(first).wrapping_add(second).wrapping_add(third) }; if !time_reached(runtime.now_milliseconds(), started, deadline) { return terminal(QueuedSkillExecutionState::Pending) }
    if attacks == 0 { send_strike(game, player_id, level, identity, (target_x, target_y), second, third); apply_attack(game, player_id, region_id, identity, level, hit_modifier, None, runtime); if let Some(state) = player_ai.player_skill_state_mut::<ScorpionExecutionState>(SCORPION_SKILL_ID) { state.attacks = 1; let _ = state.kernel.advance(SkillStage::Check, SkillStage::Calculate); let _ = state.kernel.advance(SkillStage::Calculate, SkillStage::Attack); } return terminal(QueuedSkillExecutionState::Pending) }
    if attacks == 1 { apply_attack(game, player_id, region_id, identity, level, hit_modifier, None, runtime); if let Some(state) = player_ai.player_skill_state_mut::<ScorpionExecutionState>(SCORPION_SKILL_ID) { state.attacks = 2; } return terminal(QueuedSkillExecutionState::Pending) }
    apply_attack(game, player_id, region_id, identity, level, hit_modifier, Some(final_factor), runtime); if let Some(state) = player_ai.player_skill_state_mut::<ScorpionExecutionState>(SCORPION_SKILL_ID) { let _ = state.kernel.advance(SkillStage::Attack, SkillStage::Apply); } finish_player_scorpion(game, player_id, player_ai, runtime); terminal(QueuedSkillExecutionState::Completed)
}
