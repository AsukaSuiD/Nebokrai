//! Прицельный полёт Strike и YakshaSlash.
//! Источник: gameserver.exe/GameServer.pdb, appserver/skills/strike.cpp
//! и yakshaslash.cpp. Player и Monster используют один зарегистрированный
//! progress; kernel хранит общие U/S, фазу, CAN, prepared и время Begin.
//!
//! Каждый AI захватывает таблицу и полные U/S до callbacks; NULL, смерть S
//! либо самонаведение вызывают End(0). Только Strike списывает MP и публикует
//! OnChangeStates. Затем CAN→S.Y/X→U.Y/X→направление→visual0→condition.
//! Выпуск ждёт unsigned start+delay, включает Move1 и строит свежий путь.
//! Strike проверяет дальность и именованное препятствие, Yaksha — только
//! BLOCK_UNFLY. Flight равен числу клеток, умноженному на время клетки;
...
pub(crate) fn run_targeted_projectile_ai<Runtime: GameMainLoopRuntime>(
    ...
    let strike = match skill.owner() {
        SkillOwner::CStrike => true, SkillOwner::CYakshaSlash => false,
    ...
    if target_dead || targets_self {
        ...
        if strike && let Some(player) = player {
            game.send_skill_system_info(player, if target_dead { b"GS0285" } else { b"GS0286" });
        }
        return terminal(QueuedSkillExecutionState::Rejected);
    }
    if stage == SkillStage::Begin {
        if strike && !spend_cast_mana(game, instance, player, &properties) {
            return terminal(QueuedSkillExecutionState::Rejected);
        }
    ...
    if strike { apply_strike_attack(game, instance, user, target, &properties, runtime); }
    else { apply_yaksha_slash_attack(game, instance, user, target, runtime); }
...
pub(super) fn execute_player_targeted_projectile<Runtime: GameMainLoopRuntime>(
    ...
    execute_registered_player_cast(
        ...
        |game, instance, _, runtime| {
            ...
            let accepted = match game.registered_skill(instance).map(MoveShapeSkill::owner) {
...
pub(crate) fn publish_targeted_projectile_visual(game: &CGame, skill: &MoveShapeSkill, mode: u32) {
    if !matches!(skill.owner(), SkillOwner::CStrike | SkillOwner::CYakshaSlash)
