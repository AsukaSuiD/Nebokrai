...
//! Общий с `CSkeletonArchery` `AfterUseSkill` при `End(true)` один раз вызывает
//! `CPlayer::OnWeaponDamaged`; отменяющий `End(false)` этот хвост не выполняет.
...
//! восстановления. Поле `m_bAutoRestart` конструктор оставляет нулевым, все
//! `Begin`-перегрузки его не меняют, а унаследованный `CState::Restart` является
//! пустым callback; отдельной достижимой runtime-семантики у этой ветви нет.
