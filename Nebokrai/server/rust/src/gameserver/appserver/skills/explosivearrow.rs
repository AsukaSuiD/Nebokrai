...
//! Все три варианта усекают критический float-множитель к нулю перед `int`.
...
fn calculate_attack(game: &mut CGame, player_id: i32, target_level: u8, skill_id: u32, level: i32, factor: u32, hit: i32, element_addition: u32) -> Option<(MasterInfo, AttackInformation)> {
    let player = game.find_player(player_id)?; let combat = player.combat_properties(); let master = master_info(player); let (divisor, minimum_factor) = game.globe_setup().weapon_damage_factors(); let weapon_factor = player.weapon_modifier(game.goods_factory(), i32::from(target_level), divisor, minimum_factor); let width = (combat.maximum_attack as i32).wrapping_sub(combat.minimum_attack as i32).wrapping_abs().wrapping_add(1); let physical = (combat.minimum_attack as i32).wrapping_add(game.skill_random_below(width)).max(0);
...
pub(crate) fn execute_player_explosive_arrow<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    ...
    let Some((x, y, _)) = cell else { knock_back_owner(game, player_id, region_id, back_steps, move_speed); if let Some(state) = ai.explosive_arrow_mut() { let _ = state.kernel.advance(SkillStage::Attack, SkillStage::Apply); } finish_player_explosive_arrow(game, player_id, ai, variant, runtime); return terminal(QueuedSkillExecutionState::Completed) }; let live_block = game.find_region(region_id).map_or(BLOCK_UNFLY, |owner| owner.base().skill_cell_block(x, y)); if let Some(state) = ai.explosive_arrow_mut() { state.end_tile = (x, y) }
