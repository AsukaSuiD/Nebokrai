...
pub(crate) fn execute_player_explosive_arrow<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    let Some(variant) = explosive_arrow_variant(dispatch) else { return terminal(QueuedSkillExecutionState::Rejected) }; let skill_id = variant.skill_id(); let Some((region_id, source_x, source_y, level, initial_mana)) = game.find_player(player_id).and_then(|player| Some((player.server_region_id()?, player.shape().get_tile_x().ok()?, player.shape().get_tile_y().ok()?, player.learned_skill_level(skill_id), player.mana()))) else { return terminal(QueuedSkillExecutionState::Rejected) }; let Some(properties) = game.skill_base_properties(skill_id, level) else { finish(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    let Some((x, y, _)) = cell else { knock_back_owner(game, player_id, region_id, back_steps, move_speed, runtime); if let Some(state) = ai.explosive_arrow_mut() { let _ = state.kernel.advance(SkillStage::Attack, SkillStage::Apply); } ai.mark_explosive_arrow_used(variant, runtime.now_milliseconds()); finish(game, player_id); return terminal(QueuedSkillExecutionState::Completed) }; let live_block = game.find_region(region_id).map_or(BLOCK_UNFLY, |owner| owner.base().skill_cell_block(x, y)); if let Some(state) = ai.explosive_arrow_mut() { state.end_tile = (x, y) }
