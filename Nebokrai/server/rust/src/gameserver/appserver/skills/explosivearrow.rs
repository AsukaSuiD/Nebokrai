...
pub(crate) fn execute_player_explosive_arrow<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    let Some(variant) = explosive_arrow_variant(dispatch) else { return terminal(QueuedSkillExecutionState::Rejected) }; let skill_id = variant.skill_id(); let Some((region_id, source_x, source_y, level, initial_mana)) = game.find_player(player_id).and_then(|player| Some((player.server_region_id()?, player.shape().get_tile_x().ok()?, player.shape().get_tile_y().ok()?, player.learned_skill_level(skill_id, game.skill_factory()), player.mana()))) else { return terminal(QueuedSkillExecutionState::Rejected) }; let Some(properties) = game.skill_base_properties(skill_id, level) else { if ai.player_skill_state::<ExplosiveArrowExecutionState>(dispatch.skill_id()).is_some() { abort_player_explosive_arrow(game, player_id); } return terminal(QueuedSkillExecutionState::Rejected) };
