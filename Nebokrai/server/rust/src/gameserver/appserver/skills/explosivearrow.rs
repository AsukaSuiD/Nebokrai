...
impl ExplosiveArrowVariant {
    ...
    pub(crate) const fn index(self) -> usize { match self { Self::Bow => 0, Self::WarSoul => 1, Self::Crossbow => 2 } }
...
pub(crate) struct ExplosiveArrowExecutionState {
    ...
    condition_checked: bool, attacking_started: bool, path: Vec<(i32, i32, u8)>,
...
impl ExplosiveArrowExecutionState {
    fn begin(variant: ExplosiveArrowVariant, dispatch: PlayerSkillDispatch, started_at_ms: u32, destination: (i32, i32)) -> Self { Self { kernel: SkillExecutionKernel::begin(dispatch, started_at_ms), variant, condition_checked: false, attacking_started: false, path: Vec::new(), current_position: 0, destination, end_tile: (0, 0), visual_target: None, attacked: Vec::new(), end_sent: false } }
...
pub(crate) fn complete_player_explosive_arrow<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, runtime: &mut Runtime) -> bool {
    let Some((dispatch, variant)) = ai.explosive_arrow().map(|state| (state.kernel().dispatch(), state.variant)) else { return false };
...
pub(crate) fn cancel_player_explosive_arrow<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, ai: &mut CPlayerAI, _runtime: &mut Runtime) -> bool {
    let Some(dispatch) = ai.explosive_arrow().map(|state| state.kernel().dispatch()) else { return false };
...
fn calculate_attack(game: &mut CGame, player_id: i32, target_level: u8, skill_id: u32, level: i32, factor: u32, hit: i32, element_addition: u32) -> Option<(MasterInfo, AttackInformation)> {
    ...
    if game.skill_random_below(100) < i32::from(combat.cch) { attack.critical = true; let rate = game.globe_setup().critical_rate(); for power in &mut attack.damages { power.hp_damage = (power.hp_damage as f32 * rate).round_ties_even() as i32; } } Some((master, attack))
}
...
pub(crate) fn execute_player_explosive_arrow<Runtime: GameMainLoopRuntime>(game: &mut CGame, player_id: i32, dispatch: PlayerSkillDispatch, ai: &mut CPlayerAI, runtime: &mut Runtime) -> QueuedSkillExecutionOutcome {
    let Some(variant) = explosive_arrow_variant(dispatch) else { return terminal(QueuedSkillExecutionState::Rejected) }; let skill_id = variant.skill_id(); let Some((region_id, source_x, source_y, level, initial_mana)) = game.find_player(player_id).and_then(|player| Some((player.server_region_id()?, player.shape().get_tile_x().ok()?, player.shape().get_tile_y().ok()?, player.learned_skill_level(skill_id), player.mana()))) else { return terminal(QueuedSkillExecutionState::Rejected) }; let Some(properties) = game.skill_base_properties(skill_id, level) else { if ai.explosive_arrow().is_some() { abort_player_explosive_arrow(game, player_id); } return terminal(QueuedSkillExecutionState::Rejected) };
    ...
    if ai.explosive_arrow().is_none() { let now = runtime.now_milliseconds(); let self_target = matches!(dispatch, PlayerSkillDispatch::SelfTarget { .. }) || matches!(dispatch, PlayerSkillDispatch::Object { target: ShapeIdentity { object_type: PLAYER_TYPE, id, .. }, .. } if id == player_id); if self_target { send_failure(game, player_id, 10, mp_loss, Some(b"GS0286")); return terminal(QueuedSkillExecutionState::Rejected) } if ai.explosive_arrow_last_used_ms(variant) != 0 && !time_reached(now, ai.explosive_arrow_last_used_ms(variant), reuse) { send_failure(game, player_id, 0x0d, mp_loss, Some(b"GS0278")); return terminal(QueuedSkillExecutionState::Rejected) } let Some(destination) = target_position(game, region_id, player_id, dispatch) else { return terminal(QueuedSkillExecutionState::Rejected) }; let path = game.base_magic_path(region_id, source_x, source_y, destination.0, destination.1, None); if maximum_distance != 0 && path.len() as u32 > maximum_distance { send_failure(game, player_id, 0x0b, mp_loss, Some(b"GS0290")); return terminal(QueuedSkillExecutionState::Rejected) } let Some(player) = game.find_player(player_id) else { return terminal(QueuedSkillExecutionState::Rejected) }; if let Some(text) = required_weapon_failure(game, player, variant) { send_failure(game, player_id, 0x0e, mp_loss, Some(text)); return terminal(QueuedSkillExecutionState::Rejected) } if mp_loss != 0 && (initial_mana.wrapping_sub(mp_loss) as i32) < 0 { send_failure(game, player_id, 7, mp_loss, Some(b"GS0288")); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(false); player.set_current_skill_id(Some(skill_id)); } ai.begin_explosive_arrow(ExplosiveArrowExecutionState::begin(variant, dispatch, now, destination)); }
    else if ai.explosive_arrow().is_none_or(|state| state.kernel.dispatch() != dispatch || state.variant != variant) { return terminal(QueuedSkillExecutionState::Rejected) }
    if ai.explosive_arrow().is_some_and(|state| !state.condition_checked) { let mana = game.find_player(player_id).map_or(0, CPlayer::mana); if (mana.wrapping_sub(mp_loss) as i32) < 0 { send_failure(game, player_id, 7, mp_loss, Some(b"GS0288")); abort_player_explosive_arrow(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } if let Some(player) = game.find_player_mut(player_id) { player.set_mana(mana.wrapping_sub(mp_loss)); } let _ = game.update_player_current_state(player_id, GamePlayerFightStatePhase::MoveShapeAi); let late_failure = game.find_player(player_id).and_then(|player| required_weapon_failure(game, player, variant)); if let Some(text) = late_failure { send_failure(game, player_id, 0x0e, mp_loss, Some(text)); abort_player_explosive_arrow(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) } let destination = ai.explosive_arrow().map(|state| state.destination).unwrap_or_default(); if let Some(player) = game.find_player_mut(player_id) { player.movement_shape_mut().set_direction(get_line_direction(source_x, source_y, destination.0, destination.1)); } send_start(game, player_id, skill_id, level); if let Some(state) = ai.explosive_arrow_mut() { state.condition_checked = true; let _ = state.kernel.advance(SkillStage::Begin, SkillStage::Check); } }
    let started = ai.explosive_arrow().map(|state| state.kernel.started_at_ms()).unwrap_or_default(); if !ai.explosive_arrow().is_some_and(|state| state.attacking_started) { if !time_reached(runtime.now_milliseconds(), started, delay) { return terminal(QueuedSkillExecutionState::Pending) } if let Some(player) = game.find_player_mut(player_id) { player.set_skill_moveable(true); } let Some(destination) = target_position(game, region_id, player_id, dispatch) else { abort_player_explosive_arrow(game, player_id); return terminal(QueuedSkillExecutionState::Rejected) }; let path = game.base_magic_path(region_id, source_x, source_y, destination.0, destination.1, (maximum_distance != 0).then_some(maximum_distance)); let index = path.iter().position(|cell| cell.2 == BLOCK_UNFLY).unwrap_or(path.len()); let endpoint = path.get(index).or_else(|| path.last()).copied().unwrap_or((destination.0, destination.1, BLOCK_UNFLY)); send_fire(game, region_id, player_id, skill_id, level, dispatch, destination, missile_step.wrapping_mul(index as u32)); if let Some(state) = ai.explosive_arrow_mut() { state.path = path; state.current_position = 1; state.end_tile = (endpoint.0, endpoint.1); state.attacking_started = true; let _ = state.kernel.advance(SkillStage::Check, SkillStage::Calculate); let _ = state.kernel.advance(SkillStage::Calculate, SkillStage::Attack); } }
    let Some((position, path_len, cell, end_sent)) = ai.explosive_arrow().map(|state| (state.current_position, state.path.len(), state.path.get(state.current_position).copied(), state.end_sent)) else { return terminal(QueuedSkillExecutionState::Rejected) }; if !time_reached(runtime.now_milliseconds(), started, delay.wrapping_add(missile_step.wrapping_mul(position as u32))) { return terminal(QueuedSkillExecutionState::Pending) }
    let Some((x, y, _)) = cell else { knock_back_owner(game, player_id, region_id, back_steps, move_speed, runtime); if let Some(state) = ai.explosive_arrow_mut() { let _ = state.kernel.advance(SkillStage::Attack, SkillStage::Apply); } finish_player_explosive_arrow(game, player_id, ai, variant, runtime); return terminal(QueuedSkillExecutionState::Completed) }; let live_block = game.find_region(region_id).map_or(BLOCK_UNFLY, |owner| owner.base().skill_cell_block(x, y)); if let Some(state) = ai.explosive_arrow_mut() { state.end_tile = (x, y) }
    if live_block == BLOCK_SHAPE { let mut attacked = ai.explosive_arrow_mut().map(|state| std::mem::take(&mut state.attacked)).unwrap_or_default(); let (any, visual) = attack_scope(game, player_id, region_id, variant, level, factor, hit, element_addition, (x, y), &mut attacked, runtime); if let Some(state) = ai.explosive_arrow_mut() { state.attacked = attacked; if state.visual_target.is_none() { state.visual_target = visual; } } if any { let target = ai.explosive_arrow().and_then(|state| state.visual_target); send_end(game, player_id, skill_id, level, (x, y), target); if let Some(state) = ai.explosive_arrow_mut() { state.end_sent = true; state.current_position = path_len.wrapping_add(1); } return terminal(QueuedSkillExecutionState::Pending) } }
    else if live_block == BLOCK_UNFLY { if !end_sent { send_end(game, player_id, skill_id, level, (x, y), None); } if let Some(state) = ai.explosive_arrow_mut() { state.end_sent = true; state.current_position = path_len; } }
    if let Some(state) = ai.explosive_arrow_mut() { state.current_position = state.current_position.wrapping_add(1); } terminal(QueuedSkillExecutionState::Pending)
}
