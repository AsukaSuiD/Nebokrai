...
impl CGame {
    pub(crate) fn add_fire_wall_phalanx<Runtime: GameMainLoopRuntime>(
        &mut self,
        region_id: i32,
        ...
        let result = owner.base_mut().add_fire_wall_phalanx(
            phalanx,
            tile_x,
            tile_y,
            self.area_width,
    ...
    pub(crate) fn send_fire_wall_phalanx_entry(
        &mut self,
        region_id: i32,
        phalanx_id: i32,
    ) -> Option<()> {
        let phalanx = self.find_region(region_id)?.base().find_skill_phalanx(phalanx_id)?;
        let SummonedSkillShape::FireWall(phalanx) = phalanx else { return None };
