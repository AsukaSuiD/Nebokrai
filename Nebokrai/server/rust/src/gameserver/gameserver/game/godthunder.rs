...
impl CGame {
    ...
    pub(crate) fn add_god_thunder_2_phalanx<R: GameMainLoopRuntime>(
        &mut self, region_id: i32, mut phalanx: CGodThunderPhalanx2,
        x: i32, y: i32, now_ms: u32, runtime: &mut R,
    ) -> Option<Result<i32, RegionMembershipBlock>> {
        let mut random = |maximum| game_legacy_random(&mut self.random_state, maximum);
        phalanx.initialize(x, y, &mut random);
    ...
    pub(super) fn god_thunder_2_targets(&self, region_id: i32, phalanx: &CGodThunderPhalanx2) -> Vec<(ShapeIdentity, bool)> {
        let Some(region) = self.find_region(region_id).map(ServerRegionOwner::base) else { return Vec::new() };
        let mut targets = Vec::new();
        for (x, y) in phalanx.attack_cells() {
            for (&player_id, _) in &region.war_souls_at(x, y) {
                ...
                if identity.id != phalanx.master().master_id
                    && self.find_player(identity.id).is_some_and(|player| !player.is_dead())
                    && self.owned_player_skill_target_attackable(phalanx.master(), identity, region_id)
                { targets.push((identity, true)); }
            }
            let mut shapes = Vec::new(); if region.get_shapes(x, y, self.area_width, self.area_height, self, &mut shapes).is_err() { continue; }
            for shape in shapes { let identity = shape.identity;
                if identity != phalanx.shape().identity()
                    && !(identity.object_type == phalanx.master().master_type && identity.id == phalanx.master().master_id)
                    && matches!(identity.object_type, PLAYER_TYPE | MONSTER_TYPE)
                    && self.owned_player_skill_target_attackable(phalanx.master(), identity, region_id)
    ...
    pub(super) fn calculate_god_thunder_2_attack(&mut self, phalanx: &CGodThunderPhalanx2, target_level: u8) -> Option<(AttackInformation, PlayerCombatProperties, u8, u8)> {
        let player = self.find_player(phalanx.master().master_id)?; let combat = player.combat_properties(); let occupation = player.occupation(); let level = player.level();
        let weapon = player.equipment().get_goods(2).map_or(0, |goods| goods.addon_property_value(self.goods_factory(), GAP_WEAPON_DAMAGE_LEVEL, 1));
        let (divisor, minimum) = self.globe_setup.weapon_damage_factors(); let delta = weapon.wrapping_sub(i32::from(target_level)).max(0);
        let factor = if divisor == 0.0 { 1.0 } else { (delta as f32 / divisor).min(1.0).max(minimum) };
        let critical = self.globe_setup.critical_rate(); let mut random = |maximum| game_legacy_random(&mut self.random_state, maximum);
        Some(phalanx.calculate_attack(combat, occupation, level, factor, critical, &mut random))
    }
}
