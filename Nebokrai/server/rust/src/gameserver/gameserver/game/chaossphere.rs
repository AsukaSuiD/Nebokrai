...
//! `chaosspherephalanx.rs`. Здесь остаются регистрация в регионе, точный обход
//! «боевой дух перед фигурой», пространственный `ForceMove` и применение
//! атак к независимым владельцам.
...
impl CGame {
    ...
    pub(super) fn chaos_sphere_targets(
        &self,
        region_id: i32,
        phalanx: &CChaosSpherePhalanx,
        center_x: i32,
        ...
        for (tile_x, tile_y) in CChaosSpherePhalanx::scope_cells(center_x, center_y) {
            if region.block_at(tile_x, tile_y) != Some(2) {
                for (&player_id, _) in &region.war_souls_at(tile_x, tile_y) {
                    ...
                    {
                        targets.push((ShapeIdentity { object_type: PLAYER_TYPE, id: player_id, ex_id: CGuid::GUID_INVALID }, true));
                    }
                }
            }
