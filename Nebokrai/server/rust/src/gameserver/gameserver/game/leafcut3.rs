//! Borrow-координация периодического состояния `CLeafCutState3`.
//!
//! Формула, RNG и DB-кодек принадлежат `leafcutstate3.rs`. Здесь состояние
...
impl CGame {
    pub(super) fn update_player_leaf_cut_3_state<Runtime: GameMainLoopRuntime>(
        &mut self,
        player_id: i32,
        runtime: &mut Runtime,
    ) -> bool {
        let target = self.find_player_mut(player_id).and_then(|player| {
            ...
            let state = player.take_leaf_cut_3_state_for_ai()?;
        ...
        let tick = {
            ...
            state.tick(lifetime_now, frequency_now, dead, critical_chance, critical_rate, &mut random)
        };
        match tick {
            LeafCutStateTick::Pending => {
                if let Some(player) = self.find_player_mut(player_id) { player.restore_leaf_cut_3_state_after_ai(state); }
            }
            ...
            LeafCutStateTick::Ended => {
                if let Some(player) = self.find_player_mut(player_id) { player.finish_leaf_cut_3_state(state); }
                send_leaf_cut_3_state_visual(self, region_id, identity, x, y, state, false, lifetime_now);
    ...
    pub(super) fn update_monster_leaf_cut_3_state<Runtime: GameMainLoopRuntime>(
        &mut self,
        region_id: i32,
        monster_id: i32,
        runtime: &mut Runtime,
        ...
        let state_and_target = owner.base_mut().find_monster_by_id_mut(monster_id).and_then(|monster| {
            let shape = monster.move_shape().shape();
            let identity = shape.identity();
            let x = shape.get_tile_x().ok()?;
            let y = shape.get_tile_y().ok()?;
            let dead = monster.hit_points() == 0;
            let state = monster.move_shape_mut().take_leaf_cut_3_state_for_ai()?;
            Some((state, identity, x, y, dead))
        });
        self.restore_region_owner(owner);
        let Some((mut state, identity, x, y, dead)) = state_and_target else { return false };
        let lifetime_now = runtime.now_milliseconds();
        let frequency_now = runtime.now_milliseconds();
        let critical_rate = self.globe_setup.critical_rate();
        let tick = {
            let mut random = |maximum| game_legacy_random(&mut self.random_state, maximum);
            // В слоте `CMonster::GetCCH` стоит базовая реализация `CMoveShape`,
            // которая возвращает ноль.
            state.tick(lifetime_now, frequency_now, dead, 0, critical_rate, &mut random)
        };
        match tick {
            LeafCutStateTick::Pending => {
                if let Some(mut owner) = self.take_region_owner(region_id) {
                    if let Some(monster) = owner.base_mut().find_monster_by_id_mut(monster_id) { monster.move_shape_mut().restore_leaf_cut_3_state_after_ai(state); }
                    self.restore_region_owner(owner);
                }
            }
            LeafCutStateTick::Attack(attack) => {
            ...
            LeafCutStateTick::Ended => {
                if let Some(mut owner) = self.take_region_owner(region_id) {
                    if let Some(monster) = owner.base_mut().find_monster_by_id_mut(monster_id) { monster.move_shape_mut().finish_leaf_cut_3_state(state); }
                    self.restore_region_owner(owner);
                }
                send_leaf_cut_3_state_visual(self, region_id, identity, x, y, state, false, lifetime_now);
            }
