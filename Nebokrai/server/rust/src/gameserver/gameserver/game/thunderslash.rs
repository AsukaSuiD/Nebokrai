...
impl CGame {
    pub(crate) fn add_thunder_slash_phalanx<Runtime: GameMainLoopRuntime>(
    ...
    pub(crate) fn send_thunder_slash_phalanx_entry<Runtime: GameMainLoopRuntime>(
        ...
        let SummonedSkillShape::ThunderSlash(phalanx) = phalanx else { return None };
        let identity = phalanx.shape().identity(); let (x, y) = phalanx.tile();
