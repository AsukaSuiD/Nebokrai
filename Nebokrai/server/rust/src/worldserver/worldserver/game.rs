...
use crate::worldserver::appworld::message::organsysmessage::{
    ...
    OrganizingCityWarResultDispatch,
...
pub(crate) struct WorldMainLoopCallbacks<'a, TimerCallback> {
    ...
    /// Сырой `CCountry::m_bIsWarring=false` в точном city-war end call-site.
    pub(crate) clear_city_war_country_warring: &'a mut dyn FnMut(u8),
...
impl CGame {
    ...
    /// `0x60138` и Goods War command `0x60139` исполняются; остальные остаются
    /// owned pending. Terminal actions применяются FIFO до следующего сообщения.
