...
//!
//! `Initialize` RVA `0x0006FFA0`, `GetCityState` `0x0006D490` и
//! `GetCityStateByWarNum` `0x0006D6D0`, clear/refresh callbacks
//! `0x0006D5B0/0x0006D640` и фазовые start/end/declare/mass callbacks
//! `0x0006EC40/0x0006EEA0/0x0006F020/0x0006F2D0` имеют статус `IMPLEMENTED`;
//! countdown `0x0006F530/0x0006F800` и city end/weekly rearm `0x000712C0`
//! и result callback `OnFacWinCity` `0x00071620`, а также заявка
//! `OnPlayerDeclareWar` `0x0006FAE0`, `Reload` `0x00071D90` и полный
//! `AddToByteArray` snapshot `0x0006D3B0` имеют статус `IMPLEMENTED`. Точная пара:
//! `WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb`; source
//! `e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp`.
//!
//! PDB задаёт static `map<long,tagAttackCityTime>`, а запись размером `0xBC`:
...
//! `0x0046E1EF` подтверждает `true`. Active-state duplicate scan
//! `0x0006E200` и relation rebuild `0x0006DD50/0x0006E330/0x0006E440`
...
//! исходник безусловно разыменовывал nullable faction на `0x004719F1`, поэтому
...
//! рассылает exact `0x7FE35`, пересобирает enemy relations и публикует
...
//! читается. EXE `0x0046FF85` возвращает `true` только после log; отказ —
...
//! `Option<TimerId>` локально блокирует неизвестный старый raw ID. Exact
//! `0x00471FB2` возвращает `true` независимо от старого Initialize-result;
...
pub(crate) trait AttackCityPhaseContext {
    ...
    /// `-366/0xFFFF0000`, при флаге ставит country региона в war и пишет log;
...
/// Параметры exact timer-2 top-info countdown-а городских войн.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
pub(crate) trait AttackCityEnemyRelationContext {
    ...
    /// Возвращает owner faction живого region либо `None` для всех raw gates.
...
pub(crate) trait AttackCityApplicationContext: AttackCityEnemyRelationContext {
    ...
    /// с legacy arguments `-1, -1, 0xFFFF0000`.
...
impl CAttackCitySys {
    ...
    pub(crate) fn add_to_byte_array(&self, output: &mut Vec<u8>) -> bool {
        ...
        for setup in self.attacks.values() {
            ...
            // RVA 0x0006D3B0 копирует ещё четыре неинициализированных bytes
            // пустого allocator-base `std::list`. GameServer RVA 0x00060E40
    ...
    /// Дописывает один exact faction-list snapshot для GameServer.
    ...
    /// Рассылает exact `0x7FE22`; первый legacy parameter не читался.
    ...
    /// Выполняет полный достигнутый end/relation/weekly-rearm порядок.
    ...
    where
        ...
        Context: AttackCityWarEndContext + ?Sized,
    {
        // BLOCKED_MISSING_FACT: raw `operator[]` default-constructs отсутствующую
        ...
        Context: AttackCityWarResultContext + ?Sized,
    {
        ...
        let result_log = if winner_faction_id <= 0 {
            ...
            if owner_faction_id == winner_faction_id {
                if !old_owner_exists {
                    // RVA 0x00071620, exact 0x004719F1: null result
...
// Исходный владелец PDB: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp

// ============================================================================
// FUNCTION: CAttackCitySys::OnCityWarEndToGameServer
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:634
// RVA: 0x0006D100
//
// IMPLEMENTED выше: первый long не читается, второй отправляется единственным
// signed полем broadcast-сообщения `0x7FE22`.
//
// ============================================================================
// FUNCTION: CAttackCitySys::AddToByteArray
// STATUS: IMPLEMENTED / VERIFIED_DISASSEMBLY
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:391
// RVA: 0x0006D3B0
//
// IMPLEMENTED выше: u32 count, map-order prefix `0xB0`, u32 list count и
// ordered signed IDs. Парный GameServer decoder RVA `0x00060E40` доказывает
// совместимость нулевой нормализации list allocator/event-ID residue.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:713
// RVA: 0x0006D5B0
//
// IMPLEMENTED выше как `on_clear_other_player`: существующий ID получает
// ровно один broadcast `0x7FE24`.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:723
// RVA: 0x0006D640
//
// IMPLEMENTED выше как `on_refresh_region`: существующий ID получает ровно
// один broadcast `0x7FE25`.
//
// ============================================================================
// FUNCTION: CAttackCitySys::SetCityWarEnemyFactions
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:1192
// RVA: 0x0006DD50
//
// IMPLEMENTED выше: empty-list и region/owner gates, faction-or-union expansion
// и два ordered AddCityWarEnemyOrganizing вызова на каждую пару сохранены;
// region/organizing virtual owners выражены узким context-контрактом.
//
// ============================================================================
// FUNCTION: CAttackCitySys::UpdateApplyWarFacsToGameServer
// STATUS: IMPLEMENTED / VERIFIED_DISASSEMBLY
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:411
// RVA: 0x0006E0F0
//
// IMPLEMENTED выше: signed war ID, 32-bit count и ordered signed faction IDs;
// missing schedule кодируется пустым списком. Exact EXE `0x0046E1EF` задаёт
// `mov al,1` после cleanup, поэтому возвращаемое значение доказанно `true`.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:880
// RVA: 0x0006E200
//
// IMPLEMENTED выше: map-order scan учитывает только state != CIS_NO и ищет
// точный signed faction ID в ordered `DecWarFactions`.
//
// ============================================================================
// FUNCTION: CAttackCitySys::InitialCityAllFactionEnemyRelation
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:1161
// RVA: 0x0006E330
//
// IMPLEMENTED выше: сначала общий clear, затем signed map-order schedules со
// state != CIS_NO и полный SetCityWarEnemyFactions для каждой записи.
//
// ============================================================================
// FUNCTION: CAttackCitySys::UpdateCityAllFactionEnemyRelation
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:1180
// RVA: 0x0006E440
//
// IMPLEMENTED выше в точном порядке: changed=false, полный initial rebuild,
// затем UpdateAllCityEneFacRelation.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:494
// RVA: 0x0006EC40
//
// IMPLEMENTED выше как `on_attack_city_start`: Fight -> `0x7FE20` ->
// region notice/log сохраняют исходный порядок.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:522
// RVA: 0x0006EEA0
//
// IMPLEMENTED выше как `on_attack_city_end`; только Fight переходит в No,
// после чего следуют `0x7FE21` и локализованный end-log.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:642
// RVA: 0x0006F020
//
// IMPLEMENTED выше как `on_declare_war`: Duth -> `0x7FE1F` -> region notice,
// country flag и war-log сохраняют исходный порядок.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:683
// RVA: 0x0006F2D0
//
// IMPLEMENTED выше как `on_mass`: Mass -> `0x7FE23` -> region notice/log
// сохраняют исходный порядок.
//
// ============================================================================
// FUNCTION: CAttackCitySys::OnAttackCityStartInfo
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:734
// RVA: 0x0006F530
//
// IMPLEMENTED выше: city region gate, StartTime > now и timer-2 top-info
// `WS0142`; duration использует только minute/second разности.
//
// ============================================================================
// FUNCTION: CAttackCitySys::OnAttackCityEndnfo
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:763
// RVA: 0x0006F800
//
// IMPLEMENTED выше как `on_attack_city_end_info`: city region gate,
// EndTime > now и timer-2 top-info `WS0143` в исходном порядке.
//
// ============================================================================
// FUNCTION: CAttackCitySys::OnPlayerDeclareWar
// STATUS: IMPLEMENTED, VERIFIED_DISASSEMBLY
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:932
// RVA: 0x0006FAE0
//
// Реализация выше сохраняет ordered gates, level-rejection, mutation,
// `0x7FE35`, relation rebuild, WS0103/WS0145 и WS0146. Exact 0x0046FE28
// подтверждает `m_btCountry +0x80`, 0x0046FF85 — AL=1 только после success;
// отказы возвращают AL=0. Третий long не читается; неиспользуемый owner lookup
// и STL/string/SEH cleanup удалены как не имеющие наблюдаемого эффекта.
//
// ============================================================================
// FUNCTION: CAttackCitySys::OnCityWarEnd
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:541
// RVA: 0x000712C0
//
// IMPLEMENTED выше: clear заявок, country/enemy-relation context, до пяти
// weekly collision attempts и восемь SetTimeEvent в исходном порядке.
// BLOCKED_MISSING_FACT локализует только default construction при missing ID.
//
// ============================================================================
// FUNCTION: CAttackCitySys::OnFacWinCity
// STATUS: IMPLEMENTED, VERIFIED_DISASSEMBLY
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:1017
// RVA: 0x00071620
//
// Реализация выше сохраняет обязательный входной diagnostic, exact gates,
// end-owner calls, ownership/country mutations и порядок WS0147..WS0153.
// Vtable CFaction подтверждена constructor-ом и call sites; STL/string/SEH
// cleanup удалён как compiler/library noise. Same-owner null dereference на
// 0x004719F1 локализован как MissingCurrentOwnerForDefence.
//
// ============================================================================
// FUNCTION: CAttackCitySys::Reload
// STATUS: IMPLEMENTED, VERIFIED_DISASSEMBLY
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\attackcitysys.cpp:448
// RVA: 0x00071D90
//
// Реализация выше сохраняет восемь ordered schedule-kill, повторный tax-kill,
// snapshot активных записей, OnCityWarEnd, `0x7FE22` и Initialize. Option
// локализует неизвестные raw IDs без отката предыдущих эффектов. Exact
// 0x00471FB2 подтверждает постоянный true; STL tree-copy/iterator/allocator и
// singleton-allocation/SEH cleanup не получают отдельных Rust-аналогов.
//
