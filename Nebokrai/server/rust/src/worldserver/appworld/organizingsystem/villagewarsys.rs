...
//!
//! `Initialize` RVA `0x0006BA20`, constructor `0x00069C10`,
//! `GetRegionState` `0x00068EF0`, `IsVilRegionLeftClearTime` `0x00068FA0` и
//! `GetCityStateByWarNum` `0x000690C0`, фазовые callbacks declare/start/end
//! `0x00069E00/0x0006A040/0x0006A240` и clear-player `0x0006A480` имеют
//! статус `IMPLEMENTED`. Countdown callbacks `0x0006A510/0x0006A7D0` и weekly
//! end/rearm `0x0006AA90`, результат `OnFacWinVillage` `0x0006AFE0` и заявка
//! `ApplyForVillageWar` `0x0006CCF0`, `ReLoad` `0x0006CB70` и полный
//! `AddToByteArray` snapshot `0x00068E00` имеют статус `IMPLEMENTED`. Точная пара:
//! `WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb`; source
//! `e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp`.
//!
//! PDB задаёт `tagVilWarSetup` размером `0x98`: signed ID/region IDs, шесть
...
//! `union.cpp` достигнуты здесь: запись создаётся только после разбора всех
...
//! snapshot `0x00069990` дописывает signed war ID, 32-битный count и ordered
//! signed faction IDs; exact EXE `0x00469A83` подтверждает итоговый `true`.
//! `IsAlreadyDeclarForWar` `0x00069AE0` ищет faction только в schedule со
...
//! exact `0x7FE36` snapshot и публикует `WS0289`. Level-rejection использует
//! title `WS0121` и text `WS0288`; третий `long` не читается. Exact EXE
//! `0x0046D0D5` возвращает `true` только после war-log, все отказы — `false`.
...
//! очисткой map; странность сохранена. Exact `0x0046CCE0` принудительно
...
//! арифметика возвращают локальный `BLOCKED_MISSING_FACT`.
...
pub(crate) trait VillageWarPhaseContext {
    ...
    /// отправляет organizing-info с `-366/0xFFFF0000`, затем при указанном
...
/// Параметры одного exact timer-2 top-info countdown-а.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
/// Граница region lookup и уже достигнутого organizing top-info owner-а.
...
pub(crate) trait VillageWarResultContext {
    ...
    /// Читает virtual owner getter только в достигнутом исходном месте.
...
impl CVillageWarSys {
    ...
    pub(crate) fn add_to_byte_array(&self, output: &mut Vec<u8>) -> bool {
        ...
        for setup in self.village_wars.values() {
            ...
            // RVA 0x00068E00 копирует ещё четыре неинициализированных bytes
            // пустого allocator-base `std::list`. GameServer RVA 0x0005F320
    ...
    pub(crate) fn on_attack_village_end<Context: VillageWarPhaseContext + ?Sized>(
        ...
        // RVA 0x0006A240 снова ставит `true` всем четырём странам. Это
...
fn new_village_schedule(
    ...
    // уже известными полями. Это устраняет внутреннее UB, не меняя достигнутое
...
// Исходный владелец PDB: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp

// ============================================================================
// FUNCTION: CVillageWarSys::AddToByteArray
// STATUS: IMPLEMENTED / VERIFIED_DISASSEMBLY
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:382
// RVA: 0x00068E00
//
// IMPLEMENTED выше: u32 count, map-order prefix `0x8C`, u32 list count и
// ordered signed IDs. Парный GameServer decoder RVA `0x0005F320` доказывает
// совместимость нулевой нормализации list allocator/event-ID residue.
//
// ============================================================================
// FUNCTION: CVillageWarSys::UpdateApplyWarFacsToGameServer
// STATUS: IMPLEMENTED / VERIFIED_DISASSEMBLY
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:402
// RVA: 0x00069990
//
// IMPLEMENTED выше: signed war ID, 32-bit count и ordered signed faction IDs;
// missing schedule кодируется пустым списком. Exact EXE `0x00469A83` задаёт
// `mov al,1` после cleanup, поэтому возвращаемое значение доказанно `true`.
//
// ============================================================================
// FUNCTION: CVillageWarSys::GetVilWarSetupByTime
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:480
// RVA: 0x00069AA0
//
// IMPLEMENTED выше как `get_village_war_setup_by_time`: miss возвращает None,
// hit клонирует все поля записи, включая ordered declaring factions.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:1053
// RVA: 0x00069AE0
//
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:502
// RVA: 0x00069E00
//
// IMPLEMENTED выше как `on_declare_war`: state, `0x7FE2F`, region notice,
// country flags и war-log сохраняют исходный порядок.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:539
// RVA: 0x0006A040
//
// IMPLEMENTED выше как `on_attack_village_start`; внешний notice/log owner
// вызывается после обязательной state-мутации и `0x7FE30` broadcast.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:564
// RVA: 0x0006A240
//
// IMPLEMENTED выше как `on_attack_village_end`; странная повторная установка
// `m_bIsWarring=true` странам `1..=4` сохранена в contract-е context-а.
//
// ============================================================================
...
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:601
// RVA: 0x0006A480
//
// IMPLEMENTED выше как `on_clear_player`: существующий ID рассылается одним
// `0x7FE33`, отсутствующий не производит эффектов.
//
// ============================================================================
// FUNCTION: CVillageWarSys::OnAttackVillageStartInfo
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:612
// RVA: 0x0006A510
//
// IMPLEMENTED выше: region gate, StartTime > now, component-wise difference,
// timer-2 top-info `WS0286`; hours/days исходно не входят в duration.
//
// ============================================================================
// FUNCTION: CVillageWarSys::OnAttackVillageEndInfo
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:641
// RVA: 0x0006A7D0
//
// IMPLEMENTED выше: region gate, EndTime > now, component-wise difference,
// timer-2 top-info `WS0287`; add и broadcast сохраняют исходный порядок.
//
// ============================================================================
// FUNCTION: CVillageWarSys::OnVillageWarEnd
// STATUS: IMPLEMENTED
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:935
// RVA: 0x0006AA90
//
// IMPLEMENTED выше: `0x7FE32`, clear заявок, до пяти closed-overlap попыток
// по 7 дней и шесть SetTimeEvent в исходном порядке. Второй long не читался.
// Ошибка safe calendar arithmetic сохраняет уже выполненные эффекты.
//
// ============================================================================
// FUNCTION: CVillageWarSys::OnFacWinVillage
// STATUS: IMPLEMENTED, VERIFIED_DISASSEMBLY
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:835
// RVA: 0x0006AFE0
//
// Реализация выше сохраняет точные gates, два KillTimeEvent, общий end-owner,
// ownership/counter mutations и порядок WS0290..WS0294. Vtable CFaction
// подтверждена constructor-ом и call sites; STL/string/SEH cleanup удалён как
// compiler/library noise. Четвёртый входной long доказанно не читается.
//
// ============================================================================
// FUNCTION: CVillageWarSys::ReLoad
// STATUS: IMPLEMENTED, VERIFIED_DISASSEMBLY
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:437
// RVA: 0x0006CB70
//
// Реализация выше сохраняет map-order, шесть ordered kill, snapshot активных
// записей, OnVillageWarEnd и последующий Initialize. Неинициализированный raw
// event ID локализован через Option; уже выполненные эффекты не откатываются.
// Exact 0x0046CCE0 подтверждает постоянный true после Initialize. STL tree-copy,
// iterator, allocator и SEH cleanup удалены как compiler/library noise.
//
// ============================================================================
// FUNCTION: CVillageWarSys::ApplyForVillageWar
// STATUS: IMPLEMENTED, VERIFIED_DISASSEMBLY
// COMPONENT: WorldServer
// ARTIFACT: WorldServer/Nworldserver.exe + WorldServer/WorldServer.pdb
// SOURCE: e:\svn\fengyun_russia_dev\server\worldserver\appworld\organizingsystem\villagewarsys.cpp:767
// RVA: 0x0006CCF0
//
// Реализация выше сохраняет ordered gates, level-rejection, mutation,
// `0x7FE36`, WS0289 и war-log. Exact 0x0046D0D5 устанавливает AL=1 только
// после полного success-path; 0x0046CED4 задаёт AL=0 всем отказам. Третий long
// не читается; STL/string/SEH cleanup удалён как compiler/library noise.
//
