...
//
// Реализовано `add_base_object`: exact direct jump приходит в base
// `CContainer::Remove`, который возвращает null без side effects.
// Полный декомпилят сохранён в локальном исследовательском корпусе.
//
//
...
//
// VERIFIED_DISASSEMBLY: exact `0x004E0690..0x004E07E2` сначала вызывает
// virtual `GetGoods(position)`. Occupied ветвь пишет diagnostic и возвращает
// `0`; free ветвь не вставляет goods и возвращает `(incoming != nullptr)`.
// Реализовано `add_from_db_base` с explicit diagnostic callback; null incoming
// на occupied ветви становится typed safe boundary вместо raw dereference.
// Полный декомпилят сохранён в локальном исследовательском корпусе.
//
//
...
//
// Реализовано `remove_from_position`: split получает amount и listener callback
// до уменьшения stored goods, full amount делегирует concrete GUID removal.
// `GoodsContainerPositionStorage` заменяет raw virtual slots/RTTI, не меняя
// position, amount, return-null или порядок side effects.
// Полный декомпилят сохранён в локальном исследовательском корпусе.
//
//
