//! `IMPLEMENTED` — наблюдаемый контракт `CGasOperator::GetIP` из
...
//! Исходные пути PDB:
//! `d:\complite_version\fengyun_russia\trunk\server\loginserver\applogin\gasoperator.cpp/.h`.
//! Машинный код RVA 0x421500 подтверждает порядок октетов: четыре байта
//! исходного DWORD читаются от младшего к старшему и печатаются десятичными
//! числами через точки.
//!
