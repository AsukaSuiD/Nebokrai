...
#[derive(Clone)]
pub(crate) struct PlayerFillDatabaseSettings {
    host: Vec<u8>,
    database: Vec<u8>,
    user: Vec<u8>,
    password: Vec<u8>,
...
pub(crate) struct PlayerFillDatabaseSettingsParts {
...
pub(crate) struct PlayerFillInfo {
...
pub(crate) enum RsPlayerFillOperation {
    GetPlayerFillLog,
    DeletePlayerFillLog,
}
...
#[derive(Debug)]
pub(crate) struct RsPlayerFillNotice {
    pub(crate) operation: RsPlayerFillOperation,
    pub(crate) error: RsPlayerFillDatabaseError,
}

/// Ошибка создания синхронного Linux/TDS-владельца.
#[derive(Debug)]
pub(crate) struct RsPlayerFillInitializationError(io::Error);
...
#[derive(Debug)]
pub(crate) enum RsPlayerFillDatabaseError {
    Connect(io::Error),
...
pub(crate) trait RsPlayerFillOwner {
...
pub(crate) struct TiberiusRsPlayerFillMgr {
    config: Config,
    runtime: Runtime,
