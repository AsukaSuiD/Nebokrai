...
//!
//! BLOCKED_MISSING_FACT: при отказе `_beginthreadex` оригинальный `CGame::Init`
//! всё равно сохранял запись с null handle и продолжал. Доказанного полезного
//! поведения этой resource-exhaustion ветви нет; Rust останавливает уже
//! созданные workers и возвращает локальную start-ошибку, не объявляя её
//! совместимой реакцией baseline.
//!
//! BLOCKED_MISSING_FACT: ADO-код копировал ровно 80 байт `@Assure` и читал
...
//! локально даёт обычный DB fallback `-2`; это безопасная runtime-граница, а не
//! заявленное поведение оригинала.
//!
//! BLOCKED_MISSING_FACT: `SystemTimeToVariantTime` для некорректных полей
...
//! некорректную дату не объявляется совместимой и остаётся локальной DB-ошибкой.
