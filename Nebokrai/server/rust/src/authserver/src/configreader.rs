...
//!
//! BLOCKED_MISSING_FACT: при malformed numeric token MSVC `num_get` мог
//! потребить только prefix, выставить особое сочетание state bits либо изменить
//! destination при overflow. Для корректной поставки все двенадцать numeric
//! token независимо проверены как целые нужного диапазона и три bool равны
//! `0/1`. Rust возвращает локальную ошибку поля, не выдавая эту реакцию за
//! поведение EXE. Редкая ошибка чтения уже открытого файла также свёрнута в
