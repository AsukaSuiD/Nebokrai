...
//! `QUIT` до следующей команды текущего snapshot. Остальные локальные
//! неизвестности только возвращаются в `ServerSnapshotError` и не получают
//! придуманной сетевой политики. Превышение суммарного receive-rate остаётся
//! общим guard-путём с тем же ban/QUIT.
//!
//! `OnMapStrError` публикует то же synthetic сообщение `0x10001` с
//! NUL-terminated строковой identity, что и disconnect клиента. Старые
...
use crate::nets::netlogin::mynetserverclient_client::{
...
pub(crate) struct CMyNetServerClient {
    base: CServer,
...
impl CMyNetServerClient {
    ...
    pub(crate) fn configure_receive(
        &mut self,
