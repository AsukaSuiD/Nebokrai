...
wait_for_peer miracle_world 1747
wait_for_peer miracle_billing 8188

echo "GameServer: запуск gameserver.exe через Wine"
if [ ! -f "$WINEPREFIX/system.reg" ]; then
    export MIRACLE_INITIALIZE_WINE=1
else
    export MIRACLE_INITIALIZE_WINE=0
fi
...
if [ "$MIRACLE_INITIALIZE_WINE" = 1 ]; then
