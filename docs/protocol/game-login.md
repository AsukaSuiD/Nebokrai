...
Непустые контейнеры требуют отдельной сверки: текущий `append_old_client_volume` пишет только счётчик и записи `CGoods`. Оригинальный `CPlayer::CPacketListener::OnTraversingContainer` (VA `0x0042D2F0`) перед каждой записью добавляет признак экипировки (`byte`), количество (`ushort`) и направление (`byte`); эти четыре байта также читает клиент, например перед предметом боевой феи по VA `0x00451A73–0x00451A90`. Это установленное несоответствие текущего helper-а, а не разрешённое упрощение формата. Контейнер выставленных на аукцион предметов имеет другую форму префикса; применять исправление вслепую ко всем вызовам helper-а нельзя. В записи входа, разобранной в [аудите](../status/audit.md#сдвиг-пакета-входа-перед-заданиями), эти контейнеры пусты.

Направление, внешний порядок блоков, перечисленные размеры, два булевых флага перед заданиями и один байт `country_identity` имеют конкретные статические свидетельства. Полный вложенный формат player snapshot, значения непрозрачных setup-диапазонов и текстовая кодировка имени региона остаются `PARTIAL`/`UNKNOWN` в названных местах. Наблюдённый сбой при сдвиге конкретного пакета описан в состоянии проекта; он не задаёт общей реакции клиента на любое повреждение.
