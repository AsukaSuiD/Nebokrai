...
`0xBF401` отправляет **GameServer клиенту конкретного игрока** при завершении входа, а также в короткой ветви отказа. Это не входящее сообщение WorldServer: тот передаёт GameServer результат `0x7F901`, после которого GameServer строит клиентский ответ. Смысл статусов `VERIFIED`, `PARTIAL` и `UNKNOWN` указан в [правилах свидетельств](../reconstruction/evidence-and-contracts.md). Подтверждённый оригинальный формат не означает проверенный обмен нового сервера с исходным клиентом.
...
[Диспетчер `logmessage`](../../server/rust/src/gameserver/appserver/message/logmessage.rs) принимает `0x7F901` от WorldServer. Положительный status служит ID игрока; требуется действующий client-route. Далее декодируется GameSave, проверяются ID и отсутствие дубликата, подбирается регион, создаётся player snapshot. Только после успешного snapshot [GameServer](../../server/rust/src/gameserver/gameserver/game.rs) формирует полный `0xBF401` и вызывает `send_to_player`. Перед этим выполняются login-script и выбор начального навыка; после отправки идёт отдельное сообщение BillingServer `0xEF201`. Эти условия имеют статус `VERIFIED` для достигнутого пути owner-ов; реакции на любые ещё не восстановленные входные состояния — `UNKNOWN`.
