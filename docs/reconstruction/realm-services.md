...
| `CRsPlayer::OpenPlayerBaseInDB` | pub `1:0010c2d0`, VA `0x50D2D0..0x50E915` | SQL `SELECT * FROM csl_player_base WHERE Account='%s' ORDER BY id` (литерал `0x54E5E0`); строка: index через `Add(short) 0x423B90` (word из long& count), id — `Add(ulong) 0x423C00`, name — `Add(cstring) 0x423CE0` (writer сам дописывает NUL), пять `Add(byte) 0x423B30` (level/occupation/sex/country/head), 11×`Add(ulong)` equipment id, 11×`Add(byte)` equipment level, `Add(long)` region, `Add(char) 0x423AD0` deletion-status, `count++` по long& | `player_base` |
| `CRsPlayer::OpenPlayerBaseInMem` | pub `1:0010e280`, VA `0x50F280..0x50F6E8` | Тот же формат строки для объектов стадии создания: deletion-status только через `CGame::GetDeletionPlayerTime` (`0x405560`), без restore-проверки и без DB-fallback; копия живого объекта удаляется deleting dtor после чтения | `player_base` |
| deletion-status (дефолт -1; формула) | инициализация `0x50D36C`; формула `0x50D5BE..0x50D60B` (InDB), `0x50F4E9..0x50F531` (InMem) | Статус инициализируется `0xFF`; InDB: `CGame::IsRestorePlayerExist` (`0x405530`) → ветвь -1; `GetDeletionPlayerTime` == 0 → `CRsPlayer::GetPlayerDeletionDate` (pub `1:000ffea0`); итог 0 → -1. Формула: `time(0)` (`0x51C934`), difftime (`0x51F594`), `fmul qword [0x54E5D8]` (double `1.1574074074074073e-05` = 1/86400), ftol (`0x51B278`), `sub dl, al` младших байтов: `dl = byte [0x6BA19C]` (= `CGlobeSetup::m_stSetup` static 3:0x14fc80 + `0x51C` = `deletion_days`) минус младший байт дней; `jns` сохраняет знаковый результат, иначе clamp к 0 | `player_base` |
| equipment-снимок живого объекта | helper VA `0x401B50` | Слот контейнера `[player+0x154]` [`+0x60` virtual] в порядке 0, 1, 3, 4, 2, 9, 0xA, 0xC, 0xD, 0xE, 0xF = HELM, BODY, GLOV, BOOT, WEAPON, BACK, HEADGEAR, FROCK, WING, MANTEAU, FAIRY; id = dword `[goods+0x6C]` (`0x4516B0`), level = addon `0x30` value 1 (`0x4517B0`); отсутствующий предмет → пара 0/0 | `player_base` / `rsplayer` |
