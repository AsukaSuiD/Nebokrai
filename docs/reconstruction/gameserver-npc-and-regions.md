...
| `source_property` по типу владельца | vtable-контракты тел выше + `combat/monsterformula` | PARTIAL (собственные тела getter-ов CPlayer отдельно не пересверялись) | тип 400 — боевые поля игрока; тип 600 — state-границы MIN/MAX и SOUL монстра, Element и CCH всегда 0 (`GetAddElementAtk` монстра умножает pet factor на ноль); типы 500/1100/1200 — 0 |
