...
Здесь описан действующий Rust-код. Байты и числовые ограничения находятся в [протоколе](../protocol/README.md), порядок тактов — в [серверных циклах](runtime-ordering.md), запуск процесса — в [модели процессов](process-model.md).
...
`ServerSnapshot::into_parts` возвращает одновременно I/O-действия и ошибки; ошибка парсера не отменяет остальные действия. `ServerIoCompletion` сообщает EOF, ошибку чтения или результат записи. Процессные `report_*_step` выводят часть этих результатов в stderr. В частности, [Auth](../../server/rust/src/process/authserver.rs) и [Billing](../../server/rust/src/process/billingserver.rs) не печатают успешный `SendEnded`, даже когда его `ServerSendCompletion` содержит потерянный частичный хвост. Отсутствие ошибки в журнале не подтверждает доставку. Политика частичной записи и закрытия задана [отдельно](../protocol/connection-lifecycle.md).

Наличие вызова `tracing` в нижнем слое также не доказывает фактический вывод: подключение журналирования и текущие пробелы описаны в [диагностике](../operations/diagnostics.md). Это ограничение наблюдаемости текущего кода, а не выбранный стандарт для новых обработчиков.
