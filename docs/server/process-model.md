...
Существующий пакет `nebokrai-server` собирает шесть программ: Auth, Login, World, Game, Billing и Misc. Он использует выделенные библиотеки `nebokrai-shared`, `nebokrai-realm` и `nebokrai-zone`; все части входят в workspace [Cargo.toml](../../server/rust/Cargo.toml). Доменный код при этом уже в основном вынесен в библиотеки по [принятой карте](../architecture/realm-and-zone.md): пакет остаётся переходным слоем процессных входов и ещё не делегированных hub-ов, а выделение библиотек не меняет число процессов и их жизненный цикл. Эта страница описывает текущий запуск шести бинарников переходного периода, а не конечную архитектуру.
