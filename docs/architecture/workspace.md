...
| [server/rust/realm/](../../server/rust/realm/) | Библиотека `nebokrai-realm`: `src/content/` владеет ресурсами сценариев, выбирает индексный или дисковый список и выполняет загрузку; `src/app/` владеет сообщением направления Login/GameServer ↔ WorldServer (`world_message.rs`). Зависит от Shared, `parking_lot` и `walkdir`, не импортирует прежний серверный пакет. Собственного бинарного входа пока нет. |
