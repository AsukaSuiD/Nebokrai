...
На этом пути наблюдались список персонажей, загрузка и регистрация игрока в Game; устойчивый игровой сеанс пока не подтверждён. Следующая страница — [текущий статус](status.md): она отделяет достигнутые этапы от следующих проверок. [Case study](../reconstruction/case-study-region-entry.md) показывает разбор одного контракта, а [подробный аудит](../status/audit.md#выбор-персонажа-в-локальном-клиенте) хранит наблюдения.
