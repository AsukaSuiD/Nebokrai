...
По умолчанию скрипт выполняет `cargo check --locked --lib --bins`: проверяет типы и заимствования библиотеки и шести серверных входов без линковки и запуска служб. `-Mode Build` выполняет `cargo build --locked --bins` и копирует шесть dev-бинарников в `.local/rust-bin` для [совместного запуска](rust-runtime.md). Используется локальный контекст `desktop-linux`, образ `rust:1.97.1-bookworm`, исходники монтируются только для чтения.
