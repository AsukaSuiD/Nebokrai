...
Скрипт выполняет `cargo check --locked --lib --bins`: проверяет типы и заимствования библиотеки и шести серверных входов без линковки и запуска служб. Используется локальный контекст `desktop-linux`, образ `rust:1.97.1-bookworm`, исходники монтируются только для чтения.
